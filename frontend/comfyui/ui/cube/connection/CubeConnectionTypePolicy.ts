...
export interface CubeConnectionLiteGraph {
  isValidConnection?(outputType: unknown, inputType: unknown): boolean;
}

export interface CubeConnectionCompatibility {
  accepts(outputType: unknown, inputType: unknown, strict?: boolean): boolean;
}

export interface CubeConnectionTypePolicyOptions {
  getLiteGraph(): CubeConnectionLiteGraph | null;
  logger: Pick<Console, 'debug'>;
}

/** Delegate connection compatibility to Comfy while retaining deterministic fallback semantics. */
export class CubeConnectionTypePolicy implements CubeConnectionCompatibility {
  readonly #getLiteGraph: () => CubeConnectionLiteGraph | null;
  readonly #logger: Pick<Console, 'debug'>;

  /** Bind the active Comfy type system at its dynamic host boundary. */
  constructor(options: CubeConnectionTypePolicyOptions) {
  ...
  accepts(outputType: unknown, inputType: unknown, strict = false): boolean {
    ...
    try {
      ...
      if (liteGraph?.isValidConnection) {
        if (strict) {
          ...
          return (
            liteGraph.isValidConnection(output, input) && liteGraph.isValidConnection(input, output)
          );
        }
