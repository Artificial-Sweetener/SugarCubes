...
import {
  CubeSubgraphRegistrar,
  type CubeSubgraphRegistrationResult,
} from './CubeSubgraphRegistrar.js';
...
import { CubeVersionRepository, type CubeVersionApi } from './version/CubeVersionRepository.js';
...
import type { CubeIdentitySource } from './CubeIdentityPresentation.js';
...
import type { CubeWorkflowNodePackMetadata } from './node/CubeWorkflowNodePackMetadata.js';

export interface ComfyCubeRuntimeOptions {
  app: unknown;
  api: unknown;
  cubeApi: CubeVersionApi;
  liteGraph: unknown;
  document: Document;
...
export interface ComfyCubeRuntime {
  ...
  cardReveal: CubeCardRevealService;
  versionAvailability: CubeVersionAvailabilityService;
  versionSwitch: CubeVersionSwitchService;
  ...
  registerSubgraphs(payload: ImportPayload): CubeSubgraphRegistrationResult;
...
interface CubeRuntimeGraph {
  ...
  getLink(id: string | number): import('../types/graph.js').ComfyLink | null;
...
export function createComfyCubeRuntime(options: ComfyCubeRuntimeOptions): ComfyCubeRuntime {
  ...
  const subgraphRegistrar = new CubeSubgraphRegistrar({
    ...
    discardSubgraph(id) {
      ...
      if (subgraph) discardSubgraph(subgraph);
    },
  });
...
function readPreviewEventSource(api: UnknownRecord): ComfyNodePreviewEventSource | null {
  ...
  const removeEventListener = requireFunction(
    api.removeEventListener,
    'Comfy API.removeEventListener',
  );
  return {
