...
import { requireCubeIdentity, type CubeNode } from './ComfyCubeNodeFactory.js';

export type CubeNodeAuthoringCandidate = CubeFaceGraphSummary;

/** Describe executable nodes and native boundaries without leaking Comfy graph objects. */
export function readCubeNodeAuthoringCandidate(node: CubeNode): CubeNodeAuthoringCandidate {
  ...
  return {
    nodeIds: nodes.map((entry, index) =>
      entry && typeof entry === 'object' && 'id' in entry ? entry.id : index,
    ),
...
export function buildCubeFaceChromeMetadata(node: CubeNode): CubeFaceChromeMetadata {
  return {
