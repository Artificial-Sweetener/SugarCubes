...
export interface CubeOutputPromptAdapterOptions {
  getCubes(): readonly CubeNode[];
  boundaryResolver: NativeSubgraphBoundaryResolver;
}

interface CubeExecutionIdentity {
  cubeId: string;
  defaultAlias: string;
...
export class CubeOutputPromptAdapter {
  ...
  constructor(options: CubeOutputPromptAdapterOptions) {
  ...
  apply(payloadValue: unknown): unknown {
    ...
    for (const cube of this.#getCubes()) {
      ...
      for (let slot = 0; slot < cube.subgraph.outputs.length; slot += 1) {
        ...
        if (!source) {
          throw new Error(
            `Cube '${String(cube.id)}' output ${String(slot)} has no executable source.`,
          );
        }
    ...
    if (additions.size === 0) return payloadValue;
    return {
      ...payloadValue,
      output: {
        ...payloadValue.output,
...
function readExecutionIdentity(cube: CubeNode): CubeExecutionIdentity {
...
function requireString(value: unknown, field: string, cube: CubeNode): string {
...
function readOutputName(value: unknown, slot: number): string {
