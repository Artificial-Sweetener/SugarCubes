...
export type ComfyExecutedEventListener = (event: unknown) => void;

export interface ComfyExecutionEventApi {
...
export interface ComfyCubeOutputEventBridgeOptions {
  api: ComfyExecutionEventApi;
...
export class ComfyCubeOutputEventBridge {
  readonly #api: ComfyExecutionEventApi;
  ...
  constructor(options: ComfyCubeOutputEventBridgeOptions) {
  ...
  readonly #handleExecuted: ComfyExecutedEventListener = (event) => {
    ...
    if (!executionId || !('output' in detail)) return;
