...
export function buildCubeOutputExecutionId(cubeNodeId: GraphId, outputSlot: number): string {
...
export function isCubeOutputExecutionId(value: unknown): value is string {
  if (typeof value !== 'string' || !value.startsWith(CUBE_OUTPUT_EXECUTION_LEADER)) return false;
  const slotSeparator = value.lastIndexOf(':');
  if (slotSeparator <= CUBE_OUTPUT_EXECUTION_LEADER.length) return false;
