...
export interface CubeBoundaryTarget {
  symbol: string;
  input: string;
}

export interface CubeBoundarySource {
  symbol: string;
  slot: number;
}

export interface CubeInputBoundaryDefinition {
  id: string;
  name: string;
  label: string;
  type: string;
  targets: CubeBoundaryTarget[];
}

export interface CubeOutputBoundaryDefinition {
  id: string;
  name: string;
  label: string;
  type: string;
  source: CubeBoundarySource;
}

export interface CubeBoundaryDefinitions {
  inputs: CubeInputBoundaryDefinition[];
  outputs: CubeOutputBoundaryDefinition[];
}

/** Read complete validated boundary definitions, or reject the dynamic payload as legacy. */
export function readCubeBoundaryDefinitions(value: unknown): CubeBoundaryDefinitions | null {
...
function readInputBoundary(value: unknown): CubeInputBoundaryDefinition | null {
  if (!isRecord(value)) return null;
  ...
  if (!id || !name || !label || !type || !targets || targets.length === 0) return null;
  return { id, name, label, type, targets };
}

/** Read one persisted output boundary with its explicit source endpoint. */
function readOutputBoundary(value: unknown): CubeOutputBoundaryDefinition | null {
  if (!isRecord(value)) return null;
  const id = readString(value.id);
  const name = readString(value.name);
  const label = readString(value.label);
  ...
  if (!id || !name || !label || !type || !source) return null;
  return { id, name, label, type, source };
}

/** Read one typed internal input endpoint. */
function readTarget(value: unknown): CubeBoundaryTarget | null {
  if (!isRecord(value)) return null;
...
function readSource(value: unknown): CubeBoundarySource | null {
  if (!isRecord(value)) return null;
  const symbol = readString(value.symbol);
...
function readArray<T>(value: unknown, readEntry: (entry: unknown) => T | null): T[] | null {
