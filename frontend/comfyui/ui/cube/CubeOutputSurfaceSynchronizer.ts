...
interface CubeOutputSurfaceCatalog {
...
export class CubeOutputSurfaceSynchronizer {
  ...
  readonly #handleGraphChange = (event: Event): void => {
    if (!(event instanceof CustomEvent) || event.detail?.subType !== 'after-change') return;
    this.#synchronize();
  };
  ...
  #synchronize(): void {
    ...
    try {
      for (const node of this.#nodes.list()) {
        if (synchronizeCubeOutputSurfaceNames(node)) this.#nodes.changed(node);
      }
    } finally {
...
export function synchronizeCubeOutputSurfaceNames(node: CubeNode): boolean {
