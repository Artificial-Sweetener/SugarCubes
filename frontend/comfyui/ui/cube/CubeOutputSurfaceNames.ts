...
export interface CubeOutputSurfaceDescriptor {
  name: unknown;
  type: unknown;
}
...
export function deriveCubeOutputSurfaceNames(
  outputs: readonly CubeOutputSurfaceDescriptor[],
): string[] {
  const explicitNames = outputs.map(readExplicitSurfaceName);
  const reservedNames = new Set(explicitNames.filter((name): name is string => name !== null));
...
function readExplicitSurfaceName(output: CubeOutputSurfaceDescriptor): string | null {
  const name = readString(output.name);
  if (!name || isGenericTypeName(name, output.type)) return null;
  if (!name.startsWith(OUTPUT_BINDING_PREFIX)) return name;
