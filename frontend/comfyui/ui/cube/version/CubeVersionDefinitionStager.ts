...
import type { CubeSubgraphRegistrar } from '../CubeSubgraphRegistrar.js';
import { remapCubeEmbeddedSubgraphs } from './CubeEmbeddedSubgraphRemapper.js';

export interface StagedCubeVersionDefinitions {
...
export class CubeVersionDefinitionStager {
  readonly #registrar: Pick<CubeSubgraphRegistrar, 'register' | 'discard'>;
  readonly #createId: () => string;

  /** Bind native registration and runtime identity generation. */
  constructor(options: {
    registrar: Pick<CubeSubgraphRegistrar, 'register' | 'discard'>;
    createId(): string;
  }) {
  ...
  stage(payload: ImportPayload): StagedCubeVersionDefinitions {
    ...
    if (registration.warnings.length === 0) return staged;
    this.#registrar.discard(staged.definitionIds);
    throw new Error(
      `Cube embedded definitions could not be staged: ${registration.warnings.join(' ')}`,
    );
  }
