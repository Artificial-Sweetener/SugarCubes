...
import type { ConstructedCube, CubeConstructionOptions } from '../CubeConstructionService.js';
import type { ImportPayload } from '../../import/PlacementPayload.js';
import { requireCubeIdentity, type CubeNode } from '../node/ComfyCubeNodeFactory.js';
import { readInstanceId } from '../node/CubeNodeCatalog.js';
import { buildCubeVersionBoundaryMap } from './CubeVersionBoundaryMap.js';
import type { CubeVersionReplacementRequest } from './ComfyCubeVersionReplacementAdapter.js';
import { transferCubeVersionState } from './CubeVersionStateTransfer.js';
import type { CubeVersionOption } from './CubeVersionTypes.js';
import type { StagedCubeVersionDefinitions } from './CubeVersionDefinitionStager.js';

export interface CubeVersionOptionsProvider {
  list(cubeId: string, currentVersion: string): Promise<readonly CubeVersionOption[]>;
}

export interface CubeVersionArtifactSource {
  loadArtifact(cubeId: string, option: CubeVersionOption): Promise<ImportPayload>;
}

export interface CubeVersionConstructionOwner {
  construct(payload: ImportPayload, options?: CubeConstructionOptions): ConstructedCube;
  discard(constructed: ConstructedCube): void;
}

export interface CubeVersionReplacementOwner {
  replace(request: CubeVersionReplacementRequest): void;
}

export interface CubeVersionDefinitionStageOwner {
  stage(payload: ImportPayload): StagedCubeVersionDefinitions;
  discard(staged: StagedCubeVersionDefinitions): void;
}

/** Replace one persisted Cube instance with a selected available semantic version. */
export class CubeVersionSwitchService {
  readonly #availability: CubeVersionOptionsProvider;
  readonly #repository: CubeVersionArtifactSource;
  readonly #construction: CubeVersionConstructionOwner;
  readonly #replacement: CubeVersionReplacementOwner;
  readonly #definitions: CubeVersionDefinitionStageOwner;

  /** Bind focused collaborators for the version-switch use case. */
  constructor(options: {
    availability: CubeVersionOptionsProvider;
    repository: CubeVersionArtifactSource;
    construction: CubeVersionConstructionOwner;
    replacement: CubeVersionReplacementOwner;
    definitions: CubeVersionDefinitionStageOwner;
  }) {
  ...
  async switch(node: CubeNode, targetVersion: string): Promise<CubeNode> {
    ...
    if (!normalizedTarget) throw new Error('Cube version is required.');
    if (normalizedTarget === currentVersion) return node;
    ...
    if (!sourceOption) throw new Error(`Current Cube version v${currentVersion} is unavailable.`);
    if (!targetOption) throw new Error(`Cube version v${normalizedTarget} is unavailable.`);
    ...
    let constructed: ConstructedCube | null = null;
    try {
      ...
      if (constructed) this.#construction.discard(constructed);
...
function requireCompleteConstruction(constructed: ConstructedCube): void {
...
function readAlias(identity: Record<string, unknown>, node: CubeNode): string {
