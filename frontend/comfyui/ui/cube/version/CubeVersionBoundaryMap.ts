...
export interface CubeVersionBoundaryMap {
  inputIdBySlot: ReadonlyMap<number, string>;
  inputSlotById: ReadonlyMap<string, number>;
  outputIdBySlot: ReadonlyMap<number, string>;
  outputSlotById: ReadonlyMap<string, number>;
}

/** Resolve stable canonical boundary IDs against the slots actually exposed by a node. */
export function buildCubeVersionBoundaryMap(
  payload: ImportPayload,
  node: CubeNode,
): CubeVersionBoundaryMap {
  ...
  const outputIdByName = new Map(
    topology.outputs.map((boundary, index) => [outputNames[index] ?? boundary.name, boundary.id]),
  );
...
function mapSlotIds(
  ...
  slots.forEach((slot, index) => {
    ...
    if (id) mapped.set(index, id);
  });
  return mapped;
}

/** Invert a slot map while rejecting ambiguous persisted boundary identities. */
function invertUnique(values: ReadonlyMap<number, string>): Map<string, number> {
  const inverted = new Map<string, number>();
  for (const [slot, id] of values) {
    if (inverted.has(id)) throw new Error(`Cube boundary identity '${id}' is duplicated.`);
