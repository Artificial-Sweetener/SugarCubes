...
export const EMPTY_CUBE_INPUT_POSITION: Vec2 = [120, 200];
/** Keep empty-draft boundaries visible and clearly separated in the editor. */
export const EMPTY_CUBE_OUTPUT_POSITION: Vec2 = [780, 200];
/** Frame both transient boundary affordances when an empty Cube editor first opens. */
export const EMPTY_CUBE_BOUNDARY_VIEWPORT_BOUNDS = [80, 120, 860, 260] as const;
...
interface NativeCubeBoundaryPlan {
  schema: 1;
  inputSymbols: string[];
...
export function attachNativeCubeBoundaryLayout(
  subgraph: NativeCubeSubgraph,
  inputNodes: ReadonlySet<NativeGraphNode>,
  outputNodes: ReadonlySet<NativeGraphNode>,
): void {
  const plan: NativeCubeBoundaryPlan = {
    schema: 1,
...
export function applyNativeCubeBoundaryLayout(subgraph: NativeCubeSubgraph): boolean {
  ...
  const inputChanged = writeConnectedBoundaryPosition(
    subgraph.inputNode,
    'input',
    entries,
    resolveConnectedNodes(plan.inputSymbols, nodesBySymbol),
  );
  const outputChanged = writeConnectedBoundaryPosition(
    subgraph.outputNode,
    'output',
    entries,
    resolveConnectedNodes(plan.outputSymbols, nodesBySymbol),
  );
  return inputChanged || outputChanged;
}

/** Place editable draft boundaries on opposite sides of an otherwise empty editor. */
export function applyEmptyCubeBoundaryLayout(subgraph: NativeCubeSubgraph): boolean {
...
export function labelEmptyCubeBoundaryAffordances(subgraph: NativeCubeSubgraph): void {
  if (subgraph.inputNode.emptySlot) subgraph.inputNode.emptySlot.name = 'Add Input';
  if (subgraph.outputNode.emptySlot) subgraph.outputNode.emptySlot.name = 'Add Output';
}

/** Resolve one persisted plan without trusting serialized graph extras. */
function readBoundaryPlan(extra: UnknownRecord | undefined): NativeCubeBoundaryPlan | null {
...
function readNodeSymbols(nodes: ReadonlySet<NativeGraphNode>): string[] {
  return [...nodes].map(readNodeSymbol).filter((symbol): symbol is string => symbol !== null);
}

/** Read one node's stable Cube symbol. */
function readNodeSymbol(node: NativeGraphNode): string | null {
...
function resolveConnectedNodes(
  ...
  return symbols
    ...
    .filter((node): node is NativeGraphNode => node !== undefined);
}

/** Place one boundary outside the graph and align it to its connected nodes. */
function writeBoundaryPosition(boundary: NativeCubeSubgraph['inputNode'], position: Vec2): boolean {
  let changed = false;
...
function readBoundarySlotGeometry(boundary: NativeCubeSubgraph['inputNode']): string {
...
function writeConnectedBoundaryPosition(
  boundary: NativeCubeSubgraph['inputNode'],
  kind: 'input' | 'output',
  nodes: readonly NativeGraphNode[],
  connectedNodes: readonly NativeGraphNode[],
): boolean {
...
function boundaryPosition(
  ...
  const maxX = Math.max(
    ...nodes.map((node) => (Number(node.pos[0]) || 0) + (Number(node.size[0]) || 0)),
  );
  const maxY = Math.max(
    ...nodes.map((node) => (Number(node.pos[1]) || 0) + (Number(node.size[1]) || 0)),
  );
  const boundaryWidth =
    Number(boundary.boundingRect?.[2]) ||
    Number(boundary.bounding?.[2]) ||
    Number(boundary.size?.[0]) ||
    DEFAULT_BOUNDARY_WIDTH;
  const boundaryHeight =
    Number(boundary.boundingRect?.[3]) ||
  ...
  const connectedCenterY =
    alignmentNodes.reduce(
      (total, node) => total + (Number(node.pos[1]) || 0) + (Number(node.size[1]) || 0) / 2,
      0,
    ) / alignmentNodes.length;
...
function writePosition(
  target: ArrayLike<number> & Record<number, number>,
  position: Vec2,
): boolean {
  if (target[0] === position[0] && target[1] === position[1]) return false;
  target[0] = position[0];
  target[1] = position[1];
  return true;
}
