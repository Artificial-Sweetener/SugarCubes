...
export interface CubeModelTitlePresentation {
  accessibleText: string;
  modelText: string;
  nameText: string;
  suffixText: string;
  usesModelPill: boolean;
}

export interface CubeModelTitleInput {
  targetModel: unknown;
  title: unknown;
  suffix?: unknown;
  allowModelPill?: boolean;
}

export interface CubeDefaultInstanceTitleInput {
  targetModel: unknown;
...
export function resolveCubeModelTitle({
  targetModel,
  title,
  suffix = '',
  allowModelPill = true,
}: CubeModelTitleInput): CubeModelTitlePresentation {
...
export function resolveDefaultInstanceModelTitle({
  targetModel,
  instanceTitle,
  defaultAlias,
}: CubeDefaultInstanceTitleInput): CubeModelTitlePresentation {
  ...
  return resolveCubeModelTitle({
    ...
    allowModelPill:
      Boolean(resolvedInstanceTitle) && resolvedInstanceTitle === resolvedDefaultAlias,
  });
}
