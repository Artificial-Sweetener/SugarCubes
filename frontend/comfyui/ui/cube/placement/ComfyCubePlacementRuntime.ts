...
import {
  ComfyCubeHostMutationGuard,
  type GuardedCubePlacementCanvas,
  type GuardedCubePlacementGraph,
} from './ComfyCubeHostMutationGuard.js';

interface PlacementRuntimeFeedback {
  push?(severity: string, summary: string, detail?: string): unknown;
}

export interface ComfyCubePlacementRuntimeOptions {
  rootGraph: UnknownRecord;
  canvas: UnknownRecord;
  logger: Pick<Console, 'warn'>;
  feedback?: PlacementRuntimeFeedback | null;
}

export interface ComfyCubePlacementRuntime {
  graphScope: ComfyCubeGraphScope;
  graphInventory: CubeGraphInventory;
  hostPlacementGuard: ComfyCubeHostMutationGuard;
}

/** Validate dynamic host capabilities and return focused graph-bound owners. */
export function createComfyCubePlacementRuntime(
  options: ComfyCubePlacementRuntimeOptions,
): ComfyCubePlacementRuntime {
...
function requirePlacementGraph(value: UnknownRecord): GuardedCubePlacementGraph {
  if (typeof value.add !== 'function') throw new TypeError('Comfy LGraph.add is unavailable.');
  return value as unknown as GuardedCubePlacementGraph;
}

/** Require the clipboard mutation seam shared by native paste and clone operations. */
function requirePlacementCanvas(value: UnknownRecord): GuardedCubePlacementCanvas {
  ...
  return value as unknown as GuardedCubePlacementCanvas;
}
