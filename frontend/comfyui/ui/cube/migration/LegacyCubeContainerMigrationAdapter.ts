...
interface LegacyContainerSnapshot {
  id: string;
  definitionId: string;
  title: string;
  position: Vec2;
...
interface LegacyNodeEndpoint {
  kind: 'node';
  nodeId: string;
  slot: number;
}

interface LegacyCubeEndpoint {
  kind: 'cube';
  containerId: string;
  slot: number;
}

type LegacyEndpoint = LegacyNodeEndpoint | LegacyCubeEndpoint;

interface LegacyBoundaryLink {
  id: string;
  type: string;
  origin: LegacyEndpoint;
  target: LegacyEndpoint;
}

interface LegacyContainerEnvelope {
  items: LegacyContainerSnapshot[];
  links: LegacyBoundaryLink[];
}

interface NativeLinkNode {
  inputs: unknown[];
  outputs: unknown[];
  connect(outputSlot: number, targetNode: NativeLinkNode, inputSlot: number): unknown;
}

export interface LegacyCubeContainerMigrationGraph {
...
export interface LegacyCubeContainerMigrationAdapterOptions {
  graph: LegacyCubeContainerMigrationGraph;
  factory: Pick<ComfyCubeNodeFactory, 'create'>;
...
export class LegacyCubeContainerMigrationAdapter {
  readonly #graph: LegacyCubeContainerMigrationGraph;
  readonly #factory: Pick<ComfyCubeNodeFactory, 'create'>;
  ...
  readonly #previousSerialize: ((data: UnknownRecord) => void) | null;
  readonly #configureHook: (data: UnknownRecord) => void;
  readonly #serializeHook: (data: UnknownRecord) => void;

  /** Install migration after Comfy and Cube-node lifecycle configuration. */
  constructor(options: LegacyCubeContainerMigrationAdapterOptions) {
  ...
  #restore(data: UnknownRecord): void {
    const extra = isRecord(data.extra) ? data.extra : null;
    if (!extra || extra[STORAGE_KEY] === undefined) return;
    let envelope: LegacyContainerEnvelope;
    try {
    ...
    const migrated = new Map<string, CubeNode>();
    for (const snapshot of envelope.items) {
      ...
      if (!subgraph) {
        this.#logger.warn(
          `SugarCubes skipped legacy Cube '${snapshot.id}' because definition ` +
            `'${snapshot.definitionId}' is missing.`,
        );
        continue;
      }
      try {
        const configuration: CubeNodeConfiguration = {
          subgraph,
    ...
    for (const link of envelope.links) this.#restoreLink(link, migrated);
    Reflect.deleteProperty(extra, STORAGE_KEY);
    if (migrated.size > 0) this.#setDirtyCanvas(true, true);
  }

  /** Restore one retired manual boundary link through native graph slots. */
  #restoreLink(link: LegacyBoundaryLink, cubes: ReadonlyMap<string, CubeNode>): void {
    const origin = this.#resolveEndpoint(link.origin, cubes, 'output');
    const target = this.#resolveEndpoint(link.target, cubes, 'input');
    if (!origin || !target) {
      this.#logger.warn(
        `SugarCubes could not restore legacy Cube link '${link.id}' of type '${link.type}'.`,
      );
      return;
    }
    try {
  ...
  #resolveEndpoint(
    ...
    cubes: ReadonlyMap<string, CubeNode>,
    direction: 'input' | 'output',
  ): { node: NativeLinkNode; slot: number } | null {
    const value =
      endpoint.kind === 'cube'
        ? cubes.get(endpoint.containerId)
        : this.#findRootNode(endpoint.nodeId);
    if (!isNativeLinkNode(value)) return null;
  ...
  #findRootNode(id: string): unknown {
...
function readEnvelope(value: unknown): LegacyContainerEnvelope {
...
function readSnapshot(value: unknown): LegacyContainerSnapshot {
  if (!isRecord(value)) throw new TypeError('Cube container entry must be an object.');
  return {
...
function readBoundaryLink(value: unknown): LegacyBoundaryLink {
  if (!isRecord(value)) throw new TypeError('Cube boundary link must be an object.');
  return {
...
function readEndpoint(value: unknown): LegacyEndpoint {
...
function removeLegacyEnvelope(data: UnknownRecord): void {
  if (isRecord(data.extra)) Reflect.deleteProperty(data.extra, STORAGE_KEY);
}
...
function readPair(value: unknown, label: string): Vec2 {
  if (
    !Array.isArray(value) ||
    value.length < 2 ||
...
function isNativeLinkNode(value: unknown): value is NativeLinkNode {
  return (
    isRecord(value) &&
