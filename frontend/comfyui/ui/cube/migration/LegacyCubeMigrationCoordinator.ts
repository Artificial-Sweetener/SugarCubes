...
import type { CubePlacementService, PlacedCube } from '../CubePlacementService.js';
import type { ComfyInput, ComfyOutput, GraphId } from '../../types/graph.js';
import type { LegacyCubeGraphBuilder } from './LegacyCubeGraphBuilder.js';
import type {
  LegacyConnectionOrigin,
...
export interface LegacyCubeMigrationGraph {
  ...
  getNodeById(id: GraphId): ConnectableNode | null;
}
...
export interface ConnectableNode {
  ...
  inputs: ComfyInput[];
  outputs: ComfyOutput[];
  connect(outputSlot: number, targetNode: ConnectableNode, inputSlot: number): unknown;
}

interface NativeConnectionEndpoint {
  node: ConnectableNode;
  slot: number;
}

export interface LegacyCubeMigrationResult {
  migrated: number;
  connected: number;
  warnings: string[];
}

export interface LegacyCubeMigrationCoordinatorOptions {
  graphBuilder: LegacyCubeGraphBuilder;
  placement: CubePlacementService;
  graph: LegacyCubeMigrationGraph;
...
export class LegacyCubeMigrationCoordinator {
  readonly #graphBuilder: LegacyCubeGraphBuilder;
  readonly #placement: CubePlacementService;
  readonly #graph: LegacyCubeMigrationGraph;
  readonly #logger: Pick<Console, 'error' | 'warn'>;
  /** Bind focused graph construction, placement, and root-link owners. */
  constructor(options: LegacyCubeMigrationCoordinatorOptions) {
  ...
  restore(batch: LegacyCubeMigrationBatch | null): LegacyCubeMigrationResult {
    if (!batch) return { migrated: 0, connected: 0, warnings: [] };

    const warnings = [...batch.warnings];
    const placedByKey = new Map<string, PlacedCube>();
    ...
    for (const connection of batch.connections) {
      if (this.#restoreConnection(connection, placedByKey, warnings)) connected += 1;
    }
    for (const warning of warnings) this.#logger.warn(`SugarCubes migration: ${warning}`);
    if (placedByKey.size > 0) this.#graph.setDirtyCanvas?.(true, true);
  ...
  #restoreConnection(
    connection: LegacyRootConnection,
    placedByKey: ReadonlyMap<string, PlacedCube>,
    warnings: string[],
  ): boolean {
    ...
    try {
      ...
      const message =
        `Failed to restore migrated Cube connection of type '${connection.type}': ` +
  ...
  #resolveOrigin(
    endpoint: LegacyConnectionOrigin,
    placedByKey: ReadonlyMap<string, PlacedCube>,
  ): NativeConnectionEndpoint | null {
