...
export function reconcileLegacySubgraphInstance(
  node: UnknownRecord,
  ...
  if (!originalSubgraphId || !Array.isArray(node.widgets_values)) return;

  if (hasPersistedCubeEdits(cubeMetadata)) {
    throw new Error(
      `Nested Sugar-DSL node '${nodeLabel(node)}' contains edited positional widget values ` +
  ...
  if (!definition) {
    throw new Error(
      `Nested Sugar-DSL node '${nodeLabel(node)}' is missing its persisted definition ` +
        `'${definitionId}'.`,
    );
  }
  ...
  if (declaredOriginalId && declaredOriginalId !== originalSubgraphId) {
    throw new Error(
      `Nested Sugar-DSL node '${nodeLabel(node)}' references a mismatched persisted definition.`,
    );
  }
...
function resolveBoundaryWidgetValues(definition: UnknownRecord): unknown[] {
  ...
  for (const boundary of readRecords(definition.inputs)) {
    ...
    if (!boundaryName || linkIds.length === 0) continue;
...
function resolveBoundaryTargetValue(
  linkId: unknown,
  nodes: ReadonlyMap<string, UnknownRecord>,
  links: ReadonlyMap<string, UnknownRecord>,
): ResolvedValue {
  const link = links.get(idKey(linkId));
  if (!link || idKey(link.origin_id) !== '-10') return { found: false };
  ...
  if (!target || !widget || !inputName) return { found: false };
...
function decodeSavedWidgetValues(node: UnknownRecord): Map<string, unknown> {
  const names = readRecords(node.inputs)
    .filter((input) => isRecord(input.widget))
    .map((input) => readString((input.widget as UnknownRecord).name) || readString(input.name))
    .filter(Boolean);
  ...
  if (candidates.length !== 1) {
    throw new Error(
      `Serialized node '${String(node.id ?? '')}' has widget values without a unique saved identity.`,
    );
  }
...
function decodeCandidates(
  ...
  if (nameIndex === names.length) return valueIndex === values.length ? [new Map(decoded)] : [];
...
function hasPersistedCubeEdits(metadata: UnknownRecord): boolean {
  return (
    metadata.implementation_dirty === true ||
...
function readOriginalDefinitionId(definition: UnknownRecord): string {
...
function indexRecords(value: unknown, label: string): Map<string, UnknownRecord> {
