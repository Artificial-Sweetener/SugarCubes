...
import type { LegacyCubeLink, LegacyCubePlan } from './LegacyCubeWorkflowExtractor.js';
import { reconcileLegacySubgraphInstance } from './LegacySubgraphInstanceReconciler.js';

export interface SerializedCubeDefinition extends UnknownRecord {
  id: string;
  name: string;
  nodes: UnknownRecord[];
...
export class LegacyCubeDefinitionSerializer {
  ...
  serialize(plan: LegacyCubePlan, definitionId: string): SerializedCubeDefinition {
    ...
    const nodes = plan.nodes.map((node) =>
      serializeNode(
        ...
        retainedLinkIds,
        plan.metadata,
        plan.embeddedSubgraphDefinitions,
      ),
    );
    ...
    const inputLinks = plan.inputs.flatMap((input, inputSlot) =>
    ...
    const boundaryHeight = (count: number): number => Math.max(100, 40 + count * 20);
...
function collectRetainedLinkIds(plan: LegacyCubePlan): Set<GraphId> {
  const ids = new Set<GraphId>();
  for (const link of plan.internalLinks) ids.add(link.id);
  for (const input of plan.inputs) {
    for (const target of input.targets) ids.add(target.linkId);
  }
  for (const output of plan.outputs) ids.add(output.source.linkId);
  return ids;
}
...
function serializeNode(
  ...
  origin: Vec2,
  retainedLinkIds: ReadonlySet<GraphId>,
  cubeMetadata: UnknownRecord,
  ...
  if (Array.isArray(node.inputs)) {
    node.inputs = node.inputs.map((value) => {
      ...
      const input = { ...value };
      if (!hasId(retainedLinkIds, input.link)) input.link = null;
      return input;
    });
  }
...
function assertSavedWidgetIdentities(node: UnknownRecord): void {
  const properties = isRecord(node.properties) ? node.properties : {};
  if (readOriginalSubgraphId(properties) || !Array.isArray(node.widgets_values)) return;
  if (node.widgets_values.length === 0) return;
  const names = Array.isArray(node.inputs)
    ? node.inputs.filter(isRecord).filter((input) => isRecord(input.widget))
    : [];
  if (names.length === 0) {
    throw new Error(
      `Node '${String(properties.sugarcubes_symbol ?? node.title ?? node.type ?? node.id)}' ` +
...
function readOriginalSubgraphId(properties: UnknownRecord): string {
...
function serializeGroup(source: UnknownRecord, origin: Vec2): UnknownRecord {
...
function serializeLink(link: LegacyCubeLink): UnknownRecord {
  return {
    id: link.id,
...
function measureNodes(nodes: UnknownRecord[]): {
  minX: number;
  minY: number;
  maxX: number;
} {
  if (nodes.length === 0) return { minX: 150, minY: 100, maxX: 650 };
...
function shiftPair(value: unknown, origin: Vec2): Vec2 {
  const pair = readPair(value);
  return [pair[0] - origin[0], pair[1] - origin[1]];
}

/** Read one finite numeric pair. */
function readPair(value: unknown, fallback: Vec2 = [0, 0]): Vec2 {
...
function maxNumericId(values: Iterable<unknown>): number {
  let maximum = 0;
  for (const value of values) {
    const numeric = Number(value);
    if (Number.isInteger(numeric) && numeric >= 0) maximum = Math.max(maximum, numeric);
  }
  return maximum;
}

/** Compare graph IDs without depending on whether persistence used strings or numbers. */
function hasId(ids: ReadonlySet<GraphId>, value: unknown): boolean {
  for (const id of ids) {
    if (String(id) === String(value)) return true;
  }
  return false;
}
