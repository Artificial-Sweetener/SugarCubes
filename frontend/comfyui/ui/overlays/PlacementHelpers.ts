...
/**
 * Own the SugarCubes overlay rendering layer in `frontend/comfyui/ui/overlays/PlacementHelpers.js`.
 */
...
export interface PreviewLayout extends UnknownRecord {
  title?: string;
  pos?: unknown;
  size?: unknown;
  flags?: unknown;
 ...
 * Read layout flags.
 */
export function readLayoutFlags(layout: PreviewLayout | null | undefined): UnknownRecord | null {
  if (!layout) {
    return null;
  }
 ...
 */
export function resolveCollapsedPreviewSize(
  layout: PreviewLayout | null | undefined,
  size: Vec2,
  ...
  if (ctx && title) {
    ...
    const fontFamily =
      typeof liteGraph?.NODE_FONT === 'string' && liteGraph.NODE_FONT
        ? liteGraph.NODE_FONT
        : 'Arial';
 ...
 * Read layout style.
 */
export function readLayoutStyle(layout: PreviewLayout | null | undefined): UnknownRecord | null {
  if (!layout) {
    return null;
  }
 ...
 */
export function computePayloadBounds(
  entries: readonly PreviewEntry[],
  ctx: CanvasRenderingContext2D | null | undefined = null,
  liteGraph: PreviewGraphMetrics | null | undefined = null,
): PayloadBounds | null {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
 ...
 */
export function getPlacementGroupLabel(
  defaultAlias: unknown,
  group: ComfyGroup | null | undefined,
  getGroupSugarcubes: (group: ComfyGroup | null | undefined) => CubeGroupMetadataRecord | null,
): string | null {
...
function getPreviewLabelFontSize(scale: number): number {
