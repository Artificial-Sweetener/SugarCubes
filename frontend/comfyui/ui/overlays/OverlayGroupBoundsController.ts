...
export interface BoundsCanvas extends ComfyCanvas {
  graph: ComfyGraph;
...
interface BoundsAdapter {
  getApp?(): { canvas?: ComfyCanvas | null } | null;
}
interface BoundsOptions {
  adapter: BoundsAdapter | null;
  containmentService: CubeContainmentService | null;
  collisionService: CubeCollisionService | null;
  requestDirtyRefresh: ((options: { graph: ComfyGraph; reason: string }) => void) | null;
...
function readManagedMetadata(group: ComfyGroup | null | undefined): ManagedMetadata | null {
  ...
  if (!isRecord(sugarcubes) || sugarcubes.managed !== true || !sugarcubes.instance_id) return null;
...
export class OverlayGroupBoundsController {
  ...
  private readonly adapter: BoundsAdapter | null;
  ...
  private readonly requestDirtyRefresh: BoundsOptions['requestDirtyRefresh'];

  constructor(private readonly options: BoundsOptions) {
  ...
  resolveManagedCanvasGroup(canvas: BoundsCanvas | null | undefined): ManagedGroupTarget | null {
  ...
  captureGroupDragState(canvas: BoundsCanvas): void {
  ...
  commitGroupDrag(
    graph: ComfyGraph,
    canvas: BoundsCanvas | null | undefined,
  ...
  commitSelectedGroupBounds(_graph: ComfyGraph, canvas: BoundsCanvas): CommittedGroupEntry | null {
