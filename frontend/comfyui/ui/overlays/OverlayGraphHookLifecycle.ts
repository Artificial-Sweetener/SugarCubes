...
import type { ProximityPointerMoveTracker } from './proximity/ProximityPointerMoveTracker.js';
import type { ProximityGraphMutationTracker } from './proximity/ProximityGraphMutationTracker.js';
import type { CommittedGroupEntry } from './OverlayGroupBoundsController.js';

interface HookAdapter {
  getApp?(): { canvas?: ComfyCanvas | null; clean?: HostCallback } | null;
  getLiteGraph?(): LiteGraphHost | null;
}
interface HookScheduler {
  timeout?(callback: () => void, delayMs: number): number | null;
}
interface HookEvents {
  emit?<T>(name: string, detail: T): void;
}
export interface GraphHookCanvas extends ComfyCanvas {
  graph: ComfyGraph;
  selectedItems?: { values(): Iterable<unknown> };
  onAfterChange?: (this: GraphHookCanvas, ...args: unknown[]) => unknown;
  onNodeMoved?: (this: GraphHookCanvas, node: unknown, ...args: unknown[]) => unknown;
  processMouseDown?: (
    this: GraphHookCanvas,
    event: MouseEvent | PointerEvent,
    ...args: unknown[]
  ) => unknown;
  processMouseUp?: (this: GraphHookCanvas, ...args: unknown[]) => unknown;
}
...
interface GraphHookTarget {
  snapshotManagedGroupBounds(
    graph: ComfyGraph,
  ): Map<string, import('../types/common.js').RectBounds>;
  commitSelectedGroupBounds(graph: ComfyGraph, canvas: GraphHookCanvas): CommittedGroupEntry | null;
  commitManagedGroupBoundsChanges(
    graph: ComfyGraph,
    previous: ReadonlyMap<string, import('../types/common.js').RectBounds> | null,
  ): CommittedGroupEntry[];
  ensureOverlayHook(): void;
  enqueueNodeMove(graph: ComfyGraph, node: unknown, canvas: GraphHookCanvas): void;
  flushNodeMoves(graph: ComfyGraph): void;
  scheduleBoundsReconcile(graph: ComfyGraph): void;
  clearGroupDragState(): void;
  captureGroupDragState(canvas: GraphHookCanvas): void;
  commitGroupDrag(graph: ComfyGraph, canvas: GraphHookCanvas): CommittedGroupEntry | null;
...
interface HookOptions {
  adapter: HookAdapter | null;
  scheduler: HookScheduler | null;
  target: GraphHookTarget;
  proximity: ProximityOverlay;
  placement: PlacementOverlay;
  chrome: CubeChromeOverlay;
  proximityPointerMoves: ProximityPointerMoveTracker;
  proximityGraphMutations: ProximityGraphMutationTracker;
  events: HookEvents | null;
...
export class OverlayGraphHookLifecycle {
  private cleanHooked = false;
  private graphHooksWrapped = false;
  private readonly adapter: HookAdapter | null;
  private readonly scheduler: HookScheduler | null;
  private readonly target: GraphHookTarget;
  private readonly proximity: ProximityOverlay;
  private readonly placement: PlacementOverlay;
  ...
  private readonly events: HookEvents | null;
    ...
    if (!nodeProto.__sugarcubes_collapse_hooked) {
      ...
      nodeProto.collapse = function sugarcubesPatchedCollapse(
        ...
        if (wasCollapsed !== isCollapsed) {
          const collapseManager = nodeProto.__sugarcubes_collapse_manager as
            | GraphHookTarget
            | undefined;
  ...
  ensureGraphHooks(attempt = 0): void {
    const appRef = this.adapter?.getApp?.() || null;
    const canvas = appRef?.canvas as GraphHookCanvas | undefined;
    const MAX_RETRIES = 60;
    const RETRY_DELAY_MS = 250;
    if (!canvas) {
      if (attempt < MAX_RETRIES) {
    ...
    canvas.onAfterChange = function onAfterChange(this: GraphHookCanvas, ...args: unknown[]) {
    ...
    if (typeof originalProcessMouseUp === 'function') {
      canvas.processMouseUp = function processMouseUp(this: GraphHookCanvas, ...args: unknown[]) {
        const committed =
