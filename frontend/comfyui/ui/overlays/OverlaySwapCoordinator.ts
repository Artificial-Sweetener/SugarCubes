...
export class OverlaySwapCoordinator {
  constructor(
    private readonly adapter: SwapAdapter | null,
  ...
  swapLayout(metadata: ChromeMetadata, direction: -1 | 1): void {
    if (!this.layoutService || !metadata?.instance_id) return;
  ...
  canSwapDirection(metadata: ChromeMetadata, direction: 'left' | 'right' | -1 | 1): boolean {
    if (!metadata?.instance_id) return false;
    const graph = this.adapter?.getApp?.()?.graph || null;
  ...
  resolveProximityMatchesForSwap(graph: ComfyGraph): ProximityMatch[] {
    if (!this.proximity.settings?.enabled) return [];
    try {
  ...
  resolveSwapPlan(graph: ComfyGraph, instanceId: string, direction: -1 | 1): SwapPlan | null {
    if (!this.layoutService) return null;
    ...
    if (currentIndex < 0 || !this.canSwapEntry(current)) return null;
    let neighbor: SwapEntry | null = null;
    for (
      let nextIndex = currentIndex + direction;
      nextIndex >= 0 && nextIndex < order.length;
      nextIndex += direction
    ) {
    ...
    if (!current?.instanceId || !neighbor?.instanceId) return null;
