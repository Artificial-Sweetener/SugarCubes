...
import type { ProximityInputEndpoint, ProximityOutputEndpoint } from './ProximityModel.js';

interface IndexedInput {
  x: number;
  endpoint: ProximityInputEndpoint;
}

/** Own deterministic horizontal range lookup for nearby input candidates. */
export class ProximityInputIndex {
  readonly #entries: readonly IndexedInput[];

  /** Sort an immutable view of discovered inputs once per proximity pass. */
  constructor(inputs: readonly ProximityInputEndpoint[]) {
    this.#entries = inputs
      .map((endpoint) => ({ endpoint, x: finite(endpoint.slotPos[0]) }))
      .sort(
        (left, right) => left.x - right.x || left.endpoint.key.localeCompare(right.endpoint.key),
      );
  }

  /** Return only inputs inside the maximum possible acquire/release distance. */
  query(output: ProximityOutputEndpoint, maximumDistance: number): ProximityInputEndpoint[] {
    ...
    const result: ProximityInputEndpoint[] = [];
    for (let index = start; index < this.#entries.length; index += 1) {
      const entry = this.#entries[index];
      if (!entry || entry.x > outputX + radius) break;
...
function lowerBound(entries: readonly IndexedInput[], minimumX: number): number {
  let low = 0;
