...
export interface WritableCubePack {
  owner: string;
  repo: string;
  repoRef: string;
}

interface CubePackCatalog {
  packs: WritableCubePack[];
  identityPolicy: UnknownRecord;
}

type CubePackApi = Pick<
  CubeLibraryApi,
  'listCubePacks' | 'updateIdentityPolicy' | 'createAuthoringCubePack'
>;
type CubePackDialogs = Pick<ModalService, 'openForm' | 'selectItem'>;
type CubePackToast = Pick<ToastService, 'push'>;

interface CubePackServiceOptions {
  api?: CubePackApi | null;
  dialogs?: CubePackDialogs | null;
  toast?: CubePackToast | null;
}
...
export class CubePackService {
  private readonly api: CubePackApi;
  private readonly dialogs: CubePackDialogs | null;
  private readonly toast: CubePackToast | null;

  constructor({ api, dialogs, toast }: CubePackServiceOptions = {}) {
    if (!api) {
  ...
  async listAuthoringPacks(): Promise<WritableCubePack[]> {
    const catalog = await this.loadCatalog();
    const owner = String(catalog.identityPolicy.claimed_github_owner || '').trim();
    if (!owner) return [];
    return catalog.packs.filter(
      (pack) => pack.owner.localeCompare(owner, undefined, { sensitivity: 'accent' }) === 0,
    );
  }

  /** Create one writable pack under the claimed author identity. */
  async createAuthoringPackForClaimedOwner(): Promise<WritableCubePack | null> {
    const catalog = await this.loadCatalog();
  ...
  async chooseWritablePack(): Promise<WritableCubePack | null> {
    ...
    const matchingPacks = catalog.packs.filter(
      (pack) => pack.owner.toLowerCase() === owner.toLowerCase(),
    );
  ...
  async loadCatalog(): Promise<CubePackCatalog> {
  ...
  async ensureClaimedOwner(identityPolicy: UnknownRecord = {}): Promise<string> {
    ...
    const { response, data } = await this.api.updateIdentityPolicy(
      JSON.stringify({ claimed_github_owner: values.owner }),
  ...
  async createAuthoringPack(owner: string): Promise<WritableCubePack | null> {
    const values = await this.dialogs?.openForm?.({
    ...
    const { response, data } = await this.api.createAuthoringCubePack(
      JSON.stringify({ owner, repo: values.repo, enabled: true }),
      { headers: { 'Content-Type': 'application/json' } },
    );
    if (!response.ok || data.error) {
