...
export interface LayoutMeasurement<T> {
  identity: string;
  item: T;
  authored: LayoutRect;
...
export interface SolvedLayoutItem<T> extends LayoutMeasurement<T> {
  solved: LayoutRect;
}
...
export function solveAuthoredLayout<T>(
  measurements: readonly LayoutMeasurement<T>[],
): SolvedLayoutItem<T>[] {
...
export function unionLayoutRects(rects: readonly LayoutRect[]): LayoutRect | null {
...
function solveAxis<T>(items: SolvedLayoutItem<T>[], axis: 'x' | 'y'): void {
  ...
  for (let left = 0; left < items.length; left += 1) {
    for (let right = left + 1; right < items.length; right += 1) {
      ...
      if (
        !intervalsOverlap(
          first.authored[perpendicular],
          first.authored[perpendicularExtent],
          second.authored[perpendicular],
          second.authored[perpendicularExtent],
        )
      ) {
        continue;
      }
...
function intervalsOverlap(
  ...
  return (
    firstStart < secondStart + secondExtent - RELATION_EPSILON &&
