...
export interface LiveGeometryInstance {
...
export function collectLiveGeometryInstances(graph: ComfyGraph): LiveGeometryInstance[] {
  ...
  const groups = Array.isArray(graph._groups)
    ...
    : Array.isArray(graph.groups)
      ? graph.groups
      : [];
  const result: LiveGeometryInstance[] = [];
  for (const group of groups) {
    const metadata =
      isRecord(group.properties) && isRecord(group.properties.sugarcubes)
        ? group.properties.sugarcubes
        : null;
    const baseline = readAuthoredLayoutBaseline(metadata?.[AUTHORED_LAYOUT_KEY]);
    if (!metadata || !baseline) continue;
    const memberIds = readMemberIds(metadata);
    const nodes = new Map<string, ComfyNode>();
    for (const node of graphNodes) {
      if (memberIds.size && !memberIds.has(String(node.id))) continue;
      const identity =
        isRecord(node.properties) && typeof node.properties.sugarcubes_symbol === 'string'
          ? node.properties.sugarcubes_symbol.trim()
          : '';
      if (identity && baseline.entries[identity]) nodes.set(identity, node);
    }
    if (nodes.size) result.push({ baseline, group, metadata, nodes });
  }
  return result;
}

function readMemberIds(metadata: Record<string, unknown>): Set<string> {
  const ids = new Set<string>();
  if (Array.isArray(metadata.nodes)) {
    for (const value of metadata.nodes) ids.add(String(value));
  }
  if (isRecord(metadata.markers)) {
    for (const values of [metadata.markers.inputs, metadata.markers.outputs]) {
      if (Array.isArray(values)) for (const value of values) ids.add(String(value));
    }
  }
  return ids;
}
