...
import {
  ...
  asFlavorMetadata,
  cloneValue,
  readNodePropertyValue,
  readWidgetValue,
  type FlavorMetadata,
  type FlavorServiceAdapter,
...
export class FlavorGraphProjection {
  constructor(private readonly adapter: FlavorServiceAdapter | null) {}

  getGraph(): ComfyGraph | null {
    return this.adapter?.getApp?.()?.graph || null;
  }

  findGroup(graph: ComfyGraph | null, metadata: FlavorMetadata): ComfyGroup | null {
    const instanceId = metadata.instance_id?.trim() || '';
    if (!graph || !instanceId) return null;
    return (
      getGraphGroups(graph).find(
        (group) => getGroupSugarcubes(group)?.instance_id === instanceId,
      ) || null
    );
  }
  ...
  collectValues(graph: ComfyGraph, metadata: FlavorMetadata): UnknownRecord {
    ...
    for (const control of trackedSurfaceControls(metadata.surface)) {
      ...
      if (!controlId || !symbol || !inputName) continue;
      ...
      values[controlId] = cloneValue(
        widgetValue !== '' || propertyValue == null ? widgetValue : propertyValue,
      );
    }
    return values;
  }

  applyValues(graph: ComfyGraph, metadata: FlavorMetadata, flavor: FlavorOption): void {
    const nodes = this.buildNodesBySymbol(graph, metadata);
    ...
    for (const control of trackedSurfaceControls(metadata.surface)) {
      ...
      if (
        ...
        continue;
      const node = nodes.get(symbol);
      if (node) applyNodeValue(node, inputName, cloneValue(trackedValues[controlId]));
    }
  ...
  selectionNeedsApplication(metadata: FlavorMetadata, flavor: FlavorOption | null): boolean {
    ...
    if (metadata.flavor !== flavor.id || metadata.flavor_scope !== flavor.scope) return true;
    try {
      return (
        JSON.stringify(
          filterTrackedSurfaceValues(metadata.surface, metadata.active_flavor_values),
        ) !== JSON.stringify(flavor.values || {})
      );
    } catch (_error) {
      return true;
    }
