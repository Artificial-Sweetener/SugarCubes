...
import type { CubeBrowserState, CubeLibraryEntry } from './CubeBrowserStore.js';
import type { BrowserElements } from './CubeBrowserView.js';
import type { CubeBrowserMetadataView } from './CubeBrowserMetadataView.js';
...
interface DetailViewOptions {
  ...
  metadata: CubeBrowserMetadataView;
...
export class CubeBrowserDetailView {
  ...
  render(state: CubeBrowserState): void {
    ...
    if (state.editing) this.options.metadata.renderEditor(selected, state);
    else this.options.metadata.renderReadOnly(selected);
  ...
  private renderEmpty(): void {
    ...
    if (!elements.detailTitle || !elements.detailMeta || !elements.detailDescription) return;
  ...
  private updateFavorite(state: CubeBrowserState, selected: CubeLibraryEntry): void {
  ...
  private updateEditingActions(state: CubeBrowserState, selected: CubeLibraryEntry): void {
    ...
    if (elements.promoteButton) {
      ...
      elements.promoteButton.classList.toggle(
        'sugarcubes-browser__action-hidden',
    ...
    if (elements.editButton) {
      ...
      elements.editButton.classList.toggle(
        'sugarcubes-browser__action-hidden',
        state.editing || !writable,
      );
    }
    ...
    if (elements.editCancelButton) {
      ...
      elements.editCancelButton.classList.toggle(
        'sugarcubes-browser__action-hidden',
        !state.editing,
      );
    }
    if (elements.placeButton) {
      ...
      elements.placeButton.classList.toggle(
  ...
  private updateActionState(state: CubeBrowserState, selected: CubeLibraryEntry | null): void {
    const elements = this.options.elements;
    const writable = Boolean(selected?.is_writable);
    const disabled = state.busy || !state.selected;
    if (elements.placeButton) elements.placeButton.disabled = disabled;
    if (elements.deleteButton) elements.deleteButton.disabled = disabled || !writable;
    if (elements.promoteButton)
      elements.promoteButton.disabled = disabled || !writable || !isPersonalCubeEntry(selected);
    if (elements.editButton) elements.editButton.disabled = disabled || !writable;
    if (elements.editSaveButton) elements.editSaveButton.disabled = disabled || !writable;
    if (elements.editCancelButton) elements.editCancelButton.disabled = disabled || !writable;
  }

  private updateIcon(selected: CubeLibraryEntry): void {
...
function getSelectedCube(state: CubeBrowserState): CubeLibraryEntry | null {
