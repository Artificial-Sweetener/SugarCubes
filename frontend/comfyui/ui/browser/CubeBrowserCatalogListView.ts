...
export class CubeBrowserCatalogListView {
  ...
  render(state: CubeBrowserState): void {
    ...
    if (!listBody || !documentRef) return;
    listBody.replaceChildren();
    if (!state.filtered.length) {
      if (elements.emptyState) elements.emptyState.style.display = state.loading ? 'none' : 'block';
    ...
    for (const group of state.grouped) {
      ...
      if (!openGroups.has(group.key)) list.classList.add('is-collapsed');
      for (const cube of group.cubes) list.appendChild(this.buildRow(cube, state));
  ...
  private buildRow(cube: CubeLibraryEntry, state: CubeBrowserState): HTMLDivElement {
  ...
  private renderSelection(state: CubeBrowserState): void {
    this.options
      .getElements()
      .listBody?.querySelectorAll<HTMLElement>('[data-cube]')
      .forEach((row) => {
        row.classList.toggle('is-selected', row.dataset.cube === state.selected);
        if (row.dataset.cube === state.selected) row.scrollIntoView({ block: 'nearest' });
      });
  }

  private renderEmptyState(state: CubeBrowserState): void {
