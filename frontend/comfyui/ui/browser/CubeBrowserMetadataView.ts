...
import type { CubeBrowserState, CubeEditDraft, CubeLibraryEntry } from './CubeBrowserStore.js';
import type { BrowserEditInputs, BrowserElements, BrowserViewHandlers } from './CubeBrowserView.js';

interface MetadataViewOptions {
  documentRef: Document | null;
  getElements(): Partial<BrowserElements>;
  getHandlers(): BrowserViewHandlers;
}

/** Render metadata and own all stateful editor controls. */
export class CubeBrowserMetadataView {
  private editInputs: BrowserEditInputs | null = null;
  private identityInvalid = false;

  constructor(private readonly options: MetadataViewOptions) {}
  ...
  renderReadOnly(selected: CubeLibraryEntry): void {
    ...
    if (defaultAlias) rows.push(`Default Alias: ${defaultAlias}`);
    if (isLocalCubeEntry(selected)) rows.push('local');
    else if (selected.author) rows.push(`Author: ${selected.author}`);
    const versionUpdated: string[] = [];
    if (selected.version) versionUpdated.push(`Version: ${selected.version}`);
    if (selected.mtime)
      versionUpdated.push(`Updated: ${new Date(selected.mtime).toLocaleString()}`);
    if (versionUpdated.length) rows.push(versionUpdated.join(' | '));
    if (selected.target_model) rows.push(`Target Model: ${selected.target_model}`);
    if (selected.cube_id) rows.push(`ID: ${selected.cube_id}`);
    if (selected.lineage && typeof selected.lineage === 'object') {
      const lineage = selected.lineage;
      const parts: unknown[] = [];
      if (lineage.name) parts.push(lineage.name);
      if (lineage.version) parts.push(`v${lineage.version}`);
      if (lineage.id) parts.push(`(${lineage.id})`);
      if (lineage.author) parts.push(`by ${lineage.author}`);
      if (lineage.forked_at) parts.push(`forked ${lineage.forked_at}`);
      rows.push(`Forked from: ${parts.join(' ')}`.trim());
    }
    if (selected.supported_models?.length)
      rows.push(`Models: ${selected.supported_models.join(', ')}`);
    if (selected.author_url) rows.push(`Website: ${selected.author_url}`);
    if (selected.tags?.length) rows.push(`Tags: ${selected.tags.join(', ')}`);
  ...
  renderEditor(selected: CubeLibraryEntry, state: CubeBrowserState): void {
    ...
    for (const field of [
      ...
      detailMeta.appendChild(
        $el('div.sugarcubes-browser__edit-field', [$el('label', field.label), input]),
      );
    ...
    for (const [key, label] of [
      ...
      detailMeta.appendChild(
        $el('div.sugarcubes-browser__edit-field', [$el('label', label), input]),
      );
      inputs[key] = input;
    }
    ...
    const descriptionNodes: Node[] = [description];
  ...
  private buildTargetModelEditor(targetModel: unknown): {
    ...
    const options =
      normalized && !TARGET_MODEL_OPTIONS.includes(normalized)
  ...
  private buildModelsEditor(
    models: unknown,
    state: CubeBrowserState,
  ): { container: HTMLElement; control: ModelAutocompleteControl } {
...
function isLocalCubeEntry(cube: CubeLibraryEntry): boolean {
  const sourceType =
...
function draftName(state: CubeBrowserState, selected: CubeLibraryEntry): string {
  return (
    state.editDraft?.name?.trim() ||
...
function createFallbackDraft(state: CubeBrowserState, selected: CubeLibraryEntry): CubeEditDraft {
