...
export interface PreviewHelpers {
  ...
  readVector2?(value: unknown, fallbackX: number, fallbackY: number): Vec2;
  drawGhostRect?(
    ctx: CanvasRenderingContext2D,
    rect: PreviewRect,
    style: GhostStyle,
  ...
  getPlacementGroupLabel?(defaultAlias: string | null, group: PreviewEntry): string | null;
  coerceVec2?(value: unknown): Vec2 | null;
  resolvePreviewRect?(
    entry: PreviewEntry,
    pos: Vec2,
    size: Vec2,
...
interface PreviewPlacementState extends UnknownRecord {
  active?: boolean;
  cubeId?: string | null;
  commitInProgress?: boolean;
}
export interface PreviewPlacementActions {
  computeOriginFromEvent?(event: PointerEvent): Vec2 | null;
  getState?(): PreviewPlacementState | null;
  setOrigin?(origin: Vec2): void;
  setDirty?(): void;
  start?(
    cubeId: string,
    options: { closeBrowser: boolean; defaultAlias: string | null },
  ): Promise<unknown> | unknown;
  stop?(message: string): void;
  isPointerOverCanvas?(event: PointerEvent): boolean;
  setCommitInProgress?(value: boolean): void;
  commit?(): Promise<unknown> | unknown;
}
export interface PreviewAdapter {
  getWindow?(): { devicePixelRatio?: number } | null;
}
...
interface PreviewRendererOptions {
  adapter?: PreviewAdapter | null;
  helpers?: PreviewHelpers;
  placement?: PreviewPlacementActions;
}
...
export class CubePreviewRenderer {
  private readonly adapter: PreviewAdapter | null;
  private helpers: PreviewHelpers;
  private placement: PreviewPlacementActions;
  private canvas: HTMLCanvasElement | null;
  private container: HTMLElement | null;
  private status: HTMLElement | null;
  private payload: PreviewPayload | null;
  ...
  private dragPointerId: number | null;
  private dragOrigin: Vec2 | null;
  private dragRequestId: number;
  private context: PreviewContext;
  private pointerHandlersBound: boolean;
  private pointerHandlers: PointerHandlers | null;

  constructor({ adapter = null, helpers = {}, placement = {} }: PreviewRendererOptions = {}) {
  ...
  setHelpers(helpers: PreviewHelpers = {}): void {
    this.helpers = helpers;
  }

  setPlacementActions(actions: PreviewPlacementActions = {}): void {
  ...
  getRequestState(requestKey: string): 'loading' | 'ready' | 'missing' {
  ...
  computePreviewBounds(
    ...
    const updateBounds = (rect: PreviewRect | null) => {
      if (!rect) {
        return;
      }
      const next = {
  ...
  drawCubePreview(
    ...
    const { drawGhostRect, getPlacementGroupLabel, readVector2, coerceVec2, resolvePreviewRect } =
      this.helpers;
    ...
    if (groups.length) {
      for (const group of groups) {
        ...
        drawGhostRect(
          ...
          { fill: 'rgba(70, 120, 150, 0.12)', stroke: 'rgba(70, 120, 150, 0.55)', alpha: 0.7 },
          scale,
          getPlacementGroupLabel ? getPlacementGroupLabel(defaultAlias, group) : null,
        );
      }
    } else {
      drawGhostRect(
        ...
        { fill: 'rgba(70, 120, 150, 0.12)', stroke: 'rgba(70, 120, 150, 0.55)', alpha: 0.5 },
        scale,
        defaultAlias || null,
      );
    }
    ...
    for (const entry of nodes) {
      ...
      drawGhostRect(
        ...
        { fill: 'rgba(120, 180, 210, 0.16)', stroke: 'rgba(120, 180, 210, 0.6)', alpha: 0.9 },
        scale,
      );
    }
  ...
  bindPointerHandlers(): void {
    if (!this.canvas || this.pointerHandlersBound) {
      return;
    }
    this.pointerHandlersBound = true;
    this.pointerHandlers = {
      down: (event: PointerEvent) => this.handlePointerDown(event),
      move: (event: PointerEvent) => this.handlePointerMove(event),
      up: (event: PointerEvent) => this.handlePointerUp(event),
      cancel: (event: PointerEvent) => this.handlePointerCancel(event),
    };
  ...
  updateDragPlacementOrigin(event: PointerEvent): void {
  ...
  handlePointerDown(event: PointerEvent): void {
    ...
    void Promise.resolve(
      this.placement?.start?.(cubeId, { closeBrowser: false, defaultAlias }),
    ).then(() => {
  ...
  endPreviewDrag(event: PointerEvent, placeOnRelease: boolean): void {
    if (!this.dragging) {
      return;
    }
