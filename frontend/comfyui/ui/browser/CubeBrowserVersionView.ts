...
import type { BrowserViewHandlers } from './CubeBrowserView.js';

interface VersionElements {
...
interface VersionViewOptions {
  ...
  getState(): CubeBrowserState | null;
  getSelected(): CubeLibraryEntry | null;
...
export class CubeBrowserVersionView {
  private elements: VersionElements | null = null;
  private open = false;
  private highlightedIndex = -1;
  private inputDirty = false;
  private renderSignature = '';

  constructor(private readonly options: VersionViewOptions) {}

  /** Build and bind the complete version-control subtree. */
  build(): VersionElements {
    ...
    toggle.addEventListener('click', () => {
      ...
      input.focus();
      if (wasOpen) this.closeCombobox();
      else this.openCombobox();
    });
    this.options.documentRef?.addEventListener?.('click', (event) => {
      const target = event.target instanceof Node ? event.target : null;
      if (!control.contains(target)) this.commit({ close: true });
    });
    return this.elements;
  }

  /** Render current revision state into the combobox. */
  render(state: CubeBrowserState | null, selected: CubeLibraryEntry | null): void {
    ...
    if (!this.inputDirty) input.value = this.selectedValue();
    if (disabled || !options.length) this.open = false;
    if (this.open && this.highlightedIndex < 0) this.updateHighlight(input.value);
  ...
  private handleKeydown(event: KeyboardEvent): void {
    if (!this.options.getState()?.versionOptions.length) return;
  ...
  private updateHighlight(value: string): void {
    const state = this.options.getState();
    const options = state?.versionOptions || [];
    const closest = this.options.getHandlers().onVersionClosest?.(value);
    const closestIndex = closest
      ? options.findIndex(
          (item) => item.value === closest.value && item.revisionRef === closest.revisionRef,
        )
      : -1;
  ...
  private commit({ close = false }: { close?: boolean } = {}): void {
    const state = this.options.getState();
    const input = this.elements?.input;
    const options = state?.versionOptions || [];
    if (!input || input.disabled || !options.length) {
      if (close) this.closeCombobox();
      return;
    }
    ...
    this.updateHighlight(input.value);
    if (close) this.closeCombobox();
    else this.render(state, this.options.getSelected());
  }

  private selectedValue(): string {
    const state = this.options.getState();
    return (
      state?.versionOptions.find((option) => option.value === state.selectedVersion)?.value ||
      state?.versionOptions[0]?.value ||
      ''
    );
  }

  private buildOption(option: CubeVersionOption, index: number): HTMLLIElement {
