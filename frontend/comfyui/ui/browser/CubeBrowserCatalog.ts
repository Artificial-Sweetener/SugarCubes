...
import type { CubeAuthorGroup, CubeBrowserStore, CubeLibraryEntry } from './CubeBrowserStore.js';
import type { CubeBrowserPreferences } from './CubeBrowserPreferences.js';
...
interface CatalogOptions {
  ...
  preferences: CubeBrowserPreferences;
  toast: BrowserToast | null;
  getActions(): BrowserActions;
  render(): void;
  selectFirst(cubeKey: string): void;
}

/** Coordinate the remote catalog and its local browser projection. */
export class CubeBrowserCatalog {
  private readonly cubeIdIndex = new Map<string, CubeLibraryEntry>();
  private readonly authorIndex = new Map<string, string>();
  private fetchController: AbortController | null = null;
  private refreshPromise: Promise<CubeLibraryEntry[]> | null = null;
  ...
  getCubeBySelectionKey(
    cubeKey: unknown,
    entries: readonly CubeLibraryEntry[] = this.options.store.state.filtered,
  ...
  async refresh(force: boolean): Promise<CubeLibraryEntry[]> {
  ...
  private async performRefresh(force: boolean): Promise<CubeLibraryEntry[]> {
    ...
    try {
      ...
      if (!response.ok) {
        this.reportRefreshError(
          readApiErrorMessage(data, response.statusText || 'Failed to load cubes'),
        );
        return store.state.cubes;
      }
  ...
  sortFavoritesFirst(list: readonly CubeLibraryEntry[] | null | undefined): CubeLibraryEntry[] {
    ...
    const compare = (left: CubeLibraryEntry, right: CubeLibraryEntry): number =>
      (['target_model', 'default_alias', 'display_name', 'name', 'cube_id'] as const).reduce(
        (result, key) =>
          result ||
  ...
  getAuthorGroupKey(cubeKey: string | null): string | null {
  ...
  private replaceCubes(cubes: CubeLibraryEntry[]): void {
  ...
  private buildAuthorGroups(list: readonly CubeLibraryEntry[]): CubeAuthorGroup[] {
  ...
  private abortPreviousRefresh(): void {
    try {
  ...
  private reportRefreshError(message: string): void {
