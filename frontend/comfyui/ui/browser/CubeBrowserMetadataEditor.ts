...
import type { CubeBrowserStore, CubeEditDraft, CubeLibraryEntry } from './CubeBrowserStore.js';
...
export class CubeBrowserMetadataEditor {
  ...
  toggle(): void {
    ...
    if (!selected.is_writable) {
      this.options.toast?.push(
        'warn',
        'Read-only cube',
        selected.write_block_reason?.trim() || 'This cube is read-only.',
      );
      return;
    }
  ...
  begin(selected: CubeLibraryEntry): void {
  ...
  async save(): Promise<void> {
    ...
    const originalName = normalizeDefaultAliasTitle(
      (store.state.editDraft?.original_name || this.resolveTitle(selected)).split('/').pop(),
    );
    const targetModel =
      normalizeTargetModel(inputs.target_model?.value) || this.deriveTargetModel(selected.cube_id);
    ...
    const metadata: UnknownRecord = {
      ...
      supported_models: normalizeSupportedModels(
        this.parseList(inputs.supported_models?.input?.value ?? ''),
        { targetModel },
      ),
    };
    if (targetModel) metadata.target_model = targetModel;
  ...
  buildDraft(selected: CubeLibraryEntry): CubeEditDraft {
    const editableName = this.resolveTitle(selected);
    const targetModel =
      normalizeTargetModel(selected.target_model) || this.deriveTargetModel(selected.cube_id);
  ...
  private async rename(
    selected: CubeLibraryEntry,
    defaultAlias: string,
    targetCubeId: string,
  ): Promise<string> {
    const { response, data } = await this.options.api.rename(
      JSON.stringify({
        cube_id: selected.cube_id || '',
    ...
    if (!response.ok || hasApiError(data)) {
      this.options.toast?.push(
        'error',
        'Rename failed',
        readApiErrorMessage(data, response.statusText || 'Rename failed'),
      );
      return '';
    }
  ...
  private resolveTitle(selected: CubeLibraryEntry): string {
    if (selected.default_alias?.trim()) return selected.default_alias.trim();
    if (
  ...
  private deriveTargetModel(cubeId: unknown): string {
    try {
