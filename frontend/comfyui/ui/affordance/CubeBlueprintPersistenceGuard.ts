...
interface UserDataApi {
  storeUserData: (...args: unknown[]) => unknown;
}
...
export class CubeBlueprintPersistenceGuard {
  readonly #api: UserDataApi;
  readonly #nativeStoreUserData: (...args: unknown[]) => unknown;
  #installed = false;
  ...
  constructor(api: unknown) {
    ...
    this.#api = api as unknown as UserDataApi;
  ...
  install(): void {
    ...
    this.#api.storeUserData = (...args: unknown[]) => {
      if (isSubgraphBlueprintPath(args[0]) && containsSugarMarkers(args[1])) {
        throw new Error(
          'SugarCubes are saved as versioned .cube documents and cannot be published as Subgraph Blueprints.',
        );
      }
...
function isSubgraphBlueprintPath(value: unknown): boolean {
...
function containsSugarMarkers(value: unknown): boolean {
