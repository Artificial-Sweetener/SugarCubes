...
/** Preserve the exact host presentation that must be restored after Cube selection. */
export interface PrimeVueTooltipPresentationState {
  ariaLabel: string | null;
  title: string | null;
  hadTooltipValue: boolean;
...
export class PrimeVueTooltipPresentationAdapter {
  ...
  readonly #warnedTargets = new WeakSet<HTMLElement>();

  /** Bind browser and observability collaborators at the host adapter boundary. */
  constructor(options: { document: Document; logger: Pick<Console, 'warn'> }) {
    this.#document = options.document;
    this.#logger = options.logger;
  }

  /** Capture the host-owned values before SugarCubes presents a Cube action. */
  capture(target: HTMLElement): PrimeVueTooltipPresentationState {
    return {
  ...
  present(target: HTMLElement, label: string): void {
    ...
    if (Reflect.has(target, TOOLTIP_VALUE_PROPERTY)) {
      ...
      this.#logger.warn(
        'SugarCubes: Comfy PrimeVue tooltip compatibility contract is unavailable.',
      );
    }
  ...
  restore(target: HTMLElement, state: PrimeVueTooltipPresentationState): void {
    restoreAttribute(target, 'aria-label', state.ariaLabel);
    restoreAttribute(target, 'title', state.title);
    if (state.hadTooltipValue) Reflect.set(target, TOOLTIP_VALUE_PROPERTY, state.tooltipValue);
    else Reflect.deleteProperty(target, TOOLTIP_VALUE_PROPERTY);
  ...
  #replaceVisibleTooltipText(target: HTMLElement, label: string): void {
    const tooltipId = Reflect.get(target, TOOLTIP_ID_PROPERTY);
    if (typeof tooltipId !== 'string' || tooltipId.length === 0) return;
    const tooltip = this.#document.getElementById(tooltipId);
    const text = tooltip?.querySelector<HTMLElement>('[data-pc-section="text"], .p-tooltip-text');
