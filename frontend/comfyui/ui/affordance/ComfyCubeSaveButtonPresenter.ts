...
import type {
  PrimeVueTooltipPresentationAdapter,
  PrimeVueTooltipPresentationState,
} from './PrimeVueTooltipPresentationAdapter.js';
...
/** Preserve every host-owned value changed while a Cube save action is presented. */
export interface ComfyCubeSaveButtonPresentationState {
  tooltip: PrimeVueTooltipPresentationState;
  sourceIcon: HTMLElement;
  sourceIconHidden: boolean;
  sourceIconDisplay: string;
  sourceIconDisplayPriority: string;
  compositeIcon: HTMLElement;
}
...
export class ComfyCubeSaveButtonPresenter {
  ...
  readonly #tooltips: PrimeVueTooltipPresentationAdapter;

  /** Bind DOM construction and PrimeVue presentation collaborators. */
  constructor(options: { document: Document; tooltips: PrimeVueTooltipPresentationAdapter }) {
  ...
  present(
    ...
    sourceIcon: HTMLElement,
    label: string,
    state?: ComfyCubeSaveButtonPresentationState,
  ...
  restore(button: HTMLButtonElement, state: ComfyCubeSaveButtonPresentationState): void {
    this.#tooltips.restore(button, state.tooltip);
    state.sourceIcon.hidden = state.sourceIconHidden;
    if (state.sourceIconDisplay) {
      state.sourceIcon.style.setProperty(
        'display',
        state.sourceIconDisplay,
        state.sourceIconDisplayPriority,
      );
    } else {
  ...
  #createCompositeIcon(): HTMLElement {
