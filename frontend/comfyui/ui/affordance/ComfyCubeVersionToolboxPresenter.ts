...
export interface CubeVersionToolboxModel {
...
export class ComfyCubeVersionToolboxPresenter {
  ...
  #model: CubeVersionToolboxModel | null = null;
  ...
  present(anchor: HTMLButtonElement, model: CubeVersionToolboxModel): HTMLElement {
    ...
    if (!button) throw new Error('Cube version button is unavailable.');
    ...
    if (control.previousElementSibling !== anchor) anchor.after(control);
    if (this.#menu.isOpen()) this.#menu.refresh(this.#menuModel(model));
    return control;
  }
  ...
  #createControl(): HTMLDivElement {
    ...
    const hostChevron = this.#document.querySelector<HTMLElement>(
      '[data-testid="color-picker-button"] > div > i:last-child',
    );
    chevron.className = hostChevron?.className || 'icon-[lucide--chevron-down]';
    if (hostChevron?.style.cssText) chevron.style.cssText = hostChevron.style.cssText;
  ...
  readonly #toggleMenu = (event: MouseEvent): void => {
    ...
    if (!model || model.loading || model.busy || model.error || model.options.length === 0) return;
    if (!this.#button) return;
    this.#menu.toggle(this.#button, this.#menuModel(model));
  };

  /** Project exact versions into an exclusive checked-row model. */
  #menuModel(model: CubeVersionToolboxModel): ToolboxCheckMenuModel {
    return {
      ...
      select: (version) => {
        if (version !== model.currentVersion) model.select(version);
      },
    };
  }
}
