...
export class ComfyToolboxTooltipPresenter {
  ...
  bind(target: HTMLElement, label: string): void {
    if (this.#target === target && this.#label === label) return;
    this.clear();
    this.#target = target;
    this.#label = label;
  ...
  #position(): void {
    ...
    const left = Math.max(
      4,
      Math.min(anchor.left + anchor.width / 2 - tooltipWidth / 2, viewportWidth - tooltipWidth - 4),
    );
    ...
    if (arrow) arrow.style.left = `${anchor.left + anchor.width / 2 - left}px`;
  }
