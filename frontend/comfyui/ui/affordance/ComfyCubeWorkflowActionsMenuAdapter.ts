...
export class ComfyCubeWorkflowActionsMenuAdapter {
  ...
  readonly #canvas: WorkflowMenuCanvas;
  ...
  refresh(): void {
    ...
    if (primarySave) this.#setItemText(primarySave, this.#label('save-workflow'));
    if (clear) this.#setItemText(clear, this.#label('clear-container'));
  }
  ...
  #activeWorkflowActionsMenu(): HTMLElement | null {
    const menus = this.#document.querySelectorAll<HTMLElement>(
      '[role="menu"][data-reka-menu-content][data-state="open"]',
    );
    return (
      [...menus].find(
        (menu) =>
          this.#menuItem(menu, '.pi-save', 0) !== null &&
          this.#menuItem(menu, '.pi-download', 0) !== null &&
          this.#menuItem(menu, '.pi-trash', 0) !== null &&
          this.#menuItem(menu, '.pi-times', 0) !== null,
      ) ?? null
    );
  }

  /** Resolve one direct Reka workflow action by semantic icon and occurrence. */
  #menuItem(menu: HTMLElement, iconSelector: string, index: number): HTMLElement | null {
    const items = [...menu.querySelectorAll<HTMLElement>('[role="menuitem"]')].filter(
      (item) => item.querySelector(iconSelector) !== null,
    );
    return items[index] ?? null;
  }

  /** Replace one literal-safe menu label while retaining the host value. */
  #setItemText(item: HTMLElement, value: string): void {
    const text = item.querySelector<HTMLElement>('span.flex-1');
    if (!text) return;
    if (!this.#textStates.has(text)) this.#textStates.set(text, text.textContent ?? '');
  ...
  #label(actionId: CubeHostActionId): string {
