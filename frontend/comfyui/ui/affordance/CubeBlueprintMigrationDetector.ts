...
export class CubeBlueprintMigrationDetector {
  readonly #feedback: MigrationFeedback | null;
  readonly #logger: Pick<Console, 'warn'>;
  readonly #reported = new Set<unknown>();

  /** Bind non-destructive migration feedback. */
  constructor(options: { feedback?: MigrationFeedback | null; logger: Pick<Console, 'warn'> }) {
    this.#feedback = options.feedback ?? null;
    this.#logger = options.logger;
  }

  /** Report newly discovered legacy placements once per live graph object. */
  scan(graph: object): number {
    if (!isRecord(graph) || !Array.isArray(graph._nodes)) return 0;
    const discovered = graph._nodes.filter(
      (node) => isSugarMarkedBlueprintNode(node) && !this.#reported.has(node),
    );
    for (const node of discovered) this.#reported.add(node);
    ...
    this.#feedback?.push?.(
      ...
      `${discovered.length} legacy Blueprint placement${discovered.length === 1 ? '' : 's'} remains ordinary Subgraph data. Re-import the original .cube to restore versioning and sync.`,
    );
