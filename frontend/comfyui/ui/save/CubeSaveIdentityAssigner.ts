...
export class CubeSaveIdentityAssigner {
  constructor(private readonly options: CubeSaveIdentityAssignerOptions) {}
  ...
  async ensureCubeIds(graph: ComfyGraph | null): Promise<{
    ...
    if (!graph) return { assigned: [], replacements: new Map() };
    const builder =
    ...
    const reservedCubeIds = new Set(
      (this.options.cubeBrowser?.getCubes?.() || [])
    ...
    for (const instance of instances) {
      if (instance.cubeId && isCanonicalCubeId(instance.cubeId)) continue;
      const defaultAlias = instance.defaultAlias || 'SugarCube';
      const cubeId = suggestPersonalCubeIdentity(
        defaultAlias,
        this.resolveTargetModel(instance.targetModel, instance.cubeId),
        Array.from(reservedCubeIds),
      ).cubeId;
      reservedCubeIds.add(cubeId);
      if (instance.cubeId) replacements.set(instance.cubeId, cubeId);
    ...
    for (const node of nodes) {
      if (!isCubeMarkerType(node)) continue;
      ...
      if (cubeId && !entry.cubeId) entry.cubeId = cubeId;
      if (!cubeId && node.id != null) entry.markerIds.push(node.id);
      fallbackByName.set(defaultAlias, entry);
    }
    for (const entry of fallbackByName.values()) {
      if (!entry.markerIds.length) continue;
      ...
      const cubeId = shouldReplace
        ? suggestPersonalCubeIdentity(
            entry.defaultAlias,
            this.resolveTargetModel(entry.cubeId),
            Array.from(reservedCubeIds),
          ).cubeId
        : entry.cubeId;
      if (shouldReplace) {
  ...
  resolveTargetModel(...candidates: unknown[]): string {
