...
export interface CubeSavePreflightRequest {
...
interface CubeSaveWorkflow {
  save(request?: CubeSavePreflightRequest): Promise<CubeSaveOutcome>;
  saveImplementation(request?: CubeSavePreflightRequest): Promise<CubeSaveOutcome>;
}

interface CubeSavePreflightFeedback {
  push?(severity: string, summary: string, detail?: string): unknown;
}

export interface CubeSavePreflightServiceOptions {
  workflow: CubeSaveWorkflow;
  validate(): void;
  feedback?: CubeSavePreflightFeedback | null;
}
...
export class CubeSavePreflightService {
  readonly #workflow: CubeSaveWorkflow;
  readonly #validate: () => void;
  readonly #feedback: CubeSavePreflightFeedback | null;

  /** Bind save orchestration behind a mutation-free validation boundary. */
  constructor(options: CubeSavePreflightServiceOptions) {
  ...
  async save(request: CubeSavePreflightRequest = {}): Promise<CubeSaveOutcome> {
  ...
  async saveImplementation(request: CubeSavePreflightRequest = {}): Promise<CubeSaveOutcome> {
    const failure = this.#preflight();
  ...
  #preflight(): CubeSaveOutcome | null {
    try {
      this.#validate();
      return null;
    } catch (error: unknown) {
