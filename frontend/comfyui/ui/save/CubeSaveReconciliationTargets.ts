...
import { STALE_SAVE_MODE_LATEST, type SavePlanEntry } from './CubeSaveContracts.js';
...
export class CubeSaveReconciliationTargets {
  ...
  buildMarkerTargets(savePlan: readonly SavePlanEntry[]): Record<string, string[]> {
  ...
  buildCubeNodeTargets(savePlan: readonly SavePlanEntry[]): Record<string, string[]> {
    const targets: Record<string, string[]> = {};
    for (const entry of savePlan) {
      if (!entry.cubeId) continue;
