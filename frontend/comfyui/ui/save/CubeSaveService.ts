...
interface SaveEntryMetadata extends UnknownRecord {
...
export class CubeSaveService {
  ...
  private readonly toast: SaveToast | null;
  private readonly instanceManager: SaveRefreshCoordinator | null;
  private readonly dirtyManager: SaveDirtyCoordinator | null;
  private readonly cubeBrowser: SaveBrowser | null;
  ...
  async saveImplementation({ cubeIds = null, button = null }: SaveRequest = {}): Promise<void> {
    ...
    try {
      ...
      const savePlan: SavePlanEntry[] = [];
      const previousIdByCubeId = new Map<string, string>();
  ...
  buildInstanceSaveMetadataIndex(graph: ComfyGraph): Map<string, SaveSourceEntry[]> {
  ...
  extractMetadataMarkerIds(metadata: CubeGroupMetadataRecord): GraphId[] {
  ...
  resolveSavePlanSourceMetadata(
    cubeId: string,
    sourceMetadataIndex: ReadonlyMap<string, SaveSourceEntry[]>,
  ): SaveSourceSelection {
  ...
  deriveTargetModelFromCubeIdSafe(cubeId: string): string {
    try {
  ...
  updateSourceGroupIdentity(
    source: SaveSourceEntry,
    {
      ...
      cubeVersion,
      cubeRevisionRef,
    }: { cubeId: string; defaultAlias: string; cubeVersion: string; cubeRevisionRef: string },
  ): void {
  ...
  buildSaveReconciliationTargets(savePlan: readonly SavePlanEntry[]): Record<string, string[]> {
  ...
  resolveCubeIds(cubeIds: readonly string[] | null): string[] {
  ...
  async ensureCubeIds(graph: ComfyGraph | null): Promise<{
    assigned: AssignedCubeId[];
    ...
    const fallbackByName = new Map<
      string,
      { defaultAlias: string; cubeId: string; markerIds: GraphId[] }
    >();
    for (const node of nodes) {
      ...
      if (!cubeId) {
        if (node.id != null) entry.markerIds.push(node.id);
      }
  ...
  pushToastMessage(severity: string, summary: string, detail?: string): void {
    this.toast?.push?.(severity, summary, detail);
  }

  isWritableEntry(entry: CubeBrowserEntry | null | undefined): boolean {
  ...
  buildLineagePayload(entry: CubeBrowserEntry | null | undefined): UnknownRecord | null {
    if (!entry) {
      return null;
    }
    return {
  ...
  buildHistoricalLineagePayload(
    browserEntry: CubeBrowserEntry | null | undefined,
    saveEntry: SavePlanEntry,
  ): UnknownRecord {
  ...
  buildCubeEntryIndex(entries: readonly CubeBrowserEntry[]): Map<string, CubeBrowserEntry> {
