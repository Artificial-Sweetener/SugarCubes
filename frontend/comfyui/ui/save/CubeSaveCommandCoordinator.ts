...
import {
  ...
  type CubeSaveDependencies,
  type CubeSaveOutcome,
  type SavePlanEntry,
  type SaveRequest,
...
import type { CubeSaveForkCoordinator } from './CubeSaveForkCoordinator.js';
import type { CubeSaveReconciliationTargets } from './CubeSaveReconciliationTargets.js';
import type { SavedCubeResult } from './CubeSaveReconciler.js';
import type { ComfyGraph } from '../types/graph.js';

interface CubeSaveCommandCoordinatorOptions {
  dependencies: CubeSaveDependencies;
  identities: CubeSaveIdentityAssigner;
  sources: CubeSaveSourceCatalog;
  forks: CubeSaveForkCoordinator;
  reconciliationTargets: CubeSaveReconciliationTargets;
}

/** Own the transactional application workflow for one save command. */
export class CubeSaveCommandCoordinator {
  private readonly dependencies: CubeSaveDependencies;

  constructor(private readonly options: CubeSaveCommandCoordinatorOptions) {
    this.dependencies = options.dependencies;
  }

  /** Execute a save while retaining errors and feedback at the command boundary. */
  async execute({ cubeIds = null, button = null }: SaveRequest = {}): Promise<CubeSaveOutcome> {
    const setBusy =
    ...
    try {
      ...
      if (historicalChoice === null) return { status: 'cancelled', savedCubeIds: [] };
      if (historicalChoice === 'fork') {
        ...
        for (const entry of savePlan) {
          if (entry.staleRevision) entry.staleSaveMode = STALE_SAVE_MODE_LATEST;
        }
      }
      ...
      const reviewedRequest = this.dependencies.defaultReview
        ...
        : requestBody;
      if (reviewedRequest === null) return { status: 'cancelled', savedCubeIds: [] };

      const { response, data } = await this.dependencies.api.saveImplementation(
      ...
      const warnings = Array.isArray(data.warnings)
        ...
        : [];
      if (warnings.length) this.pushToast('warn', 'SugarCubes warnings', warnings.join('\n'));
      ...
      await this.dependencies.saveReconciler.reconcile({
        ...
        cubeNodeInstanceIdsByCubeId:
          this.options.reconciliationTargets.buildCubeNodeTargets(savePlan),
        reason: 'save',
      });
  ...
  private buildSavePlan(
    graph: ComfyGraph,
    targetCubeIds: string[],
    assignedIds: string[],
    replacementIds: string[],
    replacements: ReadonlyMap<string, string>,
    cubeIndex: ReadonlyMap<string, CubeBrowserEntry>,
    sourceMetadataIndex: ReturnType<CubeSaveSourceCatalog['buildIndex']>,
    ...
    for (const cubeId of finalCubeIds) {
      ...
      const updatedCubeNodes =
        this.dependencies.cubeNodeSave?.updateIdentities(cubeNodeInstanceIds, {
          cubeId: forkedId,
          defaultAlias: forkedName,
        }) ?? 0;
      ...
      savePlan.push({
        ...
        metadata: this.options.sources.buildMetadata(
          forkedId,
          { ...entry, target_model: '', supported_models: [] },
          sourceMetadata,
        ),
  ...
  private buildRequestEntry(entry: SavePlanEntry): Record<string, unknown> {
    return {
  ...
  private resolveCubeIds(cubeIds: readonly string[] | null): string[] {
    if (Array.isArray(cubeIds) && cubeIds.length) return cubeIds.filter(Boolean);
    const dirtyManager = this.dependencies.dirtyManager;
    const implementationDirty =
      typeof dirtyManager?.getImplementationDirtyCubeIds === 'function'
        ? Array.from(dirtyManager.getImplementationDirtyCubeIds() || [])
        : Array.from(dirtyManager?.getDirtyCubeIds?.() || []);
    if (implementationDirty.length) return implementationDirty;
    return Array.from(dirtyManager?.getDirtyCubeIds?.() || []);
  }

  private pushToast(severity: string, summary: string, detail?: string): void {
