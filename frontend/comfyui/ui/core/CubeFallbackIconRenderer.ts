...
export interface FallbackIconStyle {
  fontFamily: string;
  fontWeight: number;
  inset: number;
  renderSize: number;
...
export interface FallbackIconModel {
  kind: 'initials';
  initials: string;
  fallback: FallbackIconStyle;
}

export interface FallbackIconInput {
  initials?: string;
  fallback?: Partial<FallbackIconStyle>;
}

interface FallbackDrawOptions {
  color?: string;
}

interface FallbackTextBounds {
  width: number;
  height: number;
  ascent: number;
  descent: number;
...
function fallbackStyle(model: FallbackIconInput | null | undefined) {
  ...
  return {
    ...
    fontWeight:
      typeof source.fontWeight === 'number' && Number.isFinite(source.fontWeight)
        ? source.fontWeight
        : FALLBACK_ICON_STYLE.fontWeight,
    inset:
      typeof source.inset === 'number' && Number.isFinite(source.inset)
        ? source.inset
        : FALLBACK_ICON_STYLE.inset,
    renderSize:
      Number.isFinite(renderSize) && renderSize > 0 ? renderSize : FALLBACK_ICON_STYLE.renderSize,
  };
}
 ...
 */
export function createFallbackIconModel(initials: string): FallbackIconModel {
  return {
    kind: 'initials',
    initials: initials || '?',
 ...
 */
export function resolveFallbackIconFont(
  model: FallbackIconInput | null | undefined,
  pixelSize: number,
): string {
...
function measureFallbackText(
  ...
  if (Number.isFinite(ascent) && Number.isFinite(descent) && ascent + descent > 0) {
    const measuredWidth =
      Number.isFinite(left) && Number.isFinite(right) && left + right > 0 ? left + right : width;
    return {
      width: measuredWidth,
 ...
 */
function fallbackInitialsSourceLayout(
  ctx: CanvasRenderingContext2D,
  initials: string,
  model: FallbackIconInput = {},
) {
 ...
 */
export function drawFallbackInitialsCanvas(
 ...
 */
export function drawFallbackInitialsElementCanvas(
