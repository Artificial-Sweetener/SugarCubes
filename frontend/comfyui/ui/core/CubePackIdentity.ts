...
/** Name and grouping identity for one Cube pack source. */
export interface CubePackIdentity {
  key: string;
  label: string;
  authorLabel: string;
}
...
export function resolveCubePackIdentity(value: unknown): CubePackIdentity {
  ...
  if (owner && repo) return githubIdentity(owner, repo, 'github');
  ...
  if (repoOwner && repoName) return githubIdentity(repoOwner, repoName, 'github');
  ...
  if (legacyOwner && legacyRepo) return githubIdentity(legacyOwner, legacyRepo, 'legacy');
