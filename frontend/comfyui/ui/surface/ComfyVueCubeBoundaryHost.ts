...
export class ComfyVueCubeBoundaryHost {
  ...
  constructor(body: HTMLElement, requestSlotLayoutSync: () => void = () => undefined) {
    this.#body = body;
  ...
  reconcile(): void {
    ...
    if (changed) this.#requestSlotLayoutSync();
  }
  ...
  #markSlots(row: HTMLElement, direction: 'input' | 'output'): boolean {
    const slots = [...row.querySelectorAll<HTMLElement>(`.lg-slot--${direction}`)];
    const outputPositions =
      direction === 'output' ? measureOutputTitlePositions(this.#body, row, slots.length) : [];
    let changed = false;
    for (const [index, slot] of slots.entries()) {
      this.#remember(slot);
      const boundaryIndex = String(index);
      const boundaryPosition =
        outputPositions[index] ?? `${String(((index + 1) * 100) / (slots.length + 1))}%`;
      changed =
...
function measureOutputTitlePositions(body: HTMLElement, row: HTMLElement, count: number): string[] {
