...
type SetGraphFunction = (graph: object, ...args: unknown[]) => unknown;

export interface CanvasGraphChangeSource {
 ...
 */
export class ComfyCanvasGraphChangeAdapter implements CanvasGraphChangeSource {
  readonly #canvas: UnknownRecord;
  readonly #originalSetGraph: SetGraphFunction;
  readonly #wrappedSetGraph: SetGraphFunction;
  readonly #listeners = new Set<() => void>();
  #disposed = false;

  /** Wrap one dynamic Comfy canvas after validating its navigation surface. */
  constructor(canvas: UnknownRecord) {
    if (typeof canvas.setGraph !== 'function') {
      throw new TypeError('Comfy canvas setGraph is unavailable.');
    }
    this.#canvas = canvas;
    this.#originalSetGraph = canvas.setGraph as SetGraphFunction;
  ...
  #setGraph(graph: object, args: readonly unknown[]): unknown {
