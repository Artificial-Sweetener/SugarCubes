...
export interface CubeRendererTransitionStabilizerOptions {
...
export class CubeRendererTransitionStabilizer {
  ...
  constructor(options: CubeRendererTransitionStabilizerOptions) {
  ...
  #schedule(reconcile: () => void, generation: number, remaining: number): void {
    this.#pendingFrame = this.#requestFrame(() => {
      this.#pendingFrame = null;
      if (generation !== this.#generation) return;
      reconcile();
      if (remaining > 1) this.#schedule(reconcile, generation, remaining - 1);
    });
  }
}
