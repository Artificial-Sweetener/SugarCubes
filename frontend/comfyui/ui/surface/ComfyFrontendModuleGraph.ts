...
export type ComfyRuntimeTextFetcher = (url: string) => Promise<string>;

export interface ComfyFrontendModule {
  url: string;
  source: string;
}

export interface ComfyFrontendModuleRequirement {
  key: string;
  required?: boolean;
  matches(module: ComfyFrontendModule): boolean;
}

export interface ComfyFrontendModuleDiscovery {
  entry: string;
...
export function findComfyFrontendEntry(documentRef: Document): string | null {
  return (
...
export async function discoverComfyFrontendModules(
  documentRef: Document,
  requirements: readonly ComfyFrontendModuleRequirement[],
  fetchText: ComfyRuntimeTextFetcher,
): Promise<ComfyFrontendModuleDiscovery> {
  const entry = findComfyFrontendEntry(documentRef);
  if (!entry) throw new Error('Comfy entry module was not found.');
  const pending: PendingModule[] = [{ depth: 0, priority: Number.MAX_SAFE_INTEGER, url: entry }];
  const queued = new Set<string>([entry]);
  const visited = new Set<string>();
  const modules = new Map<string, ComfyFrontendModule>();
  ...
  while (pending.length > 0 && !hasEveryRequiredModule(requirements, modules)) {
    ...
    for (const url of discoverReferencedModuleUrls(source, candidate.url)) {
      if (visited.has(url) || queued.has(url)) continue;
      queued.add(url);
      pending.push({
...
function hasEveryRequiredModule(
  requirements: readonly ComfyFrontendModuleRequirement[],
  modules: ReadonlyMap<string, ComfyFrontendModule>,
): boolean {
  return requirements.every(
    (requirement) => requirement.required === false || modules.has(requirement.key),
  );
}

/** Resolve every quoted relative JavaScript dependency without executing module code. */
export function discoverReferencedModuleUrls(source: string, moduleUrl: string): string[] {
...
function modulePriority(url: string): number {
  const filename = new URL(url).pathname.split('/').pop() ?? '';
  if (/GraphView-/i.test(filename)) return 500;
  if (/settingStore-|dialogService-/i.test(filename)) return 400;
  if (/vendor-vue-core-|vendor-primevue-/i.test(filename)) return 300;
  if (/main-/i.test(filename)) return 200;
  return 0;
}

/** Sort highest-value modules first while retaining stable shallow traversal. */
function comparePendingModules(left: PendingModule, right: PendingModule): number {
  return (
    right.priority - left.priority || left.depth - right.depth || left.url.localeCompare(right.url)
  );
}
