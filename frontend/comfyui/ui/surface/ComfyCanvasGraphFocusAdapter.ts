...
export type GraphViewportBounds = readonly [number, number, number, number];
...
export class ComfyCanvasGraphFocusAdapter {
  ...
  focus(bounds: GraphViewportBounds): boolean {
    if (!bounds.every(Number.isFinite)) return false;
