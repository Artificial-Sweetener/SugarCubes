...
import { containsCubeCanvasPoint, type CubeCanvasRect } from './CubeCanvasLayout.js';

interface CubeDropTargetCard {
  node: ComfyNode;
  rect: CubeCanvasRect;
}

/** Pair one visible Cube with the internal card geometry drawn on its face. */
export interface LiteGraphCubeDropTargetItem {
  node: ComfyNode;
  cards: readonly CubeDropTargetCard[];
}

interface CanvasDragEvent extends DragEvent {
  canvasX?: number;
  canvasY?: number;
}

interface MountedDropCallbacks {
  originalDragOver: ComfyNode['onDragOver'];
  originalDragDrop: ComfyNode['onDragDrop'];
  ownedDragOver: boolean;
  ownedDragDrop: boolean;
  installedDragOver: NonNullable<ComfyNode['onDragOver']>;
  installedDragDrop: NonNullable<ComfyNode['onDragDrop']>;
}
...
export class ComfyLiteGraphCubeDropTargetBridge {
  readonly #items = new Map<ComfyNode, LiteGraphCubeDropTargetItem>();
  readonly #mounted = new Map<ComfyNode, MountedDropCallbacks>();

  /** Reconcile native callback wrappers with the currently visible Cube faces. */
  sync(items: readonly LiteGraphCubeDropTargetItem[]): void {
    const active = new Set(items.map((item) => item.node));
    for (const node of this.#mounted.keys()) {
      if (!active.has(node)) this.#unmount(node);
    }
    this.#items.clear();
    for (const item of items) this.#items.set(item.node, item);
    for (const item of items) {
      const hooks = this.#mounted.get(item.node);
      if (hooks && ownsInstalledCallbacks(item.node, hooks)) continue;
      if (hooks) this.#mounted.delete(item.node);
  ...
  #mount(node: ComfyNode): void {
    const originalDragOver = node.onDragOver;
    const originalDragDrop = node.onDragDrop;
    const installedDragOver: NonNullable<ComfyNode['onDragOver']> = (event) => {
      const target = this.#resolveTarget(node, event);
      if (target) return target.onDragOver?.call(target, event) ?? false;
      return originalDragOver?.call(node, event) ?? false;
    };
    const installedDragDrop: NonNullable<ComfyNode['onDragDrop']> = (event) => {
      const target = this.#resolveTarget(node, event);
      if (target) return target.onDragDrop?.call(target, event) ?? false;
      return originalDragDrop?.call(node, event) ?? false;
    };
    const hooks: MountedDropCallbacks = {
      originalDragOver,
      originalDragDrop,
  ...
  #resolveTarget(node: ComfyNode, event: DragEvent): ComfyNode | null {
    const adjusted = event as CanvasDragEvent;
    const point: readonly [number, number] = [Number(adjusted.canvasX), Number(adjusted.canvasY)];
...
function ownsInstalledCallbacks(node: ComfyNode, hooks: MountedDropCallbacks): boolean {
...
function restoreCallback<Key extends 'onDragOver' | 'onDragDrop'>(
  node: ComfyNode,
  key: Key,
  value: ComfyNode[Key],
  owned: boolean,
  ...
  if (!owned) Reflect.deleteProperty(node, key);
}
