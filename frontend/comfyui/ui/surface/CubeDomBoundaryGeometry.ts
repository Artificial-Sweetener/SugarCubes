...
import type { CubePortAnchor } from '../cube/connection/CubePortPresentationController.js';

const SOCKET_EDGE_INSET = 12;
const DEFAULT_SOCKET_RADIUS = 6;

export interface CubeBoundaryMeasurement {
  anchors: CubePortAnchor[];
  graphOriginOffset: number;
  rowOffset: number;
  rowMinimumY: number;
  rowMaximumY: number;
}
...
export function measureCubeBoundary(
  body: HTMLElement,
  nodeRoot: HTMLElement,
  row: HTMLElement,
  slots: readonly HTMLElement[],
  measuredRowPositions: readonly number[],
  nodeTitleHeight: number,
): CubeBoundaryMeasurement {
...
export function measureSocketRadius(body: HTMLElement, slot: HTMLElement | undefined): number {
  const dot = slot?.querySelector<HTMLElement>('[data-testid="slot-connection-dot"]');
  if (!dot) return DEFAULT_SOCKET_RADIUS;
...
export function readLocalOffset(ancestor: HTMLElement, descendant: HTMLElement): number {
...
export function readVueNodeGraphY(
  nodeRoot: HTMLElement,
  nodeTitleHeight: number,
  fallback: number,
): number {
  ...
  const matrix =
    /matrix\(\s*[^,]+,\s*[^,]+,\s*[^,]+,\s*[^,]+,\s*[^,]+,\s*(-?\d+(?:\.\d+)?)\s*\)/u.exec(
      transform,
    );
...
function readCenterOffset(row: HTMLElement, descendant: HTMLElement): number {
...
function readLocalHeight(scaleOwner: HTMLElement, element: HTMLElement): number {
