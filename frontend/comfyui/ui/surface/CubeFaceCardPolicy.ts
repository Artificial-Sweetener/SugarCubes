...
import {
  ...
  inferCubeFaceTransformSignals,
  isHardHiddenCubeFaceNode,
...
export interface CubeFaceCardPresentation {
  cards: CubeFaceCardDecision[];
  menuEntries: CubeFaceCardMenuEntry[];
}
...
export function resolveCubeFaceCardPresentation(
  ...
  nodes.forEach((node, index) => {
    if (isHardHiddenCubeFaceNode(node)) return;
    ...
    const showActivationControl =
      cubeFaceNodeAllowsActivationControl(node) &&
      (reveal.revealable || inferCubeFaceTransformSignals(node).size > 0);
    const eligible =
      cubeFaceNodeHasVisibleWidgets(node) || showActivationControl || reveal.revealable;
    if (!eligible) return;
