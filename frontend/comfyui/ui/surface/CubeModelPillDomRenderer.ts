...
export function presentCubeModelTitle(
  host: HTMLElement,
  presentation: CubeModelTitlePresentation,
  options: CubeModelPillDomOptions = {},
): boolean {
  const signature = JSON.stringify(presentation);
  if (host.dataset.sugarcubesModelTitleSignature === signature) return false;
...
function createModelPill(documentRef: Document, modelText: string): HTMLSpanElement {
...
function ensureCubeModelPillStyles(documentRef: Document): void {
