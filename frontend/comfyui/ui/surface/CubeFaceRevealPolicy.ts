...
import type { CubeFaceCardState } from './CubeSurfaceState.js';

const BYPASS_MODE = 4;

export interface CubeFaceRevealDecision {
  authoredBypass: boolean;
  revealable: boolean;
  revealed: boolean;
  visible: boolean;
  enabledChoice: boolean;
  enabled: boolean;
}

/** Resolve reveal independently from the activation choice it gates. */
export function resolveCubeFaceRevealDecision(
  node: ComfyNode,
  persisted: CubeFaceCardState | undefined,
): CubeFaceRevealDecision {
