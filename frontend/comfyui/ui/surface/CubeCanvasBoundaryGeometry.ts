...
export function resolveCubeCanvasOutputSocketCenterX(
  frameRight: number,
  slotHeight = CUBE_CANVAS_SLOT_HEIGHT,
): number {
  return frameRight + 1 - slotHeight / 2;
}

/** Match LiteGraph's native input-slot center inset. */
export function resolveCubeCanvasInputSocketCenterX(
  frameLeft: number,
  slotHeight = CUBE_CANVAS_SLOT_HEIGHT,
): number {
...
export function resolveCubeCanvasOutputSocketRimX(
  frameRight: number,
  slotHeight = CUBE_CANVAS_SLOT_HEIGHT,
  socketRadius = CUBE_CANVAS_SOCKET_RADIUS,
): number {
