...
import {
  ...
  CUBE_PREVIEW_EDGE_INSET,
  resolveCubeOutputPortY,
} from './CubeOutputLeaderGeometry.js';

const SVG_NAMESPACE = 'http://www.w3.org/2000/svg';

export interface CubeDomOutputLeader {
  index: number;
  title: HTMLElement;
  portY: number;
  socketRadius: number;
...
export class CubeDomPortLeaderHost {
  ...
  readonly #paths = new Map<number, SVGPathElement>();
  ...
  render(leaders: readonly CubeDomOutputLeader[]): void {
    if (this.#svg.parentElement !== this.#body) this.#body.append(this.#svg);
    ...
    for (const leader of leaders) {
      ...
      const label =
        leader.title.querySelector<HTMLElement>('[data-cube-preview-output-label]') ?? leader.title;
      const titleRect = label.getBoundingClientRect();
      const labelEndX = Math.min(
        elbowX,
        Math.max(0, (titleRect.right - bodyRect.left) / safeScale + CUBE_OUTPUT_LEADER_LABEL_INSET),
      );
      ...
      path.setAttribute(
        'd',
        `M ${format(labelEndX)} ${format(labelY)} ` +
  ...
  #createPath(index: number): SVGPathElement {
