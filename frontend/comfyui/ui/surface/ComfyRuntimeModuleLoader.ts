...
export interface ComfyRuntimeAssets {
  entry: string;
  vueRuntime: string;
  hostRuntime: string;
  graphView: string;
...
export interface ComfyVueNodeRenderRuntime {
...
export type RuntimeTextFetcher = (url: string) => Promise<string>;
...
export async function discoverComfyRuntimeAssets(
  documentRef: Document,
  fetchText: RuntimeTextFetcher = fetchRuntimeText,
): Promise<ComfyRuntimeAssets> {
  ...
  if (!vueMatch?.[1] || !hostMatch?.[1] || !graphViewMatch?.[1]) {
    throw new NativeRendererCompatibilityError(
      'Comfy native renderer modules do not match the supported runtime contract.',
    );
  }
  return {
    entry,
...
export function buildNativeNodeComponentModule(source: string, graphViewUrl: string): string {
  ...
  const resolvedSource = source.replace(
    ...
    (_match, prefix: string, quote: string, relativePath: string) =>
      `${prefix}${quote}${new URL(relativePath, graphViewUrl).href}${quote}`,
  );
