...
export class ComfyVueNodeHeaderAccessoryHost {
  ...
  constructor(accessory: HTMLElement) {
    this.#accessory = accessory;
  }

  /** Move the accessory into the current native header's in-flow trailing row. */
  reconcile(nativeRoot: HTMLElement, node: ComfyNode): boolean {
    ...
    if (this.#accessory.parentElement !== row) row.append(this.#accessory);
    return true;
  }

  /** Release the accessory and every marker owned by this adapter. */
  dispose(): void {
    if (this.#header) delete this.#header.dataset.sugarcubeCardHeaderAccessory;
...
function findNativeNodeHeader(nativeRoot: HTMLElement, node: ComfyNode): HTMLElement | null {
  const expectedTestId = `node-header-${String(node.id ?? '')}`;
  for (const candidate of nativeRoot.querySelectorAll<HTMLElement>(
...
function findNativeNodeHeaderRow(header: HTMLElement): HTMLElement | null {
