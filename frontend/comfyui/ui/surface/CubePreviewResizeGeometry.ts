...
export interface CubePreviewWidthRange {
  minimum: number;
  maximum: number;
}

/** Resolve a finite range while preserving the minimum masonry column. */
export function resolveCubePreviewWidthRange(
  availableWidth: number,
  minimumMasonryWidth: number,
  gap: number,
): CubePreviewWidthRange {
...
export function clampCubePreviewWidth(width: number, range: CubePreviewWidthRange): number {
