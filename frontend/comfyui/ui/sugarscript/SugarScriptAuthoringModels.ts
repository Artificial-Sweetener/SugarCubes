...
export interface SugarScriptSourcePosition {
  offset: number;
  line: number;
  column: number;
}

export interface SugarScriptDiagnostic {
  code: string;
  severity: 'error' | 'warning';
  message: string;
  span: { start: SugarScriptSourcePosition; end: SugarScriptSourcePosition };
}

export interface SugarScriptNativeInstance {
  instanceId: string;
  alias: string;
  bypassed: boolean;
...
export interface SugarScriptNativeConnection {
...
export interface SugarScriptNativeWorkflowPlan {
  semanticHash: string;
  instances: readonly SugarScriptNativeInstance[];
  connections: readonly SugarScriptNativeConnection[];
}

export interface SugarScriptAuthoringResponse {
  valid: boolean;
  diagnostics: readonly SugarScriptDiagnostic[];
  plan: SugarScriptNativeWorkflowPlan | null;
}

/** Parse the complete untrusted endpoint response or fail before graph mutation. */
export function readSugarScriptAuthoringResponse(value: unknown): SugarScriptAuthoringResponse {
  if (!isRecord(value)) throw new TypeError('SugarScript response must be an object.');
...
function readPlan(value: unknown): SugarScriptNativeWorkflowPlan {
  if (!isRecord(value)) throw new TypeError('SugarScript workflow plan must be an object.');
...
function readInstance(value: unknown): SugarScriptNativeInstance {
  if (!isRecord(value)) throw new TypeError('SugarScript instance must be an object.');
  const payload = readImportPayload(value.payload);
  if (!payload?.cube) throw new TypeError('SugarScript instance has no valid Cube payload.');
  return {
...
function readConnection(value: unknown): SugarScriptNativeConnection {
  if (!isRecord(value)) throw new TypeError('SugarScript connection must be an object.');
  return {
...
function readDiagnostics(value: unknown): SugarScriptDiagnostic[] {
  if (!Array.isArray(value)) throw new TypeError('SugarScript diagnostics must be an array.');
...
function readPosition(value: unknown): SugarScriptSourcePosition {
  if (!isRecord(value)) throw new TypeError('SugarScript source position is invalid.');
...
function requireText(value: unknown, field: string): string {
  ...
  if (!text) throw new TypeError(`SugarScript ${field} is required.`);
  return text;
}
