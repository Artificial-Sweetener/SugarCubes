...
export interface PngSugarScriptMetadata {
  sugarScript: string | null;
...
export async function readPngSugarScriptMetadata(
  input: PngArtifactInput,
): Promise<PngSugarScriptMetadata> {
  ...
  let sawHeader = false;
  let sawEnd = false;
  let sugarScript: string | null = null;
  ...
  while (offset < bytes.byteLength) {
    ...
    if (text?.keyword === 'workflow') {
      if (workflow !== null) {
        throw artifactError(
          'image.duplicate_workflow',
          'PNG contains more than one workflow record.',
        );
      }
    ...
    if (text?.keyword === 'sugar_script') {
      if (sugarScript !== null) {
        throw artifactError(
          ...
          'PNG contains more than one SugarScript record.',
        );
      }
...
export async function readScriptOnlySugarScript(input: PngArtifactInput): Promise<string> {
  const metadata = await readPngSugarScriptMetadata(input);
  if (metadata.workflow !== null) {
    throw artifactError(
      'image.workflow_authority_present',
      'PNG contains workflow metadata; import it through the workflow authority path.',
    );
  }
...
function parseWorkflow(value: string): Record<string, unknown> {
  let parsed: unknown;
  try {
...
function readTextChunk(type: string, data: Uint8Array): PngTextRecord | null {
  if (type === 'tEXt') return readPlainText(data);
  if (type === 'iTXt') return readInternationalText(data);
  if (type === 'zTXt') {
    const separator = data.indexOf(0);
    const keyword = separator < 0 ? '' : decodeLatin1(data.subarray(0, separator));
    if (keyword === 'sugar_script' || keyword === 'workflow') {
      throw artifactError(
        'image.compressed_metadata_unsupported',
        `Compressed PNG metadata '${keyword}' is unsupported.`,
      );
    }
  }
  return null;
...
function readPlainText(data: Uint8Array): PngTextRecord | null {
  const separator = data.indexOf(0);
...
function readInternationalText(data: Uint8Array): PngTextRecord | null {
  const keywordEnd = data.indexOf(0);
  if (keywordEnd < 1 || keywordEnd + 2 >= data.byteLength) return null;
...
function artifactError(code: string, message: string): PngSugarScriptArtifactError {
