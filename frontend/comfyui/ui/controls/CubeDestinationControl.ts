...
import type { CubeAuthoringDestinationOption } from '../create/CubeAuthoringDialog.js';
import type { CubeSaveDestination } from '../create/CubeAuthoringIdentity.js';
import {
  ComfySettingsSingleSelectControl,
  type ComfySettingsSelectOption,
  type ComfySettingsSelectRenderer,
} from './ComfySettingsSelect.js';

export interface CubeDestinationControlOptions {
  candidateDestination?: CubeSaveDestination | CubeSaveDestination['kind'];
...
export class CubeDestinationControl {
  ...
  readonly #control: ComfySettingsSingleSelectControl;
  readonly #locked: boolean;
  readonly #onChange: () => void;
  readonly #onCreateDestination: (() => Promise<CubeAuthoringDestinationOption | null>) | undefined;
  readonly #onError: (message: string) => void;
  #entries: CubeAuthoringDestinationOption[];
  #selected: CubeAuthoringDestinationOption | null;

  constructor(options: CubeDestinationControlOptions) {
  ...
  destination(): CubeSaveDestination | null {
  ...
  presentation(): string {
    ...
    if (!selected) return 'Destination required';
  ...
  #select(entry: CubeAuthoringDestinationOption): void {
...
function destinationOptions(
  entries: readonly CubeAuthoringDestinationOption[],
...
function normalizeDestinations(
  destinations: readonly CubeAuthoringDestinationOption[],
  candidateDestination: CubeSaveDestination | CubeSaveDestination['kind'] | undefined,
...
function selectInitialDestination(
  destinations: readonly CubeAuthoringDestinationOption[],
  candidateDestination: CubeSaveDestination | CubeSaveDestination['kind'] | undefined,
): CubeAuthoringDestinationOption | null {
  const requested = readConcreteDestination(candidateDestination);
  if (requested) {
    const matching = destinations.find(
      (entry) =>
        entry.destination?.kind === requested.kind &&
...
function readConcreteDestination(
  destination: CubeSaveDestination | CubeSaveDestination['kind'] | undefined,
): CubeSaveDestination | null {
