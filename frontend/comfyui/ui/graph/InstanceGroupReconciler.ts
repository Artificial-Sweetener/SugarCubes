...
import type { InstanceMatch, ManagedGroup } from './InstanceManagerContracts.js';

/** Match discovered instances to authoritative or safely reusable graph groups. */
export class InstanceGroupReconciler {
  resolve(instances: readonly CubeInstance[], groups: readonly ManagedGroup[]): InstanceMatch[] {
    const byInstance = new Map<string, ManagedGroup>();
    const byMarkers = new Map<string, ManagedGroup>();
    for (const group of groups) {
      const data = getGroupSugarcubes(group);
      const instanceId = readMetadataString(data?.instance_id);
      if (instanceId) byInstance.set(instanceId, group);
      if (data?.managed) {
        const signature = buildMarkerSignature(readMarkerIdsFromMetadata(data));
        if (signature) byMarkers.set(signature, group);
      }
    }
    const claimed = new Set<ManagedGroup>();
    const matches = instances.map((instance, order): InstanceMatch => {
      const markerSignature = buildMarkerSignature(instance.markerIds);
      let group =
        byInstance.get(instance.instanceId) ||
        (markerSignature ? byMarkers.get(markerSignature) : null) ||
        null;
      if (group && getGroupSugarcubes(group) && claimed.has(group)) group = null;
      if (!group)
        group = this.findReusableGroup(
          instance,
          groups.filter((entry) => !claimed.has(entry)),
        );
      if (group && getGroupSugarcubes(group)) claimed.add(group);
      return { instance, group, order };
    });
    return this.mergeMarkerOnlyMatches(matches);
  }

  buildSignature(instances: readonly CubeInstance[], groups: readonly ComfyGroup[]): string {
    ...
    const titleSignature = groups
      .map((group) => {
        const data = getGroupSugarcubes(group);
        if (!data?.managed || !data.instance_id) return '';
  ...
  private findReusableGroup(
    ...
    for (const group of groups) {
      if (!group || getGroupSugarcubes(group)) continue;
  ...
  private mergeMarkerOnlyMatches(matches: readonly InstanceMatch[]): InstanceMatch[] {
    ...
    for (const groupMatches of grouped.values()) {
      ...
      for (const match of ordered) {
        canonical.markerLookup.inputs = mergeLists(
          canonical.markerLookup.inputs,
          match.instance.markerLookup?.inputs,
        );
        canonical.markerLookup.outputs = mergeLists(
...
function mergeLists<T>(left: readonly T[] = [], right: readonly T[] = []): T[] {
