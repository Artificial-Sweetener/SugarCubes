...
/**
 * Own the SugarCubes graph integration layer in `frontend/comfyui/ui/graph/CubeBounds.js`.
 */

import { readGroupBounds, readNodeBounds } from './Bounds.js';
import { isRecord } from '../types/common.js';
import type { Bounds, RectBounds, UnknownRecord } from '../types/common.js';
import type { ComfyGroup, ComfyNode } from '../types/graph.js';

export interface CubeBoundsPadding extends UnknownRecord {
  x: number;
  y: number;
  top_extra: number;
}

export interface CubeBoundsHeader extends UnknownRecord {
  height: number;
}

export interface CubeBoundsPayload extends RectBounds {
  padding: CubeBoundsPadding;
  header: CubeBoundsHeader;
}
...
interface MutableNumericArray {
  readonly length: number;
  [index: number]: number;
}

interface ChromeBoundsOptions {
  nodes?: readonly ComfyNode[] | null;
  markers?: readonly ComfyNode[] | null;
  padding?: unknown;
  header?: unknown;
  minimumMargins?: Partial<AutoMargins>;
}

interface InstanceBoundsOptions {
...
interface CanonicalBoundsWriteOptions {
  group?: ComfyGroup | null | undefined;
  metadata?: UnknownRecord | null | undefined;
  bounds?: RectBounds | CubeBoundsPayload | null | undefined;
}
...
function hasWritableVec2(value: unknown): value is MutableNumericArray {
  return (
...
function hasWritableBounds(value: unknown): value is MutableNumericArray {
  return (
    (Array.isArray(value) || ArrayBuffer.isView(value)) &&
    Number((value as { length?: unknown }).length) >= 4
  );
}

function isCollapsedNode(node: ComfyNode | null | undefined): boolean {
  return isRecord(node?.flags) && node.flags.collapsed === true;
}

function readLiteGraphNumber(
  name: 'NODE_COLLAPSED_WIDTH' | 'NODE_TITLE_HEIGHT',
  fallback: number,
): number {
...
function readNodeLayoutBounds(node: ComfyNode | null | undefined): Bounds | null {
  if (!node) {
    return null;
  }
  const pos = node?.pos;
...
function isValidBounds(bounds: unknown): bounds is RectBounds {
  return (
    isRecord(bounds) &&
...
function normalizePadding(padding: unknown): CubeBoundsPadding {
  ...
  if (
    resolved.x === LEGACY_CUBE_INSTANCE_PADDING.x &&
...
function resolveHeaderHeight(header: unknown): number {
...
function resolvePaddingHeader(
  bounds: unknown,
  metadata: UnknownRecord | null | undefined,
): { padding: CubeBoundsPadding; header: CubeBoundsHeader } {
 ...
 */
export function normalizeBoundsPayload(
  bounds: RectBounds,
  metadata: UnknownRecord | null | undefined,
): CubeBoundsPayload {
 ...
 */
export function resolveCanonicalPadding(
  metadata: UnknownRecord | null | undefined,
  bounds: unknown,
): { padding: CubeBoundsPadding; header: CubeBoundsHeader } {
 ...
 */
export function computeInstanceBounds(
  nodes: readonly ComfyNode[] | null | undefined,
  markers: readonly ComfyNode[] | null | undefined,
): ContentBounds | null {
 ...
 */
export function computeVisualContentBounds(
  nodes: readonly ComfyNode[] | null | undefined,
  markers: readonly ComfyNode[] | null | undefined,
): ContentBounds | null {
  const entries = [...(nodes || []), ...(markers || [])];
 ...
 */
export function inflateInstanceBounds(
  bounds: ContentBounds | null | undefined,
  padding: unknown = null,
): RectBounds | null {
  if (!bounds) {
    return null;
  }
...
function computeContentMargins(
 ...
 */
export function expandBoundsForContentMargins(
  bounds: RectBounds | null | undefined,
  contentBounds: ContentBounds | null | undefined,
  minimumMargins: Partial<ContentMargins> | null | undefined,
): RectBounds | null | undefined {
  if (!bounds || !contentBounds) {
    return bounds;
  }
  ...
  const addRight =
    (Number.isFinite(minRight) ? minRight : 0) - (next.x + next.w - contentBounds.maxX);
  if (addRight > 0) {
    next.w += addRight;
  }
  const addBottom =
    (Number.isFinite(minBottom) ? minBottom : 0) - (next.y + next.h - contentBounds.maxY);
  if (addBottom > 0) {
    next.h += addBottom;
  }
  return next;
 ...
 */
export function contentFitsWithinBounds(
  bounds: RectBounds | null | undefined,
  contentBounds: ContentBounds | null | undefined,
  tolerance = 2,
): boolean {
  ...
  return (
    contentBounds.minX >= bounds.x - resolvedSlack &&
...
function unionContentBounds(
  primary: ContentBounds | null,
  secondary: ContentBounds | null,
): ContentBounds | null {
  if (!primary) {
 ...
 */
export function resolveChromeBoundsFromContent({
  nodes,
  markers,
  padding,
  header,
  minimumMargins = CUBE_INSTANCE_AUTO_MIN_MARGINS,
}: ChromeBoundsOptions = {}): CubeBoundsPayload | null {
  ...
  const innerTop = Number.isFinite(Number(minimumMargins.innerTop))
    ...
    : 0;
  const marginTop =
    resolvedPadding.y + resolvedPadding.top_extra + resolvedHeader.height + innerTop;
 ...
 */
export function resolveNewInstanceBounds(
  options: ChromeBoundsOptions = {},
 ...
 */
export function resolveInstanceBounds({
  group,
  metadata,
  nodes,
  markers,
}: InstanceBoundsOptions = {}): CubeBoundsPayload | RectBounds | null {
 ...
 */
export function computeInnerBounds(bounds: unknown): RectBounds | null {
 ...
 */
export function expandBoundsToIncludeRect(
  bounds: CubeBoundsPayload | RectBounds | null | undefined,
  rect: RectBounds | null | undefined,
  extraPadding = 2,
): CubeBoundsPayload | RectBounds | null | undefined {
 ...
 */
export function writeCanonicalBounds({
  group,
  metadata,
  bounds,
}: CanonicalBoundsWriteOptions = {}): CubeBoundsPayload | null {
  if (!bounds) {
    return null;
  }
