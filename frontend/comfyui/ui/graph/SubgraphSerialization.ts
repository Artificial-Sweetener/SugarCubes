...
import type { Bounds, UnknownRecord, Vec2 } from '../types/common.js';
import {
  normalizeSubgraphLinks,
  normalizeSubgraphSlotType,
  readSerializedGraphId,
  type NormalizedSubgraphLink,
} from './SubgraphConnectionSerialization.js';
import type { GraphId } from '../types/graph.js';

interface NormalizeSubgraphOptions {
  fallbackName?: unknown;
  expectedInputNames?: readonly unknown[];
}

interface GraphLayoutBounds {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
interface NormalizedSubgraphIo extends UnknownRecord {
  id: string;
  type: string;
  linkIds: number[];
  name: string;
...
export interface NormalizedSubgraphPayload extends UnknownRecord {
  id: string;
  version: 1;
  revision: number;
  state: GraphState;
  ...
  name: string;
  inputNode: UnknownRecord;
  outputNode: UnknownRecord;
  inputs: NormalizedSubgraphIo[];
  outputs: NormalizedSubgraphIo[];
  widgets: UnknownRecord[];
  nodes: UnknownRecord[];
  links: NormalizedSubgraphLink[];
  floatingLinks: UnknownRecord[];
  reroutes: UnknownRecord[];
...
const DEFAULT_IO_BOUNDS: Readonly<Bounds> = Object.freeze([0, 0, 75, 100]);
const DEFAULT_NODE_SIZE: Readonly<Vec2> = Object.freeze([180, 60]);

/**
 * Normalize one subgraph payload into the current LiteGraph-compatible shape.
 */
export function normalizeSubgraphPayload(
  rawEntry: unknown,
  fallbackId: unknown,
  options: NormalizeSubgraphOptions = {},
): NormalizedSubgraphPayload | null {
 ...
 */
export function looksLikeCurrentSubgraphPayload(entry: unknown): boolean {
  if (!isRecord(entry) || Array.isArray(entry)) {
    return false;
  }
  return Boolean(
    (entry.inputNode && typeof entry.inputNode === 'object') ||
 ...
 */
export function looksLikeLegacyWorkflowSubgraph(entry: unknown): boolean {
  if (!isRecord(entry) || Array.isArray(entry)) {
    return false;
  }
  return (
    !looksLikeCurrentSubgraphPayload(entry) &&
 ...
 */
function normalizeCurrentSubgraphPayload(
  entry: UnknownRecord,
  subgraphId: string,
  options: NormalizeSubgraphOptions,
): NormalizedSubgraphPayload {
 ...
 */
function normalizeLegacySubgraphPayload(
  entry: UnknownRecord,
  subgraphId: string,
  options: NormalizeSubgraphOptions,
): NormalizedSubgraphPayload {
...
function normalizeGraphState(
  ...
  links: readonly NormalizedSubgraphLink[],
  groups: readonly UnknownRecord[],
  reroutes: readonly UnknownRecord[],
): GraphState {
  const rawState = isRecord(entry.state) ? entry.state : {};
  return {
    lastNodeId: Math.max(
      coerceInteger(rawState.lastNodeId, 0),
      coerceInteger(entry.last_node_id, 0),
      maxNumericValue(nodes.map((node) => node.id)),
    ),
    lastLinkId: Math.max(
      coerceInteger(rawState.lastLinkId, 0),
      coerceInteger(entry.last_link_id, 0),
      maxNumericValue(links.map((link) => link.id)),
    ),
    lastGroupId: Math.max(
      coerceInteger(rawState.lastGroupId, 0),
      maxNumericValue(groups.map((group) => group.id)),
    ),
    lastRerouteId: Math.max(
      coerceInteger(rawState.lastRerouteId, 0),
      maxNumericValue(reroutes.map((reroute) => reroute.id)),
    ),
  };
}
 ...
 */
function normalizeIoNode(
  rawNode: unknown,
  defaultId: number,
  layoutBounds: GraphLayoutBounds | null,
  side: 'input' | 'output',
): UnknownRecord {
...
function normalizeBounding(
  ...
  if (Array.isArray(rawBounding) && rawBounding.length === 4) {
    return rawBounding.map((value, index) =>
      index < 2
 ...
 */
function normalizeNodeEntries(rawNodes: unknown): UnknownRecord[] {
  if (!Array.isArray(rawNodes)) {
    return [];
  }
  return rawNodes
    .filter((node): node is UnknownRecord => isRecord(node) && !Array.isArray(node))
 ...
 */
function normalizeSubgraphIoArray(
  entries: unknown,
  kind: 'input' | 'output',
  subgraphId: string,
): NormalizedSubgraphIo[] {
  if (!Array.isArray(entries)) {
    return [];
  }
  const seenNames = new Set<string>();
  return entries
    .filter((entry): entry is UnknownRecord => isRecord(entry) && !Array.isArray(entry))
 ...
 */
function normalizeSubgraphIoEntry(
  entry: UnknownRecord,
  index: number,
  kind: 'input' | 'output',
  subgraphId: string,
 ...
 */
function buildLegacySubgraphInputs(
  links: readonly NormalizedSubgraphLink[],
  nodes: readonly UnknownRecord[],
  subgraphId: string,
  expectedNames: readonly unknown[] = [],
): NormalizedSubgraphIo[] {
  const grouped = groupLinksBySlot(
    links.filter((link) => link.origin_id === SUBGRAPH_INPUT_ID),
    'origin_slot',
  );
 ...
 */
function buildLegacySubgraphOutputs(
  links: readonly NormalizedSubgraphLink[],
  nodes: readonly UnknownRecord[],
  subgraphId: string,
): NormalizedSubgraphIo[] {
  const grouped = groupLinksBySlot(
    links.filter((link) => link.target_id === SUBGRAPH_OUTPUT_ID),
    'target_slot',
  );
 ...
 */
function buildLegacySubgraphIo(
  groupedLinks: ReadonlyMap<number, NormalizedSubgraphLink[]>,
  nodes: readonly UnknownRecord[],
  subgraphId: string,
  kind: 'input' | 'output',
  expectedNames: readonly unknown[] = [],
  ...
  const seenNames = new Set<string>();
  const nodeIndex = new Map<GraphId, UnknownRecord>();
  for (const node of nodes) {
    const nodeId = readSerializedGraphId(node.id);
    if (nodeId !== null) nodeIndex.set(nodeId, node);
  }
  const ioEntries: NormalizedSubgraphIo[] = [];
  ...
  for (const [slotIndex, slotLinks] of groupedLinks.entries()) {
    ...
    const name = makeUniqueName(
      readTrimmedString(expectedNames?.[slotIndex]) ||
 ...
 */
function resolveLegacyBoundarySlot(
  link: NormalizedSubgraphLink | undefined,
  nodeIndex: ReadonlyMap<GraphId, UnknownRecord>,
  kind: 'input' | 'output',
): UnknownRecord | null {
  if (!link) {
    return null;
 ...
 */
function groupLinksBySlot(
  links: readonly NormalizedSubgraphLink[],
  key: 'origin_slot' | 'target_slot',
): Map<number, NormalizedSubgraphLink[]> {
  const grouped = new Map<number, NormalizedSubgraphLink[]>();
 ...
 */
function normalizeObjectArray(entries: unknown): UnknownRecord[] {
  if (!Array.isArray(entries)) {
    return [];
  }
  return entries
 ...
 */
function normalizeLinkIdArray(linkIds: unknown): number[] {
 ...
 */
function makeUniqueName(seed: unknown, seen: Set<string>): string {
 ...
 */
function resolveDisplayLabel(entry: UnknownRecord | null, fallbackName: string): string {
  return (
 ...
 */
function maxNumericValue(values: readonly unknown[]): number {
  let max = 0;
