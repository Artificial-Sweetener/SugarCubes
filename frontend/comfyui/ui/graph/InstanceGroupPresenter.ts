...
import {
  ...
  type CubeBoundsHeader,
  type CubeBoundsPadding,
} from './CubeBounds.js';
...
export class InstanceGroupPresenter {
  ...
  apply(
    ...
    const revisionRef = normalizeRevisionRef(
      instance.cubeRevisionRef || cleaned?.cube_revision_ref,
    );
    const definitionKey =
    ...
    if (!usedNewResolver && !cleaned && (visualBounds || contentBounds)) {
      bounds =
        expandBoundsForContentMargins(bounds, visualBounds || contentBounds, {
          ...
          top:
            padding.y + padding.top_extra + header.height + CUBE_INSTANCE_AUTO_MIN_MARGINS.innerTop,
        }) ?? bounds;
    }
    ...
    if (target.__sugarcubes_imported) delete target.__sugarcubes_imported;
  ...
  private createGroup(graph: ComfyGraph, metadata: CubeGroupMetadataRecord): ManagedGroup | null {
    const Group = this.options.adapter.getLiteGraph?.()?.LGraphGroup;
    if (!Group) return null;
    const group = new Group(
      resolveInstanceDisplayName({ metadata, fallback: 'SugarCube' }) || 'SugarCube',
    );
    graph.add?.(group);
    if (!group.color) group.color = GROUP_COLOR;
    if (!group.bgcolor) group.bgcolor = GROUP_BACKGROUND;
    return group;
  }

  private bindTitleProjection(group: ManagedGroup, graph: ComfyGraph): void {
...
function normalizePadding(value: unknown): CubeBoundsPadding {
...
function normalizeHeader(value: unknown): CubeBoundsHeader {
  const source = isRecord(value) ? value : {};
  return { height: readNumber(source.height, CUBE_INSTANCE_HEADER_HEIGHT) };
}
function normalizeBoundsGeometry(value: unknown): RectBounds | null {
  const source = isRecord(value) ? value : {};
...
function readGroupBoundsGeometry(group: ManagedGroup | null): RectBounds | null {
