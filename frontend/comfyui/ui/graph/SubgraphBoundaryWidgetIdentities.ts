...
import type { ComfyLink, ComfyNode, GraphId } from '../types/graph.js';

const SUBGRAPH_INPUT_NODE_ID = '-10';

export interface SubgraphBoundarySource {
  links?: ComfyLink[];
  nodes?: ComfyNode[];
}

/** Index boundary-supplied widget names by their target node identity. */
export function indexSubgraphBoundaryWidgetNames(
  subgraph: SubgraphBoundarySource,
): ReadonlyMap<string, ReadonlySet<string>> {
  const boundaryTargets = boundaryLinkTargets(subgraph.links);
  const indexed = new Map<string, ReadonlySet<string>>();
  for (const node of subgraph.nodes ?? []) {
    ...
    for (const input of node.inputs ?? []) {
      ...
      if (linkId && name && boundaryTargets.get(linkId) === nodeId) names.add(name);
    }
    if (names.size > 0) indexed.set(nodeId, names);
  }
  return indexed;
}

/** Map subgraph-input link identities to their exact target nodes. */
function boundaryLinkTargets(links: readonly ComfyLink[] | undefined): ReadonlyMap<string, string> {
...
function readWidgetName(input: NonNullable<ComfyNode['inputs']>[number]): string {
...
function graphIdKey(value: GraphId | null | undefined): string {
