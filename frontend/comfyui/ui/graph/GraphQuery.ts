...
/**
 * Own the SugarCubes graph integration layer in `frontend/comfyui/ui/graph/GraphQuery.js`.
 */

import { isRecord } from '../types/common.js';
import type { ComfyGraph, ComfyGroup, ComfyLink, ComfyNode, GraphId } from '../types/graph.js';

export interface LinkIndex {
  outgoing: Map<string, ComfyLink[]>;
  incoming: Map<string, ComfyLink[]>;
  links: ComfyLink[];
}
 ...
 * Get graph nodes.
 */
export function getGraphNodes(graph: unknown): ComfyNode[] {
  if (!isRecord(graph)) return [];
  if (Array.isArray(graph._nodes)) return graph._nodes as ComfyNode[];
  if (Array.isArray(graph.nodes)) return graph.nodes as ComfyNode[];
  return [];
}
 ...
 * Get graph groups.
 */
export function getGraphGroups(graph: unknown): ComfyGroup[] {
  if (!isRecord(graph)) return [];
  if (Array.isArray(graph._groups)) return graph._groups as ComfyGroup[];
  if (Array.isArray(graph.groups)) return graph.groups as ComfyGroup[];
  return [];
}
...
function coerceLinkId(value: unknown): GraphId | null {
...
function collectLinksFromContainer(container: unknown): ComfyLink[] {
  if (!container) {
    return [];
  }
...
function buildLinkKey(link: ComfyLink | null | undefined): string {
...
function dedupeLinks(links: readonly ComfyLink[]): ComfyLink[] {
  const deduped: ComfyLink[] = [];
...
function collectLinksFromSlots(graph: ComfyGraph): ComfyLink[] {
  ...
  const inputsByLinkId = new Map<
    string,
    Array<{ target_id: GraphId; target_slot: number; type: unknown }>
  >();
  ...
  const linksById = new Map<string, ComfyLink>();
  for (const node of nodes) {
 ...
 * Collect graph links.
 */
export function collectGraphLinks(graph: unknown): ComfyLink[] {
  if (!isRecord(graph)) {
    return [];
  }
  ...
  return dedupeLinks(collectLinksFromSlots(graph as ComfyGraph)).filter(Boolean);
}

/**
 * Build link index.
 */
export function buildLinkIndex(graph: unknown): LinkIndex {
  const outgoing = new Map<string, ComfyLink[]>();
  const incoming = new Map<string, ComfyLink[]>();
