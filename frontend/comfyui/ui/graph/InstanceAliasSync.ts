...
interface WatchedGroup extends ComfyGroup {
  __sugarcubes_title_watch?: boolean;
}

type GroupTitleChangeHandler = (group: WatchedGroup, next: string, previous: string) => void;

interface GroupTitleState {
  current: string;
  mute: boolean;
  onChange: GroupTitleChangeHandler | null;
}

interface AliasAllocationOptions {
  currentInstanceId?: string;
  currentGroup?: ComfyGroup | null;
  ignoreGroup?: (group: ComfyGroup, metadata: CubeGroupMetadataRecord) => boolean;
}
...
interface AliasSyncOptions {
  ...
  requestDirtyRefresh?:
    | ((options: { graph?: ComfyGraph | null; reason: string }) => unknown)
    | null;
}
...
const groupState = new WeakMap<WatchedGroup, GroupTitleState>();
...
function ensureState(group: WatchedGroup | null | undefined): GroupTitleState | null {
  if (!group) {
    return null;
  }
 ...
 */
export function ensureGroupTitleWatcher(
  group: WatchedGroup | null | undefined,
  onChange: GroupTitleChangeHandler | null,
): void {
 ...
 * Set group title.
 */
export function setGroupTitle(group: WatchedGroup | null | undefined, value: string): void {
  if (!group) {
    return;
  }
 ...
 */
export function allocateUniqueInstanceAlias(
  ...
  const currentInstanceId =
    typeof options.currentInstanceId === 'string' ? options.currentInstanceId.trim() : '';
 ...
 */
export function findGroupForCubeId(
  graph: ComfyGraph | null | undefined,
 ...
 */
export function findGroupForMarkerId(
  graph: ComfyGraph | null | undefined,
  markerId: GraphId | null | undefined,
): ComfyGroup | null {
 ...
 */
export function findGroupForNodeId(
  graph: ComfyGraph | null | undefined,
 ...
 */
export function syncInstanceAlias({
  graph,
  group,
  metadata,
  cubeId,
  ...
  events,
  requestDirtyRefresh,
}: AliasSyncOptions = {}): AliasSyncResult {
