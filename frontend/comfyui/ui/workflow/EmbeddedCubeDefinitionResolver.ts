...
export interface ResolvedEmbeddedCubeDefinition {
  cubeId: string;
  cubeVersion: string;
  revisionRef: string;
...
export class EmbeddedCubeDefinitionResolver {
  ...
  resolve(workflow: unknown): ResolvedEmbeddedCubeDefinition[] {
    ...
    const byDefinitionId = new Map<string, ResolvedEmbeddedCubeDefinition>();
    for (const [index, candidate] of definitions.entries()) {
      if (!isRecord(candidate) || !isRecord(candidate.extra)) continue;
      if (!CUBE_KINDS.has(String(candidate.extra.sugarcubes_kind))) continue;
      ...
      const identity = requireRecord(
        candidate.extra.sugarcubes_cube,
        `${path}.extra.sugarcubes_cube`,
      );
      ...
      try {
        ...
        throw new TypeError(
          `Workflow value '${path}.extra.sugarcubes_cube.cube_id' must be a canonical Cube id.`,
          {
            cause: error,
          },
        );
      }
      const cubeVersion = requireString(
        identity.cube_version,
        `${path}.extra.sugarcubes_cube.cube_version`,
      );
...
function requireNoPersistedCubeInstances(
  nodes: unknown[],
  definitions: ReadonlyMap<string, ResolvedEmbeddedCubeDefinition>,
): void {
  const instanceIds = new Set<string>();
  for (const [index, candidate] of nodes.entries()) {
    if (!isRecord(candidate) || !isRecord(candidate.properties)) continue;
    if (!CUBE_KINDS.has(String(candidate.properties.sugarcubes_kind))) continue;
    const path = `nodes[${String(index)}]`;
    const definitionId = requireString(candidate.type, `${path}.type`);
    const definition = definitions.get(definitionId);
    if (!definition) {
      throw new TypeError(
        `Cube node at '${path}' is missing embedded definition '${definitionId}'.`,
      );
    }
    const identity = requireRecord(
      candidate.properties.sugarcubes_cube,
      `${path}.properties.sugarcubes_cube`,
    );
    const instanceId = requireString(
      identity.instance_id,
      `${path}.properties.sugarcubes_cube.instance_id`,
    );
...
function requireRecord(value: unknown, path: string): UnknownRecord {
  if (!isRecord(value)) throw new TypeError(`Workflow value '${path}' must be an object.`);
  return value;
}
...
function requireString(value: unknown, path: string): string {
  ...
  if (!normalized) throw new TypeError(`Workflow value '${path}' must be a non-empty string.`);
  return normalized;
}
