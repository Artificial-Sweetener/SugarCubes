...
import {
  CubeWorkflowLibraryState,
  type WorkflowCubeClassification,
...
interface WorkflowLibraryDialogs {
  confirm(options: {
    title: string;
    message: string[];
    confirmLabel: string;
...
interface WorkflowLibraryFeedback {
  push(severity: string, summary: string, detail: string): void;
}

interface WorkflowLibraryHost {
  getApp?(): ComfyApplication | null;
}

interface WorkflowLibraryActionOptions {
  api: WorkflowLibraryApi;
  state: CubeWorkflowLibraryState;
  host: WorkflowLibraryHost;
  dialogs: WorkflowLibraryDialogs;
  feedback: WorkflowLibraryFeedback;
}
...
export class CubeWorkflowLibraryActions {
  ...
  readonly #host: WorkflowLibraryHost;
  readonly #dialogs: WorkflowLibraryDialogs;
  readonly #feedback: WorkflowLibraryFeedback;

  /** Bind API, workflow snapshot, approval, and feedback ports. */
  constructor(options: WorkflowLibraryActionOptions) {
  ...
  classification(instanceId: string): WorkflowCubeClassification | null {
    return this.#state.read(instanceId);
  }

  /** Acknowledge that the embedded workflow copy remains authoritative. */
  keep(instanceId: string): void {
    const classification = this.#requireClassification(instanceId);
    this.#feedback.push(
      'info',
      'Cube kept in workflow',
      `${classification.cubeId} remains embedded and runnable without a library match.`,
    );
  }
  ...
  async syncSource(instanceId: string): Promise<void> {
    await this.#run('Unable to synchronize Cube source', async () => {
      ...
      if (!approved) return;
      const workflow = await this.#workflowSnapshot();
      const result = await this.#api.syncWorkflowCubeSource(
        JSON.stringify({
          workflow,
      ...
      this.#feedback.push(
        status === 'exact' ? 'success' : 'warn',
        'Cube source synchronized',
        `Source comparison: ${status}. The workflow copy was not replaced.`,
      );
    });
  }

  /** Fork exact embedded content into a separately writable personal Cube. */
  async forkToLocal(instanceId: string): Promise<void> {
    await this.#run('Unable to fork Cube', async () => {
      ...
      if (!newCubeId) return;
      const workflow = await this.#workflowSnapshot();
      const result = await this.#api.forkWorkflowCube(
        JSON.stringify({
          workflow,
          definition_id: classification.definitionId,
          expected_semantic_hash: classification.semanticHash,
      ...
      this.#feedback.push(
        'success',
        'Cube fork created',
        `${newCubeId} is writable in Local Cubes; the workflow source remains unchanged.`,
      );
    });
  }

  /** Resolve one current classification and require the requested operation. */
  #requireClassification(instanceId: string, operation?: string): WorkflowCubeClassification {
    const classification = this.#state.read(instanceId);
    if (!classification) throw new Error('Cube library status is not available yet.');
  ...
  async #workflowSnapshot(): Promise<UnknownRecord> {
    const app = this.#host.getApp?.();
    const graph = isRecord(app?.graph) ? (app.graph as ComfyGraph) : null;
    const result = await app?.graphToPrompt?.();
    const record = isRecord(result) ? result : {};
    if (!isRecord(record.workflow)) throw new Error('Current workflow payload is unavailable.');
  ...
  async #run(summary: string, operation: () => Promise<void>): Promise<void> {
    try {
      await operation();
    } catch (error: unknown) {
...
function requestBody(workflow: UnknownRecord, classification: WorkflowCubeClassification): string {
...
function requireSuccess(result: ApiJsonResult, operation: string): void {
  if (result.response.ok && !isRecord(result.data.error)) return;
...
function defaultForkCubeId(cubeId: string): string {
  const filename =
    cubeId
      .split('/')
      .at(-1)
...
function validateLocalCubeId(value: string): string | null {
  try {
...
function readResultString(value: UnknownRecord, key: string): string {
