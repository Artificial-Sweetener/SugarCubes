...
export interface CreatePersonalCubeModalOptions {
  candidate?: PersonalCubeCandidate | null;
  deriveIdentity?: ((name: string) => PersonalCubeIdentity | null) | null;
}
...
export class CreatePersonalCubeModal {
  ...
  open({
    candidate,
    deriveIdentity,
  }: CreatePersonalCubeModalOptions = {}): Promise<PersonalCubeIdentity | null> {
    ...
    const update = () => {
      ...
      try {
        ...
        this.shell.setError(error instanceof Error ? error.message : 'Name is invalid.');
      }
    };
    ...
    update();
    return result as Promise<PersonalCubeIdentity | null>;
  }

  buildPreviewRow(label: string, valueNode: Node): HTMLElement {
  ...
  buildWarnings(values: unknown): HTMLElement {
