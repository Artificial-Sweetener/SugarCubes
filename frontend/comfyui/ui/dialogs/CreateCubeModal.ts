...
export interface CubeDestination {
  key: string;
  sourceKind?: string;
  namespace?: string;
  owner?: string;
...
export interface CreateCubeModalCandidate {
  cubeId?: unknown;
  ...
  markerIds?: readonly unknown[];
  warnings?: readonly unknown[];
}

export interface CreateCubeModalResult {
  defaultAlias: string;
  cubeName: string;
  cubeId: string;
  targetModel: string;
...
export interface CreateCubeModalOptions {
  candidate?: CreateCubeModalCandidate | null;
  deriveCubeId?: (
    defaultAlias: string,
    destination: CubeDestination,
    targetModel: string,
  ) => string;
  destinations?: CubeDestination[];
  onCreateDestination?: (() => Promise<CubeDestination | null>) | null;
}
...
function splitCubeId(cubeId: unknown): { cubeId: string; filename: string; source: string } {
...
function defaultDestination(): CubeDestination {
  return {
    key: 'local/personal',
    sourceKind: 'local',
    namespace: 'personal',
...
export class CreateCubeModal {
  ...
  private readonly navigator: CreateModalGraphNavigator;

  constructor({ adapter = null }: { adapter?: ModalAdapter | null } = {}) {
  ...
  open({
  ...
  }: CreateCubeModalOptions = {}): Promise<CreateCubeModalResult | null> {
    const initialName =
      normalizeDefaultAliasTitle(normalizeString(candidate?.defaultAlias).split('/').pop()) ||
      'SugarCube';
    ...
    let selectedDestination =
      destinationEntries.find(
        (entry) => !entry?.action && entry?.key === candidate?.destinationKey,
      ) ||
      destinationEntries.find((entry) => !entry?.action) ||
      defaultDestination();
    const form = $el('form.sugarcubes-modal__form.sugarcubes-create-cube__form') as HTMLFormElement;
    ...
    const targetModelSelect = $el(
      'select.p-inputtext.p-component.sugarcubes-modal__text-input.sugarcubes-create-cube__target-model-select',
    ) as HTMLSelectElement;
    const customTargetModelInput = $el(
      'input.p-inputtext.p-component.sugarcubes-modal__text-input.sugarcubes-create-cube__custom-target-model',
      {
        type: 'text',
        placeholder: 'Enter target model',
      },
    ) as HTMLInputElement;
    const supportedModelsInput = $el(
      'input.p-inputtext.p-component.sugarcubes-modal__text-input.sugarcubes-create-cube__supported-models',
      {
        type: 'text',
    ...
    const destinationSelect = $el(
      'select.p-inputtext.p-component.sugarcubes-modal__text-input.sugarcubes-create-cube__destination-select',
    ) as HTMLSelectElement;
    ...
    const customTargetModelField = this.buildTextField(
      'Custom target model',
      customTargetModelInput,
      {
    ...
    form.append(
      nameField,
      targetModelField,
      customTargetModelField,
      supportedModelsField,
      destinationField,
      preview,
      descriptionField,
      warningList,
    ...
    const updateDerivedValues = () => {
      ...
      const { cubeId, filename, source } = splitCubeId(
        deriveId(defaultAlias, selectedDestination, targetModel),
      );
    ...
    const selectDestination = (destination: CubeDestination | undefined): void => {
    ...
    const formatDestinationOption = (destination: CubeDestination): string => {
      const label = normalizeString(destination?.label) || 'Cube pack';
      const detail = normalizeString(destination?.detail);
      return detail ? `${label} (${detail})` : label;
    };
    const renderDestinationOptions = (): void => {
      const options = destinationEntries.map((destination) =>
        $el('option', {
    ...
    const syncCustomTargetModelField = (): void => {
    ...
    const handleDestinationChange = async (): Promise<void> => {
      const destination = destinationEntries.find(
        (entry) => entry?.key === destinationSelect.value,
      );
      if (destination?.action === 'create-pack') {
        ...
        try {
          const created =
            typeof onCreateDestination === 'function' ? await onCreateDestination() : null;
          if (created) {
            if (!destinationEntries.some((entry) => entry?.key === created.key)) {
              const actionIndex = destinationEntries.findIndex(
                (entry) => entry?.action === destination.action,
              );
          ...
          selectedDestination = previous;
          this.shell.setError(
            error instanceof Error ? error.message : 'Failed to create cube pack.',
          );
          renderDestinationOptions();
          updateDerivedValues();
        } finally {
    ...
    return result.finally(() => this.navigator.detach()) as Promise<CreateCubeModalResult | null>;
  }
  ...
  buildPreviewRow(label: string, valueEl: HTMLElement): HTMLElement {
  ...
  buildWarningList(warnings: readonly unknown[] | null | undefined): HTMLElement {
