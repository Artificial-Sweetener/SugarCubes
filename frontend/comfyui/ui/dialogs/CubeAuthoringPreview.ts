...
import type { CubeAuthoringCandidate } from '../create/CubeAuthoringDialog.js';
import type { PersonalCubeIdentity } from '../create/PersonalCubeIdentity.js';

interface PreviewUpdate {
  destination: string;
  identity: PersonalCubeIdentity | null;
  name: string;
  targetModel: string;
}
...
export class CubeAuthoringPreview {
  ...
  readonly #defaultAlias: HTMLElement;
  readonly #identity: HTMLElement;
  readonly #filename: HTMLElement;
  readonly #targetModel: HTMLElement;
  readonly #destination: HTMLElement;

  constructor(documentRef: Document, candidate: CubeAuthoringCandidate | null | undefined) {
    ...
    this.element.append(
      previewRow(documentRef, 'Default alias', this.#defaultAlias),
      previewRow(documentRef, 'Cube ID', this.#identity),
      previewRow(documentRef, 'Filename', this.#filename),
      previewRow(documentRef, 'Target model', this.#targetModel),
      previewRow(documentRef, 'Destination', this.#destination),
    );
  ...
  update({ destination, identity, name, targetModel }: PreviewUpdate): void {
...
function previewRow(documentRef: Document, label: string, value: Node): HTMLElement {
...
function previewValue(documentRef: Document, tagName: 'code' | 'span' = 'span'): HTMLElement {
...
function selectionSummary(candidate: CubeAuthoringCandidate | null | undefined): string {
  if (!candidate?.nodeIds && !candidate?.markerIds) return '';
...
function filename(cubeId: string | undefined): string {
