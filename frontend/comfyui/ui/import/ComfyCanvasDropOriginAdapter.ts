...
import type { ComfyCanvas } from '../types/graph.js';

export interface CubePlacementCanvas extends ComfyCanvas {
  convertCanvasToOffset?(point: Vec2): unknown;
  ds?: {
    scale?: number;
    offset?: number[];
    animateToBounds?(bounds: [number, number, number, number], redraw: () => void): void;
    fitToBounds?(bounds: [number, number, number, number]): void;
  };
  last_mouse_position?: number[];
}

export interface ComfyCanvasDropOriginAdapterOptions {
  getCanvas(): CubePlacementCanvas | null;
  logger: Pick<Console, 'warn'>;
}

/** Own fallback ordering for pointer, viewport-center, and transform origins. */
export class ComfyCanvasDropOriginAdapter {
  readonly #getCanvas: () => CubePlacementCanvas | null;
  readonly #logger: Pick<Console, 'warn'>;

  /** Bind the dynamic canvas lookup used across workflow replacements. */
  constructor(options: ComfyCanvasDropOriginAdapterOptions) {
    ...
    try {
      if (typeof canvas.convertCanvasToOffset === 'function') {
        ...
        if (Array.isArray(converted) && converted.length >= 2) {
          const value: Vec2 = [Number(converted[0]), Number(converted[1])];
