...
export interface CubeImportOutcomeReporterDependencies {
  focusImportedCube(result: ImportResult): void;
  pushToast(severity: ToastSeverity, summary: string, detail: string): void;
}

export interface CubeImportReportOptions {
  focus?: boolean;
}

export interface CubeImportOutcome {
  summary: string;
...
export class CubeImportOutcomeReporter {
  readonly #dependencies: CubeImportOutcomeReporterDependencies;

  /** Bind feedback and viewport collaborators. */
  constructor(dependencies: CubeImportOutcomeReporterDependencies) {
  ...
  report(
    ...
    options: CubeImportReportOptions = {},
  ): CubeImportOutcome {
    const payload = readImportPayload(payloadValue) ?? {};
    const backendMessages = backendWarnings.filter(
    ...
    if (!importResult?.success) {
      this.#dependencies.pushToast(
        'warn',
        `SugarCube ${defaultAlias} import incomplete`,
    ...
    if (options.focus !== false) {
      this.#dependencies.focusImportedCube(importResult as ImportResult);
    }
    return { summary, frontendWarnings };
  }
}
