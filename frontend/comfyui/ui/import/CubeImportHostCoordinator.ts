...
import { CubeHostFeedback } from '../core/CubeHostFeedback.js';
...
import {
  ...
  resolvePreviewRect,
} from '../overlays/PlacementHelpers.js';
...
} from './ComfyCanvasDropOriginAdapter.js';
...
interface CubeImportHostCoordinatorOptions {
  ui: SugarCubesUI;
...
export class CubeImportHostCoordinator {
  readonly #ui: SugarCubesUI;
  readonly #app: ComfyApplication | null;
  readonly #feedback: CubeHostFeedback;
  readonly #getRuntime: () => ComfyCubeRuntime;
  readonly #outcomes: CubeImportOutcomeReporter;
  ...
  constructor(options: CubeImportHostCoordinatorOptions) {
    ...
    this.#commands = new CubeImportCommandService({
      ...
      reportOutcome: (defaultAlias, warnings, result, payload) =>
        this.#outcomes.report(defaultAlias, warnings, result, payload),
    ...
    this.#ui.cubeBrowser.configure({
      actions: {
        computeDropOrigin: () => this.computeDropOrigin(),
        importCubeByName: (cubeId, importOptions) =>
          this.#commands.importCurrent(cubeId, importOptions),
        importCubeRevision: (cubeId, revisionRef, importOptions) =>
          this.#commands.importRevision(cubeId, revisionRef, importOptions),
        ...
        startCubePlacement: (cubeId, placementOptions) =>
          overlayManager.placement.start(cubeId, placementOptions),
      },
      helpers: {
        coerceVec2,
        computePayloadBounds: (entries, context) =>
          computePayloadBounds(entries, context, adapter.getLiteGraph?.()),
        drawGhostRect,
        getPlacementGroupLabel: (defaultAlias, group) =>
          getPlacementGroupLabel(defaultAlias, group, getGroupSugarcubes),
        readVector2,
        resolvePreviewRect: (entry, position, size, context) =>
          resolvePreviewRect(entry, position, size, context, adapter.getLiteGraph?.()),
      },
      placement: {
  ...
  #focusImportedCube(result: ImportResult): void {
    const canvas = this.#ui.adapter.getCanvas() as CubePlacementCanvas | null;
