...
export interface MutableImportBounds {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
export function computeGridPosition(origin: Vec2, index: number): Vec2 {
...
export function resolveOutputSlotIndex(node: WritableImportNode, slotSpec: unknown): number {
  if (typeof slotSpec === 'number' && Number.isFinite(slotSpec)) return slotSpec;
  if (typeof slotSpec === 'string' && slotSpec) {
    const outputs: ComfyOutput[] = node.outputs ?? [];
...
export function resolveInputSlotIndex(node: WritableImportNode, inputName: string): number {
  if (!Array.isArray(node.inputs)) return -1;
  return node.inputs.findIndex((input: ComfyInput) => input.name === inputName);
}

/** Ensure a prepared connection can target a named node input. */
export function ensureInputSlot(node: WritableImportNode, inputName: string): boolean {
  if (resolveInputSlotIndex(node, inputName) !== -1) return true;
  ...
  if (widget && typeof node.convertWidgetToInput === 'function') {
    try {
      if (node.convertWidgetToInput(widget) !== false) return true;
    } catch (_error) {
...
export function applyInputValueToNode(
  node: WritableImportNode,
  inputName: string,
  value: unknown,
): boolean {
  ...
  if (!Object.prototype.hasOwnProperty.call(node.properties, inputName)) return false;
...
export function applyExecutionMode(node: WritableImportNode, value: unknown): void {
  if (typeof value === 'number' && Number.isInteger(value) && value > 0) node.mode = value;
}
