...
try:
    ...
    from backend.services.tracked_repo_service import CubeCommitResult
...
def collect_selected_cube_ids(
    analysis: CubeAnalysis, cube_ids: Sequence[str]
) -> list[str]:
...
class CubeExportService:
    ...
    def _resolve_node_class_mappings(self) -> Mapping[str, Any]:
        ...
        if not isinstance(mappings, Mapping):
            _logger.warning(
                "SugarCubes export received a non-mapping node registry provider result"
            )
            return {}
        return mappings
