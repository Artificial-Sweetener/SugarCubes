...
test('Stable save submits exact classified content and refreshes transient state', async () => {
  ...
  const saveWorkflowCubeToStable = jest.fn(async (body: BodyInit | null, options?: RequestInit) => {
    capturedBody = body;
  ...
  await actions.saveToStable('cube-instance');
...
test('source synchronization performs no network action when approval is declined', async () => {
  ...
  const actions = new CubeWorkflowLibraryActions({
    api: {
      saveWorkflowCubeToStable: jest.fn(async () => successResult()),
      forkWorkflowCube: jest.fn(async () => successResult()),
    ...
    definitions: [
      {
        ...
        permitted_operations: ['keep', 'save_to_stable', 'track_source', 'fork'],
      },
    ],
  });
}
