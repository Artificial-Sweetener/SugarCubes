...
function makeGraph({ groupTitle = 'New Name', markerName = 'Old Name', existingInstanceId = '' } = {}) {
  const graph = {
    _nodes: [],
    _groups: [],
    links: {
...
function makeDuplicateSpawnGraph({ aliases = ['Demo', 'Demo'], importedIndexes = [1] } = {}) {
  const graph = {
    _nodes: [],
    _groups: [],
    links: {},
  ...
  const makeMarker = ({ id, type, pos, instanceId, alias, inputLink = null, outputLinks = [] }) => ({
    id,
    type,
    pos,
    size: [80, 40],
...
describe('InstanceManager group title sync', () => {
  test('uses marker alias and updates group title', () => {
    ...
    const sugarcubes = group.properties.sugarcubes;
  ...
  test('syncs marker alias rename across group and markers', () => {
    ...
    markers[0].widgets.find((widget) => widget.name === 'instance_alias').value = 'Fresh Name';
  ...
  test('keeps existing instance_id when markers disagree', () => {
    ...
    markers[0].widgets.find((widget) => widget.name === 'instance_id').value = 'inst-a';
    markers[1].widgets.find((widget) => widget.name === 'instance_id').value = 'inst-b';
  ...
  test('canonicalizes instance_id when markers disagree and metadata is missing', () => {
    ...
    markers[0].widgets.find((widget) => widget.name === 'instance_id').value = 'b';
    markers[1].widgets.find((widget) => widget.name === 'instance_id').value = 'a';
    const instanceBuilder = new InstanceBuilder({ instanceIdFactory: () => 'inst-new' });
    const manager = new InstanceManager({ adapter: makeAdapter(), instanceBuilder });
  ...
  test('normalizes managed group bounds from content during refresh', () => {
    ...
    group.pos = [-120, -90];
    group.size = [520, 360];
    group.properties.sugarcubes.bounds = {
      x: -120,
      y: -90,
      w: 520,
      h: 360,
  ...
  test('expands tight unmanaged bounds using visual node bounds for header clearance', () => {
    ...
    for (const marker of markers) {
      marker.getBounding = () => [marker.pos[0], marker.pos[1] - 30, marker.size[0], marker.size[1] + 30];
    }
  ...
  test('force refresh normalizes stale bounds even when the instance signature is unchanged', () => {
    ...
    expect(group.pos).toEqual([-10, -60]);
    expect(group.size).toEqual([380, 130]);
    expect(group.properties.sugarcubes.bounds).toMatchObject({ x: -10, y: -60, w: 380, h: 130 });
  });
  ...
  test('scheduled force refresh carries the force flag into the next frame', () => {
    ...
    const manager = new InstanceManager({
      adapter: makeAdapter(),
      scheduler: { raf: (callback) => callback() },
    });
