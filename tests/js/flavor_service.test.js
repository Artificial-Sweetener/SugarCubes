...
function createFlavorApi() {
  ...
  const readState = (cubeId) => ({
    schema_version: 1,
    cube_id: cubeId,
    surfaces: {},
    ...(states.get(cubeId) || {}),
  });
  const writeState = (cubeId, state) => {
    const next = {
      schema_version: 1,
      cube_id: cubeId,
  ...
  return {
    states,
    getLocalFlavors: jest.fn(async (cubeId) => ({
      response: { ok: true },
      data: { state: readState(cubeId) },
    })),
    saveLocalFlavor: jest.fn(async (payload) => {
    ...
    deleteLocalFlavor: jest.fn(async (payload) => {
      const body = JSON.parse(payload);
      const state = readState(body.cube_id);
      const surface = state.surfaces[body.surface_signature] || {
        selected_flavor_id: '',
    ...
    selectLocalFlavor: jest.fn(async (payload) => {
      const body = JSON.parse(payload);
      const state = readState(body.cube_id);
      const surface = state.surfaces[body.surface_signature] || {
        selected_flavor_id: '',
    ...
    migrateLocalFlavors: jest.fn(async (payload) => {
      const body = JSON.parse(payload);
    ...
    reconcileLocalFlavors: jest.fn(async (payload) => {
      const body = JSON.parse(payload);
      const state = readState(body.cube_id);
...
describe('flavor service', () => {
  ...
  test('saveCurrentFaceValuesAsLocalFlavor persists current surface values', async () => {
    ...
    const saved = await service.saveCurrentFaceValuesAsLocalFlavor(
      graph._groups[0].properties.sugarcubes,
    );
    ...
    expect(payload.surfaces['surface-a'].flavors[0].values).toEqual({ 'ksampler.cfg': 7 });
    expect(
      payload.surfaces['surface-a'].flavors[0].values,
    ).not.toHaveProperty('ksampler.seed');
  ...
  test('saveCurrentFaceValuesAsAuthoredFlavor seeds prompt from alias fallbacks', async () => {
    ...
    const api = {
      saveAuthoredFlavor: jest.fn(async () => ({
        response: { ok: true },
    ...
    graph._groups[0].properties.sugarcubes.instance_alias = 'Instance Seed';
    await service.saveCurrentFaceValuesAsAuthoredFlavor(graph._groups[0].properties.sugarcubes);
  ...
  test('saveCurrentFaceValuesAsDefault uses cube defaults copy and payload', async () => {
    ...
    expect(graph._groups[0].properties.sugarcubes.flavor_options).toEqual([
  ...
  test('saveCurrentFaceValuesAsDefault reads unsymbolized managed surface nodes', async () => {
    ...
    const service = new FlavorService({ adapter, api, dirtyManager: { requestRefresh: jest.fn() } });
  ...
  test('selectFlavor ignores local flavor requests in defaults-only mode', async () => {
    ...
    await service.storage.loadCubeState('cube-1');

    await service.selectFlavor({
      metadata: graph._groups[0].properties.sugarcubes,
      flavor: {
        id: 'portrait_local',
        name: 'Portrait',
        scope: 'local',
    ...
    const metadata = graph._groups[0].properties.sugarcubes;
  ...
  test('refreshGroupMetadata clamps stale selected state to authored default', async () => {
    ...
    api.states.set('cube-1', {
      ...
      surfaces: {
        'surface-a': {
          ...
          flavors: [{ id: 'portrait_local', name: 'Portrait Local', values: { 'ksampler.cfg': 9 } }],
        },
      },
    });
    const service = new FlavorService({ adapter, storage, api, dirtyManager });
    ...
    service.refreshGroupMetadata(graph, graph._groups[0], graph._groups[0].properties.sugarcubes);
    ...
    expect(metadata.local_flavors).toEqual([]);
    expect(metadata.flavor_options.map((entry) => [entry.id, entry.scope, entry.selected])).toEqual([
  ...
  test('hydrateFromDefinition only updates groups with the matching definition key', async () => {
    ...
    expect(graph._groups[0].properties.sugarcubes.surface_signature).toBe('surface-old');
    expect(graph._groups[1].properties.sugarcubes.surface_signature).toBe('surface-new');
    expect(graph._groups[1].properties.sugarcubes.active_flavor_values).toEqual({
      'ksampler.cfg': 9,
    });
  });
  ...
  test('manageFlavors deletes the selected local flavor through dialog selection', async () => {
    ...
    const deleted = await service.manageFlavors(graph._groups[0].properties.sugarcubes);
