...
function makeInstance({ instanceId, bounds, markers, ...rest }) {
  return {
    instanceId,
    bounds,
    markers: markers || [],
...
function buildIndex({ instances, markerMap, graph }) {
  return {
    instances,
...
describe('CubeLayoutEngine', () => {
  ...
  test('swapOrder keeps span stable across repeated swaps', () => {
    ...
    const applyPlacements = (entries, placements) => {
  ...
  test('appendAfter places a cube after the last bounds', () => {
    const order = [
      makeInstance({ instanceId: 'A', bounds: { x: 10, y: 5, w: 20, h: 10 } }),
    ];
