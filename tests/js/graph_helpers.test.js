...
import { getNodeCenter, isPointInBounds, readGroupBounds, readNodeBounds } from '../../web/comfyui/ui/graph/Bounds.js';
...
describe('graph helpers', () => {
  ...
  test('read/write widget values handle callbacks safely', () => {
    const widget = { name: 'default_alias', value: 'demo', callback: () => { throw new Error('nope'); } };
  ...
  test('updateMarkersForCubeId updates cube markers only', () => {
    ...
    expect(updateMarkersForCubeId(graph, 'old', { cubeId: 'new', defaultAlias: 'New Name' })).toBe(1);
  ...
  test('resolveInstanceDisplayName prefers custom alias then default alias', () => {
    ...
    expect(resolveInstanceDisplayName({ metadata, group, fallback: 'Fallback' })).toBe('Alias Name');
    expect(
    ...
    expect(resolveInstanceDisplayName({ metadata: { instance_alias: '', default_alias: 'Canonical Name' } })).toBe(
      'Canonical Name',
    );
    expect(resolveInstanceDisplayName({ metadata: { instance_alias: '' }, group, fallback: 'Fallback' })).toBe(
      'Group Title',
    );
    expect(resolveInstanceDisplayName({ metadata: { instance_alias: '' }, fallback: 'Fallback' })).toBe(
      'Fallback',
    );
  });
});
