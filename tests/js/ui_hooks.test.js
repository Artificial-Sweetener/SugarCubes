...
beforeEach(() => {
  app.reset();
  app.graph = { _nodes: [], _groups: [], onNodeAdded: () => {}, onNodeRemoved: () => {}, onNodeConnectionChange: () => {} };
  app.canvas = {
    graph: app.graph,
    setDirty: jest.fn(),
    onAfterChange: () => {},
  ...
  globalThis.LiteGraph = {
    ...
    LGraphNode: function LGraphNode() {},
  ...
  globalThis.LiteGraph.LGraphNode.prototype.collapse = function collapse() {
...
describe('ui hooks and scheduling', () => {
  ...
  test('placement payload shift keeps group bounds relative to shifted origin', async () => {
    await loadUi();
    const ui = (await import('../../web/comfyui/ui/index.js')).getSugarCubesUI();
    const buildShiftedPayload = ui.overlayManager?.placement?.buildShiftedPlacementPayload;

    expect(typeof buildShiftedPayload).toBe('function');
    ...
    const shifted = buildShiftedPayload(payload, [50, -30], [150, 170]);
  ...
  test('overlay hooks wrap drawConnections and drawForeground', async () => {
    const proto = globalThis.LiteGraph.LGraphCanvas.prototype;
  ...
  test('connection hook triggers proximity preview only for cube markers', async () => {
    const setDirty = app.canvas.setDirty;
    await loadUi();
    const extension = app._extensions[0];
    await extension.setup();
    ...
    app.graph.onNodeConnectionChange({ type: 'KSampler' });
    expect(setDirty).not.toHaveBeenCalled();

    app.graph.onNodeConnectionChange({ type: 'SugarCubes.CubeInput' });
  ...
  test('node moved triggers containment before collision and dedupes per raf', async () => {
    ...
    const ui = (await import('../../web/comfyui/ui/index.js')).getSugarCubesUI();
    const enforce = jest.fn(() => ({ instances: new Set(['inst-1']) }));
    const collide = jest.fn(() => ({ moved: true }));
    ui.containmentService.enforceForNodes = enforce;
    ui.collisionService.resolveCollisions = collide;

    app.canvas.onNodeMoved({ id: 1, pos: [0, 0], size: [10, 10] });
    app.canvas.onNodeMoved({ id: 1, pos: [0, 0], size: [10, 10] });
    ...
    expect(enforce.mock.invocationCallOrder[0]).toBeLessThan(
      collide.mock.invocationCallOrder[0],
    );
  });
  ...
  test('node moved uses selectedItems when callback node is wrong', async () => {
    ...
    const ui = (await import('../../web/comfyui/ui/index.js')).getSugarCubesUI();
    const enforce = jest.fn(() => ({ instances: new Set() }));
    ui.containmentService.enforceForNodes = enforce;
    ui.collisionService.resolveCollisions = jest.fn();
    ...
    app.canvas.onNodeMoved(wrongNode);
  ...
  test('expanding a node clamps it after bounds settle', async () => {
    ...
    const node = new globalThis.LiteGraph.LGraphNode();
    node.id = 314;
    ...
    expect(
      enforce.mock.calls.some((args) => Array.isArray(args?.[0]?.nodes) && args[0].nodes[0] === node),
    ).toBe(true);
  });
  ...
  test('group drag commits moved group bounds to metadata', async () => {
    ...
    const ui = (await import('../../web/comfyui/ui/index.js')).getSugarCubesUI();
    const indexStub = { instances: [] };
    ui.containmentService.buildIndex = jest.fn(() => indexStub);
    const collide = jest.fn();
    ui.collisionService.resolveCollisions = collide;
  ...
  test('onAfterChange commits managed group bounds even without selected group state', async () => {
    ...
    const indexStub = { instances: [] };
    ui.containmentService.buildIndex = jest.fn(() => indexStub);
    ui.containmentService.enforceForNodes = jest.fn(() => ({ instances: new Set() }));
    const collide = jest.fn();
    ui.collisionService.resolveCollisions = collide;
