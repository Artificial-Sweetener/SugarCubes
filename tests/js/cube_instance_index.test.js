...
function makeMarker({ id, type, defaultAlias, cubeId, instanceId, alias, pos, size }) {
...
function makeGroup(metadata, { bounding, pos, size } = {}) {
  const group = {
    _bounding: bounding,
    pos,
    size,
...
function buildGraph({ nodes, groups, links }) {
  return {
    _nodes: nodes,
    _groups: groups,
    links: links || [],
