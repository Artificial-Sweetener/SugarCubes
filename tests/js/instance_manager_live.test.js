...
function makeMarker({ id, type, cubeId, defaultAlias, instanceId, alias }) {
  return {
    id,
    type,
    pos: [id * 10, 0],
...
describe('InstanceManager metadata', () => {
  test('preserves instance alias when refreshing an existing managed group', () => {
    ...
    const links = {
      ...
      12: { id: 12, origin_id: node.id, origin_slot: 0, target_id: outputMarker.id, target_slot: 0 },
    };
    ...
    const targetGroup = {
      ...
      properties: {
        sugarcubes: {
          ...
          nodes: [node.id],
          bounds: { x: 0, y: 0, w: 300, h: 240, padding: { x: 10, y: 10, top_extra: 16 }, header: { height: 24 } },
        },
      },
    };
  ...
  test('recomputes stale managed bounds when content is outside group', () => {
    ...
    const links = {
      ...
      22: { id: 22, origin_id: node.id, origin_slot: 0, target_id: outputMarker.id, target_slot: 0 },
    };
    ...
    expect(metadata.bounds.y).toBeLessThanOrEqual(contentMinY);
    expect(metadata.bounds.y + metadata.bounds.h).toBeGreaterThanOrEqual(contentMaxY);
    expect(targetGroup.pos[1]).toBe(metadata.bounds.y);
    expect(targetGroup.size[1]).toBe(metadata.bounds.h);
  });
});
