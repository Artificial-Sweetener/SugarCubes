...
function makeGraph() {
  ...
  sampler.outputs[0].links.push(1);
...
describe('InstanceBuilder', () => {
  ...
  test('ignores marker candidates when cube_id is missing', () => {
    ...
    inputMarker.outputs[0].links.push(2);
  ...
  test('uses instance_id instead of default_alias for identity', () => {
    ...
    sampler.inputs = [{ type: 'IMAGE', link: 2 }];
    inputMarker.outputs[0].links.push(2);
    outputMarker.widgets.find((widget) => widget.name === 'instance_id').value = 'inst-shared';
    inputMarker.widgets.find((widget) => widget.name === 'instance_id').value = 'inst-shared';
    outputMarker.widgets.find((widget) => widget.name === 'default_alias').value = 'Alpha';
    inputMarker.widgets.find((widget) => widget.name === 'default_alias').value = 'Beta';
    const builder = new InstanceBuilder({
  ...
  test('separates markers with different instance_id values into distinct instances', () => {
    ...
    sampler.inputs = [{ type: 'IMAGE', link: 2 }];
    inputMarker.outputs[0].links.push(2);
    outputMarker.widgets.find((widget) => widget.name === 'instance_id').value = 'b';
    inputMarker.widgets.find((widget) => widget.name === 'instance_id').value = 'a';
    const warnings = [];
    const builder = new InstanceBuilder({
  ...
  test('keeps default_alias per component when cube_id is shared', () => {
    ...
    const outputMarker = graph._nodes.find((node) => node.type === 'SugarCubes.CubeOutput');
    const inputMarker = graph._nodes.find((node) => node.type === 'SugarCubes.CubeInput');
    inputMarker.widgets.find((widget) => widget.name === 'default_alias').value = 'Second Cube';
    ...
    expect(outputMarker.widgets.find((widget) => widget.name === 'default_alias').value).toBe(
      'Demo Cube',
    );
    expect(inputMarker.widgets.find((widget) => widget.name === 'default_alias').value).toBe(
      'Second Cube',
    );
  });
