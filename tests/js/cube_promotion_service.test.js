...
describe('cube promotion service', () => {
  ...
  test('rejects managed and imported cubes before opening promotion UI', async () => {
    const packService = { chooseWritablePack: jest.fn() };
    const service = new CubePromotionService({
      packService,
      toast: { push: jest.fn() },
    });
...
describe('cube pack service', () => {
  test('claims an owner and creates the first writable pack in one flow', async () => {
    const openForm = jest
      .fn()
      .mockResolvedValueOnce({ owner: 'ExampleUser' })
      .mockResolvedValueOnce({ repo: 'Example-Cubes' });
    const api = {
      listCubePacks: jest.fn(async () => ({
        response: { ok: true },
        data: { repos: [], identity_policy: { claimed_github_owner: '' } },
      })),
      updateIdentityPolicy: jest.fn(async () => ({
        response: { ok: true },
        data: { claimed_github_owner: 'ExampleUser' },
      })),
      createAuthoringCubePack: jest.fn(async () => ({
        response: { ok: true },
        data: {
          repo: {
            owner: 'ExampleUser',
    ...
    expect(JSON.parse(api.updateIdentityPolicy.mock.calls[0][0])).toEqual({
      claimed_github_owner: 'ExampleUser',
    });
    expect(JSON.parse(api.createAuthoringCubePack.mock.calls[0][0])).toEqual({
      owner: 'ExampleUser',
      repo: 'Example-Cubes',
      enabled: true,
    });
