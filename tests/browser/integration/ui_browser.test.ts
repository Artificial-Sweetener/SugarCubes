...
import type {
  ...
  MockSidebarTab,
} from '../../frontend_core/support/mocks/app.js';
...
let app = hostApp as unknown as (typeof import('../../frontend_core/support/mocks/app.js'))['app'];
let api = hostApi as unknown as (typeof import('../../frontend_core/support/mocks/api.js'))['api'];
...
describe('ui browser behaviors', () => {
  ...
  test('browser header keeps Cube authoring out of the sidebar', async () => {
    const restore = silenceConsole();
    const container = await mountSidebar();
    expect(container.querySelector('.sugarcubes-browser__create-from-selection')).toBeNull();
    restore();
  });
