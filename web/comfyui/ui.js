...
import { getGroupSugarcubes, normalizeGroupInstanceAlias, setGroupSugarcubes, } from './ui/graph/GroupMetadata.js';
import { coerceVec2, readVector2 } from './ui/graph/VectorUtils.js';
import { computePayloadBounds, drawGhostRect, getPlacementGroupLabel, readLayoutFlags, readLayoutStyle, resolvePreviewRect, } from './ui/overlays/PlacementHelpers.js';
import { buildImportSummary, buildShiftedPlacementPayload, collectCubeIdsFromPayload, readImportPayload, } from './ui/import/PlacementPayload.js';
import { CubeImportCommandService } from './ui/import/CubeImportCommandService.js';
import { applyExecutionMode, applyExtrasToNode, applyInputValueToNode, computeGridPosition, ensureInputSlot, resolveInputSlotIndex, resolveOutputSlotIndex, updateBoundsWithNode, } from './ui/import/ImportNodeWriter.js';
...
let registeredExtensionManager = null;
...
function reportImportOutcome(defaultAlias, backendWarnings, importResult, payloadValue, options = {}) {
    ...
    const warningMessages = backendWarnings.filter((warning) => typeof warning === 'string' && Boolean(warning));
...
function buildImportedSubgraphHintLookup(payload) {
    ...
    for (const entry of nodeEntries) {
        ...
        const title = (typeof entry?.layout?.title === 'string' && entry.layout.title.trim()) ||
    ...
    for (const entry of subgraphs) {
        ...
        try {
            ...
            rebindSubgraphWidgetValues(normalized, (classType) => {
                if (!classType)
                    return null;
                return (globalThis.LiteGraph?.createNode?.(classType) ?? null);
            });
...
function remapImportedLayoutIds(layout, { nodeIdMap, markerIdMap } = {}) {
    ...
    return {
        ...
        groups: layout.groups.map((group) => {
            ...
            return {
                ...group,
                sugarcubes: remapImportedGroupMetadata(group.sugarcubes, {
                    ...(nodeIdMap ? { nodeIdMap } : {}),
...
async function applyPreparedImport(payloadValue, options = {}) {
    ...
    try {
        for (const entry of nodeEntries) {
            ...
            const sizeVec = readVector2(layout?.size, Array.isArray(liteNode.size) ? liteNode.size[0] : 140, Array.isArray(liteNode.size) ? liteNode.size[1] : 60);
        ...
        for (const entry of markerEntries) {
            ...
            const layout = entry?.layout || {};
            const [posX, posY] = resolvePosition(layout);
            const sizeVec = readVector2(layout?.size, Array.isArray(markerNode.size) ? markerNode.size[0] : 120, Array.isArray(markerNode.size) ? markerNode.size[1] : 40);
            ...
            const widgetValues = entry?.widget_values && typeof entry.widget_values === 'object' ? entry.widget_values : {};
        ...
        for (const connection of connectionEntries) {
            const fromSymbol = typeof connection?.from?.symbol === 'string' ? connection.from.symbol : null;
            const toSymbol = typeof connection?.to?.symbol === 'string' ? connection.to.symbol : null;
            const inputName = typeof connection?.to?.input === 'string' ? connection.to.input : null;
            ...
            if (!fromNode || !toNode) {
                result.warnings.push(`Skipping connection '${fromSymbol}' -> '${toSymbol}.${inputName}' (node missing).`);
                continue;
            }
            if (!Array.isArray(fromNode.outputs) || fromNode.outputs.length === 0) {
                result.warnings.push(`Skipping connection '${fromSymbol}' -> '${toSymbol}.${inputName}' (no outputs).`);
                continue;
            }
            ...
            if (!ensureInputSlot(toNode, inputName)) {
                result.warnings.push(`Skipping connection '${fromSymbol}' -> '${toSymbol}.${inputName}' (input unavailable).`);
                continue;
            }
            const inputIndex = resolveInputSlotIndex(toNode, inputName);
            if (inputIndex === -1) {
                result.warnings.push(`Skipping connection '${fromSymbol}' -> '${toSymbol}.${inputName}' (input unresolved).`);
                continue;
            }
            try {
                if (!fromNode.connect) {
                ...
                result.warnings.push(`Failed to connect '${fromSymbol}' -> '${toSymbol}.${inputName}': ${message}`);
            }
        }
    }
    catch (error) {
...
function recreateLayoutGroups(layout, context) {
    ...
    const dropOrigin = readVector2(Array.isArray(context.dropOrigin) ? context.dropOrigin : [0, 0], 0, 0);
    ...
    const createGroup = (groupPayload, options = {}) => {
        ...
        const cubeIcon = (isRecord(context.cube?.icon) && context.cube.icon) ||
            (isRecord(cubeMetadata.icon) && cubeMetadata.icon) ||
            null;
        const canonicalDefaultAlias = (typeof context.cube?.default_alias === 'string' && context.cube.default_alias.trim()) ||
            (typeof cubeMetadata.default_alias === 'string' && cubeMetadata.default_alias.trim()) ||
            '';
        const instanceAliasSeed = (typeof context.instanceAlias === 'string' && context.instanceAlias.trim()) ||
        ...
        if (sugarcubesPayload) {
            const next = applyCubeDefinitionIdentity({ ...sugarcubesPayload }, {
            ...
            const cubeFlavorPayload = context.cube
                ? {
                    ...context.cube,
    ...
    const syntheticPayload = {
        ...
        sugarcubes: {
            ...
            default_alias: (typeof contextMetadata.default_alias === 'string' &&
...
function readPreparedCubeIdentity(payload) {
    ...
    return {
        ...
        revisionRef: typeof revision.revision_ref === 'string' && revision.revision_ref.trim()
...
function focusImportedNode(result) {
    if (result.primaryNodeId == null)
        return;
    const graph = appRef?.graph;
    const node = graph?.getNodeById?.(result.primaryNodeId);
    const canvas = adapter.getCanvas();
    if (!node || !canvas?.centerOnNode)
        return;
    try {
