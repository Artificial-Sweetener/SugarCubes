...
export class FormModal {
    ...
    open({ title = 'Form', message = [], fields = [], confirmLabel = 'Save', cancelLabel = 'Cancel', confirmClassName = 'p-button-primary', } = {}) {
        ...
        for (const field of Array.isArray(fields) ? fields : []) {
            ...
            const wrapperClass = field?.type === 'checkbox'
        ...
        const handleConfirm = () => {
            ...
            for (const { field, input, errorEl } of this.fieldElements.values()) {
                errorEl.textContent = '';
                const rawValue = field?.type === 'checkbox' ? Boolean(input?.checked) : String(input?.value || '');
                ...
                const validationError = typeof field?.validate === 'function' ? field.validate(normalized, values) : '';
                if (validationError) {
