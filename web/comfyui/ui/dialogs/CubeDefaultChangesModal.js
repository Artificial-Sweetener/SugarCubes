...
export class CubeDefaultChangesModal {
    ...
    open(reviews) {
        ...
        return this.#shell.open({
            ...
            description: 'The implementation will be updated. Choose whether this save should also replace its current defaults.',
            body,
    ...
    #buildDecisionSection(review, decision, showName) {
...
function createChoice({ label, description, onChange, }) {
...
function buildMoreInfo(reviews) {
...
function buildChangeList(review, showName) {
    const container = $el('section.sugarcubes-default-review__change-list');
    if (showName)
        container.append($el('h3', { textContent: review.displayName }));
    ...
    for (const [section, changes] of bySection) {
        ...
        for (const change of changes)
            group.append(buildChangeRow(change));
...
function decisionAnnotation(change) {
...
function toDecisions(decisions) {
