...
export class CreateModalGraphNavigator {
  ...
  isBackdropEvent(event) {
    ...
    if (typeof this.dialog?.contains === 'function' && this.dialog.contains(target)) {
      return false;
    }
    return target === this.overlay || Boolean(this.overlay.contains?.(target));
  }
  ...
  handleWheel(event) {
    ...
    const nextScale =
      event.deltaY < 0 ? Number(ds.scale) * zoomSpeed : Number(ds.scale) / zoomSpeed;
  ...
  panBy(dx, dy) {
    const canvas = this.resolveCanvas();
    const ds = canvas?.ds;
    if (!ds || !Array.isArray(ds.offset) || !isFiniteNumber(ds.scale) || Number(ds.scale) === 0) {
      return;
    }
    ds.offset[0] += dx / Number(ds.scale);
    ds.offset[1] += dy / Number(ds.scale);
    this.requestRedraw(canvas);
  }

  resolveCanvas() {
    const app = this.adapter?.getApp?.() || this.adapter?.getWindow?.()?.app || null;
    const windowRef =
    ...
    return (
      ...
      globalRef?.LGraphCanvas?.active_canvas ||
      null
    );
  }
