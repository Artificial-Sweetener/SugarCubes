...
export class CreatePersonalCubeModal {
    ...
    open({ candidate, deriveIdentity, } = {}) {
        ...
        const initialName = normalizeDefaultAliasTitle(String(candidate?.defaultAlias || '')
            .split('/')
            .pop()) || 'SugarCube';
    ...
    buildWarnings(values) {
        const warnings = (Array.isArray(values) ? values : []).filter((value) => typeof value === 'string' && value.trim());
