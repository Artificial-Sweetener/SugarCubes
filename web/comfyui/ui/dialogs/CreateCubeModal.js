...
export class CreateCubeModal {
  ...
  open({ candidate, deriveCubeId, destinations = [], onCreateDestination } = {}) {
    const initialName =
      normalizeDefaultAliasTitle(normalizeString(candidate?.defaultAlias).split('/').pop()) ||
      'SugarCube';
    const deriveId = typeof deriveCubeId === 'function' ? deriveCubeId : () => '';
    ...
    const handleDestinationChange = async () => {
      ...
      if (destination?.action === 'create-pack') {
        ...
        try {
          ...
          selectedDestination = previous;
          this.shell.setError(error?.message || 'Failed to create cube pack.');
          renderDestinationOptions();
          updateDerivedValues();
        } finally {
    ...
    const handleConfirm = () => {
      ...
      try {
        ...
        this.shell.setError(error?.message || 'Target model is invalid.');
        return;
      }
      if (!targetModel) {
