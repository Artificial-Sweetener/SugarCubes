...
export class CubePromotionService {
  ...
  canPromote(cube) {
    try {
      ...
      return parsed.sourceKind === 'local' && parsed.namespace === 'personal' && cube?.is_writable;
    } catch (_error) {
      return false;
    }
  }
  ...
  async promote(cube) {
    ...
    try {
      ...
      if (!response?.ok || data?.error) {
        throw new Error(data?.error?.message || response?.statusText || 'Promotion failed');
      }
      const targetCubeId = String(data?.cube?.cube_id || '').trim();
      if (!targetCubeId) {
        throw new Error('Promotion response did not include the managed cube identity.');
      }
      const defaultAlias = String(data?.cube?.default_alias || '').trim();
      ...
      this.toast?.push('error', 'Promotion failed', error?.message || String(error));
      return null;
    }
  }
