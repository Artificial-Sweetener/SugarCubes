...
export function createDefaultCubeSurfaceState() {
    return {
...
export function parseCubeSurfaceState(value) {
    ...
    return {
        ...
        preview: {
            ...
            selectedOutput: typeof preview.selected_output === 'string' && preview.selected_output.trim()
...
export function serializeCubeSurfaceState(state) {
    return {
        schema: SURFACE_SCHEMA,
        node_order: [...state.nodeOrder],
        cards: Object.fromEntries(Object.entries(state.cards).map(([nodeId, card]) => [
            nodeId,
            {
...
function parseCardStates(value) {
    if (!isRecord(value))
        return {};
    const cards = {};
    for (const [nodeId, candidate] of Object.entries(value)) {
        if (!nodeId.trim() || !isRecord(candidate))
            continue;
...
function parseCurrentCardState(candidate) {
    if (typeof candidate.authored_bypass !== 'boolean' ||
...
function migrateLegacyCardState(candidate) {
