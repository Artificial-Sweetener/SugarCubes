...
export class ComfyGraphPreviewImageSource {
    ...
    constructor(documentRef, rootGraph, logger = null) {
        this.#document = documentRef;
        this.#rootGraph = rootGraph;
        this.#logger = logger;
    }
    /** Return Comfy's decoded image for one stable output URL. */
    find(url, sourceLocator) {
        const requested = parseUrl(url, this.#document.baseURI);
        if (!requested)
            return null;
        const images = this.#images().filter(({ image }) => image.naturalWidth > 0 && image.naturalHeight > 0);
        for (const { image } of images) {
            const candidate = parseUrl(image.currentSrc || image.src, this.#document.baseURI);
            if (!candidate)
                continue;
            if (candidate.href === requested.href || isSameComfyView(candidate, requested))
                return image;
        }
        if (sourceLocator) {
    ...
    #images() {
        const images = Array.from(this.#document.images).map((image) => ({
            image,
            locator: null,
        }));
...
function isSameComfyView(candidate, requested) {
    if (candidate.origin !== requested.origin || candidate.pathname !== requested.pathname)
        return false;
    return ['filename', 'subfolder', 'type'].every((key) => candidate.searchParams.get(key) === requested.searchParams.get(key));
}
