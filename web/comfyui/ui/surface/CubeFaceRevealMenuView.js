...
export class CubeFaceRevealMenuView {
    ...
    constructor(documentRef) {
        ...
        if (!windowRef)
            throw new Error('Cube reveal menus require a browser window.');
    ...
    #position() {
        ...
        const top = Math.min(Math.max(VIEWPORT_INSET, rect.bottom + POPUP_GAP), Math.max(VIEWPORT_INSET, this.#window.innerHeight - VIEWPORT_INSET));
        this.#menu.style.top = `${String(top)}px`;
        this.#menu.style.right = `${String(Math.max(VIEWPORT_INSET, this.#window.innerWidth - rect.right))}px`;
        this.#menu.style.maxHeight = `${String(Math.max(0, this.#window.innerHeight - top - VIEWPORT_INSET))}px`;
    }
}
/** Build the only reveal-menu element that remains inside the native header. */
function createMenuButton(documentRef) {
