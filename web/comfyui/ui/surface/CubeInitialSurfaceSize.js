...
import { CUBE_DOM_SURFACE_SECTION_GAP, CUBE_SURFACE_FRAME_PADDING, CUBE_SURFACE_SECTION_GAP, CUBE_VUE_CONTENT_HORIZONTAL_INSET, } from './CubeSurfaceGeometry.js';
...
export const resolveCubeInitialSurfaceSize = (request) => {
    const defaults = createDefaultCubeSurfaceState();
    const state = parseCubeSurfaceState(request.surface);
    const columnWidth = clamp(state.minimumColumnWidth, defaults.minimumColumnWidth, MAXIMUM_INITIAL_COLUMN_WIDTH);
    const masonryGap = clamp(state.gap, 0, MAXIMUM_INITIAL_MASONRY_GAP);
    const masonryWidth = INITIAL_MASONRY_COLUMN_COUNT * columnWidth + (INITIAL_MASONRY_COLUMN_COUNT - 1) * masonryGap;
    ...
    const canvasWidth = canvasLeftInset +
        masonryWidth +
        (previewWidth > 0 ? CUBE_SURFACE_SECTION_GAP + previewWidth : CUBE_SURFACE_FRAME_PADDING);
    const vueWidth = CUBE_VUE_CONTENT_HORIZONTAL_INSET +
