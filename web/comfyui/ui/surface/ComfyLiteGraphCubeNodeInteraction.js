...
export class ComfyLiteGraphCubeNodeInteraction {
    ...
    #handlePointerDown = (event) => {
        ...
        if (event.button === 2) {
            ...
            if (previewItem && this.#previewActions)
                this.#consume(event);
            return;
        }
        if (event.button !== 0)
        ...
        if (titlebarAction) {
            this.#consume(event);
            dispatchCubeFaceTitlebarAction(titlebarAction, buildCubeFaceChromeMetadata(item.node), this.#chromeActions, {
                left: event.clientX,
                top: event.clientY,
                right: event.clientX,
...
function findPreviewItem(item, point) {
    for (const section of layoutCubeCanvasPreviewSections(item.layout.preview, item.preview ?? null)) {
        const previewItem = section.items.find((candidate) => containsCubeCanvasPoint(candidate.rect, point));
        if (previewItem)
            return previewItem;
    }
    return null;
