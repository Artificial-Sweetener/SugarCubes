...
export class ComfyCanvasViewStateAdapter {
    ...
    constructor(canvas, setDirtyCanvas, logger = null) {
        this.#canvas = canvas;
    ...
    capture() {
        ...
        this.#logger?.debug(`SugarCubes captured the surface viewport at scale ${String(scale)} and offset ${String(x)},${String(y)}.`);
    ...
    restore(state) {
        ...
        this.#logger?.debug(`SugarCubes restored the surface viewport at scale ${String(state.scale)} and offset ${String(state.offset[0])},${String(state.offset[1])}.`);
    }
}
/** Narrow Comfy's Array or typed-array offset to its mutable numeric surface. */
function readMutableOffset(value) {
    if (!isRecord(value))
        return null;
