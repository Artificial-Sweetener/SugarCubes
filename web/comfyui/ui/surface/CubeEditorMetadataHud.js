...
import { CubeEditorMetadataDraftStore, } from './CubeEditorMetadataDraftStore.js';
...
export class CubeEditorMetadataHud {
    ...
    constructor(documentRef, actions, options = {}) {
    ...
    show(node) {
        ...
        this.#render();
        void this.#actions.canEdit(node).then((editable) => {
            if (requestId !== this.#requestId || this.#activeNode !== node)
                return;
    ...
    focus() {
        if (!this.#activeNode || !this.#values)
            return;
        this.#collapsed = false;
        this.#render();
        this.#root
            ?.querySelector(`[${FOCUS_KEY_ATTRIBUTE}="default-alias"]`)
            ?.focus({ preventScroll: true });
    }
    /** Route the active editor save shortcut through the visible metadata owner. */
    requestSave() {
        const button = this.#root?.querySelector('.sugarcubes-cube-editor-metadata__save');
    ...
    setDescription(description) {
        if (!this.#activeNode || !this.#values || !this.#editable)
            return false;
    ...
    #render() {
        ...
        if (this.#editable)
            titlebar.append(this.#createSaveButton());
        titlebar.append(this.#createCollapseButton());
        root.replaceChildren(titlebar);
        if (!this.#collapsed)
            root.append(this.#createBody(node, values));
    ...
    #createBody(node, values) {
        const body = this.#document.createElement('div');
        body.className = 'sugarcubes-cube-editor-metadata__body';
        if (this.#editable) {
            body.append(this.#createField('Name', this.#createTextInput('defaultAlias', values.defaultAlias)), this.#createField('Target model', this.#createTargetModelEditor(values.targetModel)), this.#createField('Supported models', this.#createSupportedModelsEditor(values.supportedModels)), this.#createField('Description', this.#createDescriptionInput(values.description)));
            if (isDraftCubeNode(node)) {
                body.append(this.#createField('Save to', this.#createDestinationSelect(values.destination)));
            }
            if (this.#saveError)
                body.append(this.#createSaveError(this.#saveError));
            return body;
        }
        body.append(this.#createReadOnlyField('Target model', values.targetModel || 'Not specified'), this.#createReadOnlyField('Supported models', values.supportedModels.length ? values.supportedModels.join(', ') : 'Not specified'), this.#createReadOnlyField('Description', values.description || 'No description'), this.#createReadOnlyField('Access', 'Read only'));
        return body;
    }
    /** Present a save failure beside the controls that can resolve it. */
    #createSaveError(message) {
    ...
    #createSupportedModelsEditor(models) {
        const control = new ComfySettingsAutocompleteControl(this.#document, this.#settingsSelectRenderer, {
    ...
    #createTargetModelEditor(value) {
        ...
        const control = new ComfySettingsSingleSelectControl(this.#document, this.#settingsSelectRenderer, {
            ...
            onChange: (selected) => {
                ...
                if (customSelected) {
                    queueMicrotask(() => {
                        this.#root
                            ?.querySelector(`[${FOCUS_KEY_ATTRIBUTE}="customTargetModel"]`)
                            ?.focus();
                    });
                }
            },
    ...
    #createDestinationSelect(value) {
        const control = new ComfySettingsSingleSelectControl(this.#document, this.#settingsSelectRenderer, {
            ...
            value,
            onChange: (selected) => this.#updateValue('destination', selected === 'pack' ? 'pack' : 'local'),
        });
        this.#settingsSelectControls.add(control);
        return control.element;
    }
    ...
    #refreshEditedState() {
        ...
        if (!root || !values)
            return;
        const title = root.querySelector('.sugarcubes-cube-editor-metadata__titlebar strong');
        if (title)
            title.textContent = deriveDefaultAlias(values);
        const save = root.querySelector('.sugarcubes-cube-editor-metadata__save');
        if (save)
            save.disabled = false;
    ...
    #createSaveButton() {
        ...
        button.addEventListener('click', () => {
            ...
            void this.#actions.save(node, normalizeValues(values)).then((outcome) => {
    ...
    #createCollapseButton() {
        ...
        button.title = label;
        button.append(createComfyPrimeIconElement(this.#document, this.#collapsed ? 'chevron-down' : 'chevron-up'));
    ...
    #releaseMountedControls() {
        for (const control of this.#settingsSelectControls)
            control.dispose();
        this.#settingsSelectControls.clear();
    }
    /** Retain dirty authoring state before navigation removes the viewport card. */
    #retainActiveDraft() {
        if (!this.#activeNode || !this.#values || !this.#dirty)
            return;
...
function captureFocusedControl(documentRef, root) {
...
function restoreFocusedControl(root, focused) {
    if (!focused)
        return;
    const control = root.querySelector(`[${FOCUS_KEY_ATTRIBUTE}="${focused.key}"]`);
    ...
    control.setSelectionRange(Math.min(focused.selectionStart, maximum), Math.min(focused.selectionEnd, maximum));
}
...
function readValues(node) {
    ...
    return {
        ...
        targetModel,
        supportedModels: supportedModels.length || !isDraftCubeNode(node) ? supportedModels : [targetModel],
...
function deriveAuthoringName(defaultAlias) {
...
function deriveDefaultAlias(values) {
...
function readDestination(cubeId) {
