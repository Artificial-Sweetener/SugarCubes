 ...
 */
export function resolveCubeExternalInterface(node) {
    const graph = node.subgraph;
    const draft = node.properties?.sugarcubes_kind === 'cube_draft' ||
        node.properties?.sugarcubes_cube?.kind === 'draft' ||
        graph.extra?.sugarcubes_kind === 'cube_draft' ||
        graph.extra?.sugarcubes_cube?.kind === 'draft';
    return {
        inputSlots: resolveConnectedSlots(node.inputs ?? [], readBoundarySlots(graph.inputNode), draft, (slot) => node.resolveSubgraphInputLinks?.(slot)?.length ?? null),
        outputSlots: resolveConnectedSlots(node.outputs ?? [], readBoundarySlots(graph.outputNode), draft, (slot) => typeof node.resolveSubgraphOutputLink === 'function'
...
function readBoundarySlots(value) {
...
function resolveConnectedSlots(ports, boundaries, hideUninspectableDraftPorts, readNativeConnectionCount) {
    ...
    if (!boundaries)
        return hideUninspectableDraftPorts ? [] : ports.map((_, index) => index);
...
function hasConnections(boundary) {
    if (!boundary)
        return false;
