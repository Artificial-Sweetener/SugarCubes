...
import { parseCubeSurfaceState, serializeCubeSurfaceState, } from '../../surface/CubeSurfaceState.js';
import { requireCubeSurface } from '../node/ComfyCubeNodeFactory.js';
/** Apply source live values and presentation state only where stable identities still match. */
export function transferCubeVersionState(source, target) {
    ...
    for (const [symbol, sourceNode] of sourceBySymbol) {
        ...
        if (typeof sourceNode.mode === 'number')
...
function transferNodeFields(source, target) {
...
function transferSurfaceState(source, target, sourceBySymbol, targetBySymbol) {
    ...
    const sourceSymbolById = new Map([...sourceBySymbol].map(([symbol, node]) => [String(node.id), symbol]));
    const targetIdBySymbol = new Map([...targetBySymbol].map(([symbol, node]) => [symbol, String(node.id)]));
    ...
    for (const targetId of targetState.nodeOrder) {
        if (!nodeOrder.includes(targetId))
    ...
    return {
        ...
        preview: {
            ...sourceState.preview,
            selectedOutput: sourceState.preview.selectedOutput && outputNames.has(sourceState.preview.selectedOutput)
...
function indexNodesBySymbol(node) {
    ...
    for (const internal of node.subgraph._nodes) {
        ...
        if (indexed.has(symbol))
            throw new Error(`Cube node symbol '${symbol}' is duplicated.`);
