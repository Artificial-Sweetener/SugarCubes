...
export function readCubeClipboardViolation(value, targetIsRoot) {
    if (!isRecord(value))
        return null;
    const definitions = Array.isArray(value.subgraphs) ? value.subgraphs : [];
    const cubeDefinitionIds = new Set();
    for (const definition of definitions) {
        if (!isRecord(definition) || !isCubeKind(readExtraKind(definition)))
            continue;
        const id = readString(definition.id);
        if (id)
            cubeDefinitionIds.add(id);
    }
...
function isSerializedCubeNode(value, cubeDefinitionIds) {
    if (!isRecord(value))
        return false;
