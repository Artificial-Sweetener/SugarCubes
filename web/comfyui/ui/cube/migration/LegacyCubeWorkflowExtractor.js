...
export class LegacyCubeWorkflowExtractor {
    ...
    extractInPlace(workflow) {
        ...
        const connections = this.#collectExternalConnections(links, removedIds, consumedLinkIds, inputMarkers, outputMarkers, warnings);
        const extractedGroups = new Set(managed.map((group) => group.source));
        for (const inner of extractedInnerGroups)
            extractedGroups.add(inner);
    ...
    #readManagedGroup(group, nodeById, warnings, fallbackKey) {
        ...
        const key = readString(metadata.instance_id) || (groupId ? `legacy-group-${groupId}` : fallbackKey);
        const title = readString(metadata.alias) ||
        ...
        for (const id of memberIds) {
            ...
            if (node)
                ownedNodes.push(cloneRecord(node));
            else
                warnings.push(`Legacy Cube '${title}' references missing node '${id}'.`);
        }
        return {
            source: group,
            memberIds,
    ...
    #assignInnerGroups(managed, groups) {
        const extracted = new Set();
        for (const candidate of groups) {
            if (isRecord(candidate.sugarcubes))
                continue;
    ...
    #collectTopology(group, links, nodeById, consumedLinkIds, warnings) {
        ...
        for (const markerId of group.inputMarkerIds) {
            ...
            if (targets.length === 0) {
                warnings.push(`Legacy Cube '${group.plan.title}' input '${readMarkerName(marker, markerId)}' has no target.`);
                continue;
            }
        ...
        for (const markerId of group.outputMarkerIds) {
            ...
            if (!source) {
                warnings.push(`Legacy Cube '${group.plan.title}' output '${readMarkerName(marker, markerId)}' has no source.`);
                continue;
            }
            if (sources.length > 1) {
                warnings.push(`Legacy Cube '${group.plan.title}' output '${readMarkerName(marker, markerId)}' had multiple sources; the first was preserved.`);
            }
    ...
    #collectExternalConnections(links, removedIds, consumedLinkIds, inputMarkers, outputMarkers, warnings) {
        const connections = [];
        for (const link of links) {
            if (consumedLinkIds.has(readIdKey(link.id)))
                continue;
            const originId = readIdKey(link.originNodeId);
            const targetId = readIdKey(link.targetNodeId);
            if (!removedIds.has(originId) && !removedIds.has(targetId))
                continue;
            ...
            if (origin && target) {
                ...
                warnings.push(`Legacy link '${String(link.id)}' bypasses Cube boundary markers and could not be migrated.`);
            }
        }
        return connections;
    }
...
function parseLegacyLink(value) {
...
function readGraphId(value, fallback) {
    return isGraphId(value) ? value : fallback;
}
/** Normalize one graph identity for set membership. */
function readIdKey(value) {
    return isGraphId(value) ? String(value) : '';
}
/** Read a set of serialized graph identities. */
function readIdSet(value) {
    return new Set((Array.isArray(value) ? value : []).map(readIdKey).filter((entry) => entry.length > 0));
}
...
function readMarkerName(marker, fallback) {
