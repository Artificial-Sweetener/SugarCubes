...
export class LegacyCubeGraphBuilder {
    ...
    build(plan) {
        ...
        if (subgraph._nodes.length !== plan.nodes.length) {
            throw new Error(`Comfy restored ${subgraph._nodes.length} of ${plan.nodes.length} internal nodes.`);
        }
        ...
        for (const node of subgraph._nodes) {
            ...
            if (symbol)
                ...
                warnings.push(`Migrated Cube '${plan.title}' contains node '${String(node.id)}' without a stable symbol.`);
        }
