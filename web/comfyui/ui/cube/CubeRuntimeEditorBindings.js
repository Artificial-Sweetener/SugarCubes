...
export function buildCubeRuntimeEditorBindings(options, openEditor) {
    return {
        openEditor,
        ...(options.editorMetadata ? { canEdit: options.editorMetadata.canEdit } : {}),
        onEditDenied: () => options.feedback?.push?.('info', 'Cube definition is read-only', 'Fork this Cube before editing its definition.'),
    };
}
