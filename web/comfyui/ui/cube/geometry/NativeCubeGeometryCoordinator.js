...
export class NativeCubeGeometryCoordinator {
    ...
    #scheduleVerification(context, generation, delayIndex) {
    ...
    #reflow(context, renderer, document) {
        ...
        if (!nodesChanged && !boundariesChanged)
            return;
    ...
    #readCurrentContext() {
        ...
        if (!cube || !readNativeCubeAuthoredLayout(cube.subgraph))
            return null;
        const nodes = new Map();
        for (const node of cube.subgraph._nodes) {
            const symbol = isRecord(node.properties) && typeof node.properties.sugarcubes_symbol === 'string'
