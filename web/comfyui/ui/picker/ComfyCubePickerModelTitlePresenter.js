...
export class ComfyCubePickerModelTitlePresenter {
    ...
    present(result, definition) {
        ...
        if (!presentation.usesModelPill)
            return;
        const markedTitle = result.querySelector(`[${PRESENTED_MODEL_TYPE_ATTRIBUTE}]`);
...
function findUniqueTitleLeaf(result, displayName) {
    const matches = [...result.querySelectorAll('span, div')].filter((candidate) => candidate.children.length === 0 && candidate.textContent?.trim() === displayName);
