...
export class ComfyCubePickerPackPresenter {
    ...
    present(result, definition) {
        ...
        const markedCategory = result.querySelector(`[${PRESENTED_PACK_TYPE_ATTRIBUTE}]`);
        const category = markedCategory ??
            [...result.querySelectorAll('span')].find((span) => span.textContent?.trim() === expectedCategory);
        if (!category)
            return;
