...
export class ComfyCubeAddMenuPresenter {
    ...
    #renderResults(query) {
        const results = this.#menu?.querySelector('.sugarcubes-add-cube-menu__results');
        ...
        const rows = candidates.map((candidate) => this.#createCandidateRow(candidate));
        if (rows.length === 0)
            rows.push(this.#createEmptyRow(normalized));
    ...
    #createCandidateRow(candidate) {
        ...
        row.append(createCubeModelTitleElement(this.#document, presentation, {
    ...
    #position() {
        ...
        const left = Math.max(4, Math.min(this.#anchor.right - this.#menu.offsetWidth, viewportWidth - this.#menu.offsetWidth - 4));
        const below = this.#anchor.bottom + 4;
        const top = below + this.#menu.offsetHeight <= viewportHeight - 4
            ? below
