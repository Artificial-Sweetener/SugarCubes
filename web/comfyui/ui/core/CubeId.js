...
function validateCubePathSegment(value) {
 ...
 */
export function parseCanonicalCubeId(cubeId) {
    ...
    if (separator <= 0) {
        throw new Error('Cube id must use canonical owner/repo/path/to/cube.cube or local/namespace/path/to/cube.cube format.');
    }
 ...
 */
export function isCanonicalCubeId(cubeId) {
    try {
 ...
 */
export function normalizeDefaultAliasTitle(value) {
    ...
    return parts
        .map((part, index) => normalizeTitleToken(part, {
        index,
 ...
 */
export function suggestCanonicalCubePath(defaultAlias) {
 ...
 */
export function deriveCubeIdFromDefaultAlias(cubeId, defaultAlias) {
 ...
 */
export function deriveSourceAuthorLabel(cubeId, { localLabel = 'local' } = {}) {
    const parsed = parseCanonicalCubeId(cubeId);
 ...
 */
export function suggestLocalCubeId(defaultAlias, namespace = 'personal') {
...
function parseGithubCubeId(remainder) {
...
function parseLocalCubeId(remainder) {
...
function normalizeCanonicalCubePath(relativePath) {
    const cleaned = typeof relativePath === 'string'
        ? relativePath
            .trim()
            .replace(/\\/g, '/')
            .replace(/^\/+|\/+$/g, '')
...
function normalizeTitleToken(token, { index, isLast }) {
...
function hasMixedCasingBeyondFirstCharacter(word) {
