...
function fallbackStyle(model) {
  const source = model?.fallback && typeof model.fallback === 'object' ? model.fallback : {};
  ...
  return {
    ...
    inset: Number.isFinite(source.inset) ? source.inset : FALLBACK_ICON_STYLE.inset,
    renderSize:
...
export function drawFallbackInitialsCanvas(ctx, model, x, y, size, options = {}) {
  ...
  ctx.translate(-layout.bounds.x, -layout.bounds.y);
