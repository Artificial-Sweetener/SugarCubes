...
function normalizeGraphState(entry, nodes, links, groups, reroutes) {
  const rawState = entry?.state && typeof entry.state === 'object' ? entry.state : {};
  return {
    lastNodeId: Math.max(
  ...
  if (Array.isArray(rawBounding) && rawBounding.length === 4) {
    return rawBounding.map((value, index) =>
      index < 2
        ...
        : Math.max(0, coerceNumber(value, DEFAULT_IO_BOUNDS[index])),
    );
  }
...
function normalizeLinkEntries(rawLinks) {
  ...
  return rawLinks.map((link, index) => normalizeLinkEntry(link, index)).filter(Boolean);
}
...
function buildLegacySubgraphIo(groupedLinks, nodes, subgraphId, kind, expectedNames = []) {
  ...
  for (const [slotIndex, slotLinks] of groupedLinks.entries()) {
    ...
    ioEntries.push({
      ...
      name,
      label,
      ...(readOptionalString(slotMeta?.localized_name)
        ? { localized_name: readOptionalString(slotMeta.localized_name) }
        : {}),
