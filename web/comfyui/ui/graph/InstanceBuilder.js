...
function computeCubeSubgraph(cubeMarkers, graphNodes, outgoing, incoming, markerIds) {
    const nodeById = new Map(graphNodes.filter(Boolean).map((node) => [String(node.id), node]));
    const isMarkerId = (nodeId) => markerIds.has(String(nodeId));
    ...
    const expanded = new Set(combined);
...
function buildMarkerMetadataLookup(graph) {
    ...
    for (const group of getGraphGroups(graph)) {
        ...
        const markerIds = Object.values(metadata.markers).flatMap((value) => Array.isArray(value) ? value : []);
...
export class InstanceBuilder {
    logger;
    instanceIdFactory;
    constructor({ logger, instanceIdFactory } = {}) {
    ...
    build(graph) {
        ...
        for (const node of nodes) {
            ...
            const metadataInstanceId = metadata?.managed && typeof metadata.instance_id === 'string'
                ? metadata.instance_id.trim()
                : '';
            const effectiveInstanceId = metadataInstanceId || markerInstanceId;
            const cubeVersion = normalizeCubeVersion(readCubeMarkerVersion(node)) ||
                normalizeCubeVersion(metadata?.cube_version);
            const metadataRevisionRef = typeof metadata?.cube_revision_ref === 'string' ? metadata.cube_revision_ref : '';
            const cubeRevisionRef = readCubeMarkerRevisionRef(node) || metadataRevisionRef;
            const metadataDefinitionKey = typeof metadata?.cube_definition_key === 'string' ? metadata.cube_definition_key : '';
            const cubeDefinitionKey = metadataDefinitionKey || buildCubeDefinitionKey(cubeId, cubeVersion);
        ...
        for (const cubeMarkers of cubeMap.values()) {
            ...
            for (const startId of componentNodes) {
                ...
                const definitionMetadata = componentMarkers
                    .map((marker) => marker.node?.id != null ? markerMetadataById.get(String(marker.node.id)) : null)
                    .find((metadata) => metadata?.managed) || null;
                const cubeVersion = componentMarkers.find((marker) => marker.cubeVersion)?.cubeVersion || '';
                const cubeRevisionRef = componentMarkers.find((marker) => marker.cubeRevisionRef)?.cubeRevisionRef || '';
                const cubeDefinitionKey = componentMarkers.find((marker) => marker.cubeDefinitionKey)?.cubeDefinitionKey ||
                    buildCubeDefinitionKey(cubeId, cubeVersion);
                const icon = componentMarkers.find((marker) => marker.node?.id != null && markerMetadataById.get(String(marker.node.id))?.icon);
                const definitionIcon = icon?.node?.id != null
                ...
                instances.push({
                    ...
                    defaultAlias,
                    instanceAlias,
                    targetModel: typeof definitionMetadata?.target_model === 'string'
