...
export class InstanceBuilder {
  ...
  build(graph) {
    ...
    for (const node of nodes) {
      ...
      const cubeRevisionRef = readCubeMarkerRevisionRef(node) || metadata?.cube_revision_ref || '';
      const cubeDefinitionKey =
        metadata?.cube_definition_key || buildCubeDefinitionKey(cubeId, cubeVersion);
      ...
      const kind = CUBE_MARKER_KINDS[node.type];
      const entry = {
        id: String(node.id),
        kind,
        node,
    ...
    for (const cubeMarkers of cubeMap.values()) {
      ...
      for (const startId of componentNodes) {
        ...
        instances.push({
          ...
          nodes: componentNodeIds.map((id) => nodeById.get(id)).filter(Boolean),
