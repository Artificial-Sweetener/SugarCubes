...
function collectLinksFromContainer(container) {
  ...
  if (typeof container.values === 'function') {
    try {
      ...
      // ignore iterator failures and continue with object fallback
    }
  }
  if (!links.length && typeof container.forEach === 'function') {
    try {
      container.forEach((value) => append(value));
    } catch (_error) {
      // ignore callback failures and continue with object fallback
    }
  }
    ...
    for (let slotIndex = 0; slotIndex < node.outputs.length; slotIndex += 1) {
      ...
      for (const linkId of outputLinks) {
        ...
        if (Array.isArray(targets) && targets.length) {
          ...
          existing.target_slot = target.target_slot;
