...
function hasWritableVec2(value) {
  return (Array.isArray(value) || ArrayBuffer.isView(value)) && (value?.length || 0) >= 2;
}

function hasWritableBounds(value) {
  return (Array.isArray(value) || ArrayBuffer.isView(value)) && (value?.length || 0) >= 4;
}
...
function normalizePadding(padding) {
  const padX = Number(padding?.x);
  const padY = Number(padding?.y);
  const topExtra = Number(padding?.top_extra);
  const resolved = {
...
function resolvePaddingHeader(bounds, metadata) {
  const paddingSource = bounds?.padding ?? metadata?.bounds?.padding ?? metadata?.padding ?? null;
  const headerSource = bounds?.header ?? metadata?.bounds?.header ?? metadata?.header ?? null;
 ...
 */
export function inflateInstanceBounds(bounds, padding) {
  if (!bounds) {
    return null;
  }
  ...
  const header = resolveHeaderHeight(padding?.header);
  return {
    x: bounds.minX - padX,
...
export function writeCanonicalBounds({ group, metadata, bounds } = {}) {
  ...
  const targetMetadata =
    metadata && typeof metadata === 'object' ? metadata : group?.properties?.sugarcubes;
  ...
  if (group) {
    ...
    if (targetMetadata) {
      ...
      group.properties.sugarcubes.bounds = payload;
    }
