...
/**
 * Own the SugarCubes graph integration layer in `frontend/comfyui/ui/graph/DirtyTracker.js`.
 */
...
import { computeCosmeticHash, computeImplementationHash, computeSurfaceHash, computeSurfaceValuesHash, } from './DirtyHasher.js';
...
export class DirtyTracker {
    ...
    evaluator;
    stateApplier;
    warnedMissingSymbols;
    constructor({ logger = null, baselineStore = null, baselineResolver, evaluator = null, stateApplier = null, }) {
    ...
    resolveSurfaceBaselineHash(metadata) {
        const surface = isRecord(metadata.surface) ? metadata.surface : null;
    ...
    refresh({ graph, knownCubeIds } = {}) {
        ...
        const knownSet = knownCubeIds instanceof Set
            ? knownCubeIds
        ...
        for (const group of groups) {
            ...
            const implementationCurrentHash = computeImplementationHash(graph, nodeIds, markerIds, anchor, readSurface(metadata.surface), useDefinition
                ? {
                    idLookup,
                    useSymbols: true,
                    useInputNames: true,
            ...
            const cosmeticCurrentHash = computeCosmeticHash(graph, nodeIds, markerIds, anchor, localGroup || group);
            ...
            if (evaluation.implementationReasons.includes('missing-symbols') &&
                this.logger &&
    ...
    markClean({ graph, cubeIds } = {}) {
        ...
        for (const group of groups) {
            ...
            const nodeById = new Map(getGraphNodes(graph)
                ...
                .map((node) => [String(node.id), node]));
            const anchor = resolveInstanceAnchorFromNodes(nodeById, ids);
            const implementationCurrentHash = computeImplementationHash(graph, nodeIds, markerIds, anchor, readSurface(metadata.surface), {
                useSymbols: false,
                useInputNames: false,
            });
...
function resolveDefinitionKey(metadata) {
    const existing = typeof metadata?.cube_definition_key === 'string' ? metadata.cube_definition_key.trim() : '';
    if (existing) {
        return existing;
    }
