...
export interface SaveSummaryEntry {
  committed?: boolean;
...
export class SugarCubeExportError extends Error {
  ...
  constructor(message: string, detail = '', violations?: unknown) {
    super(message);
    this.detail = detail;
    this.violations = violations;
  }

  static from(error: unknown): SugarCubeExportError {
    if (!error) {
...
export function formatSaveSummaryEntry(entry: SaveSummaryEntry | null | undefined): string {
...
export function buildErrorDetail(errorPayload: unknown): string {
...
export function formatEndpoint(endpoint: unknown): string {
  if (!isRecord(endpoint)) {
    return '<unknown>';
  }
  const parts: string[] = [];
  if (endpoint.title) parts.push(String(endpoint.title));
  if (endpoint.cube) parts.push(`[${String(endpoint.cube)}]`);
