...
export class CubeSaveReconciler {
  constructor({ definitionStore, instanceManager, flavorService, dirtyManager } = {}) {
  ...
  async reconcile({
    ...
    if (!graph) {
      throw new Error('Unable to reconcile saved cubes without a graph');
    }
  ...
  publishDefinitions(graph, saved, markerIdsByCubeId) {
    ...
    for (const savedEntry of Array.isArray(saved) ? saved : []) {
      ...
      const definitionCube = definition?.cube;
      ...
      if (!cubeId || !definitionCube) {
        continue;
      }
      const definitionKey = buildCubeDefinitionKey(cubeId, cubeVersion);
      const markerIds = Array.isArray(markerIdsByCubeId?.[cubeId]) ? markerIdsByCubeId[cubeId] : [];
      if (!markerIds.length) {
