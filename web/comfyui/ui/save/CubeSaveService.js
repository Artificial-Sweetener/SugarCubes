...
import { ANY_TARGET_MODEL, defaultSupportedModelsForTarget, deriveRouteFromCubeId, deriveTargetModelFromCubeId, normalizeSupportedModels, normalizeTargetModel, } from '../core/ModelTargets.js';
import { isCubeMarkerType, updateMarkersForCubeId, updateMarkersForIds, } from '../graph/CubeMarkers.js';
...
import { CURRENT_REVISION_REF, buildCubeDefinitionKey, isCurrentRevisionRef, normalizeCubeVersion, normalizeRevisionRef, } from '../core/CubeDefinitionKey.js';
...
export class CubeSaveService {
    ...
    async saveImplementation({ cubeIds = null, button = null, } = {}) {
        ...
        try {
            ...
            for (const cubeId of finalCubeIds) {
                ...
                const forkedId = suggestPersonalCubeIdentity(forkedName, this.resolvePersonalTargetModel(entry.target_model, sourceMetadata.targetModel, cubeId), reservedIds).cubeId;
                ...
                const updatedCubeNodes = this.cubeNodeSave?.updateIdentities(cubeNodeInstanceIds, {
                    cubeId: forkedId,
                    defaultAlias: forkedName,
                }) ?? 0;
            ...
            const workflowSupported = isRecord(workflowPayload) && !Array.isArray(workflowPayload);
            ...
            const savedIds = saved
                ...
                .filter(Boolean);
            if (!savedIds.length) {
                this.pushToastMessage('info', 'No changes', 'The save operation did not persist any SugarCubes.');
                return { status: 'no_changes', savedCubeIds: [] };
            }
    ...
    buildInstanceSaveMetadataIndex(graph) {
        ...
        const addEntry = (entry) => {
            ...
            if (dedupeKey.trim() &&
                list.some((existing) => `${existing.instanceId || ''}|${existing.sourceDefinitionKey || ''}|${existing.markerIds?.join(',') || ''}` ===
                    dedupeKey)) {
                return;
            }
            list.push(entry);
        ...
        for (const instance of this.cubeNodeSave?.listInstances() ?? []) {
            ...
            addEntry({
                ...
                sourceVersion,
                sourceRevisionRef,
                sourceDefinitionKey: instance.cubeDefinitionKey || buildCubeDefinitionKey(instance.cubeId, sourceVersion),
        ...
        for (const group of getGraphGroups(graph)) {
            ...
            const sourceDefinitionKey = typeof metadata.cube_definition_key === 'string' && metadata.cube_definition_key.trim()
                ? metadata.cube_definition_key.trim()
                : buildCubeDefinitionKey(cubeId, sourceVersion);
            addEntry({
                cubeId,
                defaultAlias: typeof metadata.default_alias === 'string' && metadata.default_alias.trim()
                ...
                sourceDefinitionKey,
                staleRevision: !isCurrentRevisionRef(sourceRevisionRef),
                targetModel: typeof metadata.target_model === 'string' && metadata.target_model.trim()
                ...
                group,
                instanceId: typeof metadata.instance_id === 'string' && metadata.instance_id.trim()
        ...
        const builder = this.instanceManager?.instanceBuilder ||
            new InstanceBuilder({ logger: this.adapter.getConsole?.() ?? null });
    ...
    extractMetadataMarkerIds(metadata) {
        ...
        return Object.values(rawMarkers).flatMap((value) => Array.isArray(value) ? value.filter((entry) => entry != null) : []);
    }
    resolveSavePlanSourceMetadata(cubeId, sourceMetadataIndex) {
        const entries = sourceMetadataIndex.get(cubeId) ?? [];
    ...
    buildSaveEntryMetadata({ cubeId, browserEntry = null, sourceMetadata = null, }) {
        ...
        const targetModel = normalizeTargetModel(browserEntry?.target_model) ||
        ...
        return {
            ...
            ...(sourceMetadata?.selectedSourceEntry?.surfaceSize
                ? { surface_size: [...sourceMetadata.selectedSourceEntry.surfaceSize] }
                : {}),
    ...
    resolvePersonalTargetModel(...candidates) {
    ...
    async forkStaleSaveEntries({ graph, savePlan, cubeIndex, usedNames, }) {
        for (const entry of savePlan) {
            ...
            usedNames.add(forkedName);
            const reservedIds = [...cubeIndex.keys(), ...savePlan.map((candidate) => candidate.cubeId)];
            const forkedId = suggestPersonalCubeIdentity(forkedName, this.resolvePersonalTargetModel(entry.targetModel, browserEntry.target_model, entry.cubeId), reservedIds).cubeId;
    ...
    buildCubeNodeSaveReconciliationTargets(savePlan) {
        const targets = {};
        for (const entry of savePlan) {
            if (!entry.cubeId)
                continue;
    ...
    resolveCubeIds(cubeIds) {
        ...
        const implementationDirty = typeof this.dirtyManager?.getImplementationDirtyCubeIds === 'function'
    ...
    async ensureCubeIds(graph) {
        ...
        const reservedCubeIds = new Set((this.cubeBrowser?.getCubes?.() || [])
        ...
        for (const instance of instances) {
            ...
            const cubeId = suggestPersonalCubeIdentity(defaultAlias, this.resolvePersonalTargetModel(instance.targetModel, instance.cubeId), Array.from(reservedCubeIds)).cubeId;
        ...
        for (const entry of fallbackByName.values()) {
            ...
            const cubeId = shouldReplace
                ? suggestPersonalCubeIdentity(entry.defaultAlias, this.resolvePersonalTargetModel(entry.cubeId), Array.from(reservedCubeIds)).cubeId
                : entry.cubeId;
            if (shouldReplace) {
            ...
            if (!cubeId) {
                const personalId = suggestPersonalCubeIdentity(entry.defaultAlias, this.resolvePersonalTargetModel(entry.cubeId), Array.from(reservedCubeIds)).cubeId;
...
function readInstanceSupportedModels(instance) {
    const value = instance.supportedModels;
