...
interface ChromeItem extends UnknownRecord {
  key: string;
...
interface ChromeCanvas extends ComfyCanvas {
  graph?: ComfyGraph;
  editor_alpha?: number;
...
interface GroupDrawPrototype extends ComfyGroup {
  draw?: GroupDraw;
  __sugarcubes_title_icon_renderer?: CubeChromeOverlay;
...
const CHROME_BUTTONS: readonly ChromeButtonDefinition[] = Object.freeze([
  {
    key: 'swap-left',
    style: 'action',
    tooltip: 'Swap left',
...
function resolveGroupTitleColor(group: ComfyGroup): string {
...
function resolveGroupFontFamily(): string {
...
function resolveGroupTitlePadding(): number {
  const liteGraph = typeof globalThis !== 'undefined' ? globalThis.LiteGraph : null;
...
function parseColorToRgb(color: unknown): Rgb | null {
...
function triadicColor(baseColor: string, offset: number): string {
...
function sumPillWidths(entries: readonly PillEntry[], gap: number): number {
  if (!entries.length) {
    return 0;
  }
...
function buildStackedBadgeLayout(
  ctx: CanvasRenderingContext2D,
  nameText: unknown,
  authorText: unknown,
  maxWidth: number,
 ...
 */
function computeCenteredBadgeSlot({
  groupX,
  groupWidth,
  inset,
  titlebarLeftWidth,
 ...
 */
function removeUnpinnedPillEntry(entries: PillEntry[], pinnedKey: string): boolean {
...
function hasSwapEligibility(metadata: ChromeMetadata): boolean {
...
export class CubeChromeOverlay {
  ...
  private badgeRegions: BadgeRegion[];
  private hoveredKey: string | null;
  private hoveredInstance: string | null;
  private hoveredBadgeInstance: string | null;
  private hoveredBadgeKey: string | null;
  private readonly instanceState: Map<string, ChromeInstanceState>;
  ...
  drawManagedGroupWithTitleIcon(
    ...
    this.drawGroupTitleIcon(ctx as CanvasRenderingContext2D, group, canvasInstance as ChromeCanvas);
    return undefined;
  }
  ...
  renderHeader(
    ...
    if (stackedBadgeLayout.visible) {
      ...
      const badgeX = clampNumber(
        unclampedBadgeX,
        centeredBadgeSlot.left,
        centeredBadgeSlot.right - stackedBadgeLayout.width,
      );
  ...
  convertEventToCanvasPoint(
    ...
    const relative: Vec2 = [event.clientX - rect.left, event.clientY - rect.top];
