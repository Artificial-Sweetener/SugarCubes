...
export function resolveCubeEntrySource(entry) {
    if (!isRecord(entry)) {
        return null;
    }
    const source = isRecord(entry.source) ? entry.source : {};
    const structuredGithub = buildGithubSource(entry.owner, entry.repo) || buildGithubSource(source.owner, source.repo);
    if (structuredGithub) {
