...
export class ProximityLinkRenderer {
    ...
    render(matches, context, canvas) {
        ...
        for (const match of matches) {
            ...
            if ((outputNode && outputNode.graph !== canvas.graph) ||
            ...
            const outputPosition = outputNode
                ? (this.#positionSource?.resolveGraphPosition(outputNode, 'output', outputSlotIndex, outputFallback) ?? outputFallback)
                : outputFallback;
            const inputPosition = inputNode
                ? (this.#positionSource?.resolveGraphPosition(inputNode, 'input', inputSlotIndex, inputFallback) ?? inputFallback)
                : inputFallback;
        ...
        if (signature === this.#lastReportedRenderSignature)
            return;
        this.#lastReportedRenderSignature = signature;
        this.#logger.debug(`SugarCubes painted ${String(renderedCount)} of ${String(matches.length)} proximity matches` +
    ...
    #drawNativeLink(options) {
        const renderLink = typeof options.canvas.renderLink === 'function'
        ...
        try {
            ...
            renderLink(options.context, [options.start[0], options.start[1]], [options.end[0], options.end[1]], fakeLink, false, false, null, resolveSlotDirection(options.outputSlot, true, liteGraph), resolveSlotDirection(options.inputSlot, false, liteGraph), { disabled: false });
            return true;
        }
        catch (error) {
...
export function computeDashPattern(connectionWidth, scale) {
...
function resolveSlotDirection(slot, isOutput, liteGraph) {
    if (slot?.dir !== undefined && slot.dir !== null)
        return slot.dir;
...
function resolveLinkColor(canvas, liteGraph, slotType, logger) {
