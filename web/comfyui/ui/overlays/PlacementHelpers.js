...
export function readLayoutFlags(layout) {
  ...
  const extraFlags = layout.extra?.flags;
  if (extraFlags && typeof extraFlags === 'object') {
    return extraFlags;
  }
  return null;
}
...
export function readLayoutStyle(layout) {
  ...
  const extraStyle = layout.extra?.style;
 ...
 */
export function computePayloadBounds(entries, ctx, liteGraph) {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
