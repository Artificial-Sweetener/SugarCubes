...
export class CubeChromeOverlay {
  constructor({ adapter, actions, resolveSource } = {}) {
  ...
  buildMenuOptions({ metadata, isDirty: _isDirty, flavors: _flavors }) {
    return [
      {
  ...
  installGroupTitleIconRenderer() {
    ...
    if (!GroupRef?.prototype || typeof GroupRef.prototype.draw !== 'function') {
      return;
    }
    GroupRef.prototype.__sugarcubes_title_icon_renderer = this;
    if (GroupRef.prototype.__sugarcubes_title_icon_patched) {
      return;
    }
    const originalDraw = GroupRef.prototype.draw;
    GroupRef.prototype.__sugarcubes_title_icon_original_draw = originalDraw;
    GroupRef.prototype.__sugarcubes_title_icon_patched = true;
    GroupRef.prototype.draw = function drawSugarCubesGroupTitleIcon(...args) {
      const renderer = GroupRef.prototype.__sugarcubes_title_icon_renderer;
  ...
  releaseGroupTitleIconRenderer() {
    const liteGraph =
      ...
      (typeof globalThis !== 'undefined' ? globalThis.LiteGraph : null);
    const GroupRef = liteGraph?.LGraphGroup;
    if (GroupRef?.prototype?.__sugarcubes_title_icon_renderer === this) {
      GroupRef.prototype.__sugarcubes_title_icon_renderer = null;
    }
  }
  ...
  drawManagedGroupWithTitleIcon(group, originalDraw, args) {
    ...
    this.drawGroupTitleIcon(ctx, group, args[0]);
    return undefined;
  }
  ...
  renderHeader(ctx, bounds, metadata, group) {
    ...
    let pillEntries = pillLayout.entries;
      ...
      if (item.icon) {
        ...
        const labelSize =
          Number(item.labelScale) && Number.isFinite(item.labelScale)
            ? Math.max(10, Math.round(fontSize * item.labelScale))
            : fontSize;
        ctx.save();
  ...
  drawIcon(ctx, icon, x, y, size, color, options = {}) {
    ...
    ctx.globalAlpha = Math.max(0, Math.min(1, alpha));
    if (icon === 'pen') {
      this.drawPenIcon(ctx, x, y, size, color);
      ctx.restore();
      return;
    }
    if (icon === 'cube') {
      this.drawCubeIcon(ctx, x, y, size, color);
      ctx.restore();
      return;
    }
    ctx.restore();
