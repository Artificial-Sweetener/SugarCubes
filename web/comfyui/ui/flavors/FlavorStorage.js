...
function normalizeState(cubeId, state) {
  const normalizedCubeId = normalizeCubeId(state?.cube_id || cubeId);
  return {
...
function readResponseError(response, data, fallback) {
  return data?.error?.message || response?.statusText || fallback;
}
...
export class FlavorStorage {
  ...
  readSurfaceState(cubeId, surfaceSignature) {
    ...
    const surfaceState = state.surfaces?.[key];
    return {
      selected_flavor_id:
        typeof surfaceState?.selected_flavor_id === 'string' ? surfaceState.selected_flavor_id : '',
      flavors: Array.isArray(surfaceState?.flavors) ? surfaceState.flavors : [],
    };
  }
  ...
  async reconcileLocalFlavors({ cubeId, surfaceSignature, authoredFlavors, renameMap } = {}) {
    ...
    const { response, data } = await this.api.reconcileLocalFlavors(
      JSON.stringify({
        ...
        surface_signature: signature,
        authored_flavors: Array.isArray(authoredFlavors) ? authoredFlavors : [],
        rename_map: renameMap && typeof renameMap === 'object' ? renameMap : {},
      }),
      { headers: { 'Content-Type': 'application/json' } },
    );
    if (!response?.ok || data?.error) {
  ...
  async migrateBrowserStorage() {
    ...
    const storage = this.storage.storage || null;
