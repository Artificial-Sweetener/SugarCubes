...
import { asFlavorMetadata, cloneValue, findLocalFlavorCollisions, flavorKeySets, normalizeAuthoredFlavorEntries, normalizeFlavorNameKey, normalizeLocalFlavorEntries, normalizeSurface, } from './FlavorSupport.js';
import { normalizeFlavorId } from './FlavorSelection.js';
...
export class FlavorDefinitionLifecycle {
    ...
    async hydrate({ cubeId, definitionKey, entry, graph, forceApply = false, } = {}) {
        const entryRecord = isRecord(entry) ? entry : {};
        const payload = isRecord(entryRecord.payload) ? entryRecord.payload : {};
        const cube = isRecord(payload.cube) ? payload.cube : null;
        if (!graph || !cubeId || !cube)
            return 0;
        ...
        for (const group of getGraphGroups(graph)) {
            ...
            const groupKey = metadata.cube_definition_key?.trim() ||
                buildCubeDefinitionKey(metadata.cube_id, metadata.cube_version);
            if (hasVersion && groupKey && targetKey && groupKey !== targetKey)
                continue;
            ...
            setGroupSugarcubes(group, flattenCubeGroupMetadata(preset, {
                ...metadata,
    ...
    refreshGroup(graph, group, metadata, { forceApply = false } = {}) {
        const authored = normalizeAuthoredFlavorEntries(metadata.authored_flavors || metadata.flavors, metadata.surface);
        ...
        const next = flattenCubeGroupMetadata(writeCubePresetMetadata(metadata, {
            flavor: 'default',
            flavor_scope: 'authored',
    ...
    async reconcileLocalFlavors(graph, group, metadata) {
        const authored = normalizeAuthoredFlavorEntries(metadata.authored_flavors || metadata.flavors, metadata.surface);
        if (!metadata.cube_id || !metadata.surface_signature || !authored.length)
            return;
        const local = this.options.storage.readSurfaceState(metadata.cube_id, metadata.surface_signature);
        ...
        const nextLocal = normalizeLocalFlavorEntries(this.options.storage.readSurfaceState(metadata.cube_id, metadata.surface_signature).flavors, current.surface || metadata.surface);
        setGroupSugarcubes(group, { ...current, local_flavors: nextLocal });
        this.options.toast?.push?.('warn', 'Local flavors renamed', `${result.conflict_count} local flavor name conflicted with authored flavors.`);
        this.options.dirtyManager?.requestRefresh?.({ graph, reason: 'local-flavor-reconcile' });
    }
    async promptCollisionRenames({ authoredFlavors = [], localFlavors = [], } = {}) {
        const collisions = findLocalFlavorCollisions(localFlavors, authoredFlavors);
        if (!collisions.length || !this.options.dialogs?.promptText)
            return {};
        const authoredKeys = flavorKeySets(authoredFlavors);
        const renameMap = {};
        const renamedNames = new Set();
        for (const flavor of collisions) {
            const replacement = (await this.options.dialogs.promptText({
