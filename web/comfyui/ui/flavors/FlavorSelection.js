...
function cloneValues(values) {
    const cloned = JSON.parse(JSON.stringify(isRecord(values) ? values : {}));
 ...
 */
export function buildFlavorOptions({ authoredFlavors, localFlavors, selectedFlavorId, selectedFlavorScope, } = {}) {
...
export function resolveSelectedFlavor(options, selectedFlavorId, selectedFlavorScope) {
    ...
    return (entries.find((entry) => entry?.id === selectedFlavorId && entry?.scope === (selectedFlavorScope || entry?.scope)) ||
