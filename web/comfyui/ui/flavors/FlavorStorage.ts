...
import type { ApiJsonResult, ApiResultResponse } from '../core/CubeLibraryApi.js';
import type { UnknownRecord } from '../types/common.js';

export interface LocalFlavor extends UnknownRecord {
  id?: unknown;
  name?: unknown;
  values?: unknown;
}

export interface LocalFlavorSurfaceState extends UnknownRecord {
  selected_flavor_id: string;
  flavors: LocalFlavor[];
}

export interface LocalFlavorCubeState extends UnknownRecord {
  schema_version: number;
  cube_id: string;
  surfaces: Record<string, LocalFlavorSurfaceState>;
  updated_at: string;
}

interface FlavorStorageApi {
...
interface FlavorStorageOptions {
  storage?: BrowserStorageAdapter | null;
  api?: FlavorStorageApi | null;
}

interface LocalFlavorReference {
  cubeId?: unknown;
  surfaceSignature?: unknown;
  flavorId?: unknown;
}

interface SaveLocalFlavorRequest extends LocalFlavorReference {
  name?: unknown;
  values?: unknown;
  authoredFlavors?: readonly unknown[];
}

interface ReconcileLocalFlavorsRequest extends LocalFlavorReference {
  authoredFlavors?: readonly unknown[];
  renameMap?: unknown;
}
...
function cloneValue(value: unknown): UnknownRecord {
  return JSON.parse(JSON.stringify(isRecord(value) ? value : {})) as UnknownRecord;
}

function normalizeCubeId(cubeId: unknown): string {
  return String(cubeId || '').trim();
}

function normalizeState(cubeId: unknown, state: unknown): LocalFlavorCubeState {
...
function normalizeSurfaces(value: unknown): Record<string, LocalFlavorSurfaceState> {
  if (!isRecord(value)) return {};
  return Object.fromEntries(
    Object.entries(value).map(([signature, surfaceValue]) => {
      const surface: UnknownRecord = isRecord(surfaceValue) ? surfaceValue : {};
      return [
        signature,
        {
          selected_flavor_id:
            typeof surface.selected_flavor_id === 'string' ? surface.selected_flavor_id : '',
          flavors: Array.isArray(surface.flavors)
            ? surface.flavors.filter((entry): entry is LocalFlavor => isRecord(entry))
            : [],
        },
      ];
    }),
...
function readResponseError(
  response: ApiResultResponse | null | undefined,
  data: UnknownRecord,
  fallback: string,
): string {
  const error = isRecord(data.error) ? data.error : {};
  return (
    (typeof error.message === 'string' ? error.message : '') || response?.statusText || fallback
  );
}
...
export class FlavorStorage {
  private readonly storage: BrowserStorageAdapter | null;
  private readonly api: FlavorStorageApi | null;
  private readonly cache: Map<string, LocalFlavorCubeState>;

  constructor({ storage = null, api = null }: FlavorStorageOptions = {}) {
    this.storage = storage;
    this.api = api;
    this.cache = new Map<string, LocalFlavorCubeState>();
  }

  buildStorageKey(cubeId: unknown): string {
    return `${STORAGE_KEY_PREFIX}${normalizeCubeId(cubeId)}`;
  }

  readCubeState(cubeId: unknown): LocalFlavorCubeState {
    const key = normalizeCubeId(cubeId);
    return normalizeState(key, this.cache.get(key));
  }

  writeCubeState(cubeId: unknown, state: unknown): void {
    const key = normalizeCubeId(cubeId);
    this.cache.set(key, normalizeState(key, state));
  }

  async loadCubeState(cubeId: unknown): Promise<LocalFlavorCubeState> {
    const key = normalizeCubeId(cubeId);
    if (!key) {
  ...
  readSurfaceState(cubeId: unknown, surfaceSignature: unknown): LocalFlavorSurfaceState {
  ...
  async saveLocalFlavor({
    ...
    flavorId,
    authoredFlavors = [],
  }: SaveLocalFlavorRequest = {}): Promise<LocalFlavor | null> {
    ...
    return (
      surfaceState.flavors.find((entry) => entry?.id === surfaceState.selected_flavor_id) ||
  ...
  async deleteLocalFlavor({
    ...
    surfaceSignature,
    flavorId,
  }: LocalFlavorReference = {}): Promise<boolean> {
    const key = normalizeCubeId(cubeId);
    const signature = String(surfaceSignature || '').trim();
    ...
    const { response, data } = await this.api.deleteLocalFlavor(
      JSON.stringify({
        cube_id: key,
        surface_signature: signature,
        flavor_id: targetId,
  ...
  async setSelectedFlavorId({
    cubeId,
    surfaceSignature,
    flavorId,
  }: LocalFlavorReference = {}): Promise<void> {
    const key = normalizeCubeId(cubeId);
    const signature = String(surfaceSignature || '').trim();
    if (!key || !signature) {
      return;
    ...
    const { response, data } = await this.api.selectLocalFlavor(
      JSON.stringify({
        cube_id: key,
        surface_signature: signature,
        flavor_id: String(flavorId || '').trim(),
  ...
  async migrateBrowserStorage(): Promise<UnknownRecord> {
    ...
    const states: Array<{ cube_id: string; state: UnknownRecord }> = [];
