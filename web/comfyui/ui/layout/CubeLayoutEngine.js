...
function markerKind(node) {
  return CUBE_MARKER_KINDS[node?.type] || '';
}
...
function collectConnectedComponent(instances, outgoing, incoming, anchorInstanceId) {
  ...
  while (queue.length) {
    ...
    visited.add(current);
    const neighbors = [
      ...(outgoing.get(current) ? Array.from(outgoing.get(current)) : []),
      ...(incoming.get(current) ? Array.from(incoming.get(current)) : []),
    ];
...
export function insertBetween(order, _leftId, rightId, newBounds, gap) {
  ...
  const shift = readNumber(newBounds?.w, 0) + readNumber(gap, 0);
  const moves = new Map();
  if (!target || !shift) {
    return moves;
  }
...
export function replaceCube(order, targetId, newBounds, _gap) {
  ...
  const delta = readNumber(newBounds?.w, targetBounds.w) - targetBounds.w;
  if (!delta) {
    return moves;
  }
  for (const entry of entries) {
