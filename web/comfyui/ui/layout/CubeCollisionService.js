...
export class CubeCollisionService {
  ...
  resolveCollisions({ graph, activeInstanceId, activeInstance, index } = {}) {
    ...
    const active =
      activeInstance ||
      (activeInstanceId ? indexRef?.instanceById?.get?.(String(activeInstanceId)) : null);
    if (!active || !active.bounds) {
      return { moved: false, iterations: 0 };
    }

    const useBuckets = instances.length >= this.bucketThreshold;
    const buckets = useBuckets ? bucketizeInstances(instances, this.bucketSize) : null;
