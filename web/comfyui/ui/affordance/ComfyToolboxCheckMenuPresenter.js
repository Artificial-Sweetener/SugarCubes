...
export class ComfyToolboxCheckMenuPresenter {
    #document;
    #featureMenuAttribute;
    #featureItemAttribute;
    #anchor = null;
    #menu = null;
    #model = null;
    #signature = '';
    ...
    toggle(anchor, model) {
        ...
        menu.querySelector(`[${ITEM_ATTRIBUTE}]`)?.focus();
    }
    ...
    #createRow(item) {
        ...
        row.setAttribute('role', this.#model?.selectionMode === 'single' ? 'menuitemradio' : 'menuitemcheckbox');
    ...
    #handleOutsidePointer = (event) => {
        ...
        if (!(target instanceof Node))
            return;
        if (this.#menu?.contains(target) || this.#anchor?.contains(target))
            return;
        this.close();
    };
