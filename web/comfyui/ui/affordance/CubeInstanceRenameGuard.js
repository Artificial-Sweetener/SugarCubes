...
export class CubeInstanceRenameGuard {
    ...
    #onFocusIn(event) {
        ...
        if (!input || this.#sessions.has(input))
            return;
    ...
    #scheduleRestore(input, session) {
        if (session.restoreTimer !== null)
            return;
    ...
    #resolveNode(input) {
        const header = input.closest('[data-testid^="node-header-"]');
    ...
    #restore(session) {
        const view = this.#document.defaultView;
        if (view && session.restoreTimer !== null)
            view.clearTimeout(session.restoreTimer);
        ...
        if (session.node.title !== session.instanceTitle)
            this.#nodes.changed(session.node);
    }
}
