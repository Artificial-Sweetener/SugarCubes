...
export class CubeHostAffordanceController {
    ...
    async saveCube(node) {
        if (isDraftCubeNode(node)) {
            await this.#cubeCreation.saveDraft(readInstanceId(node), readCubeNodeAuthoringCandidate(node));
            return;
        }
    ...
    setDescription(description) {
        ...
        if (!hud?.setDescription(description.trim())) {
            this.#feedback?.push?.('info', 'Cube metadata is read-only', 'Fork or sync this Cube before changing its description.');
        }
    }
    /** Clear authored implementation nodes while retaining native Cube boundaries and metadata. */
    async clearActiveCube(currentGraph) {
        const runtime = this.#getRuntime();
        const context = runtime?.contexts.resolveEditor(currentGraph) ?? null;
        if (!runtime || !context?.isCubeRoot)
            return false;
        const confirmed = await this.#confirm.open({
            title: 'Clear Cube implementation?',
            message: 'This removes every node and connection inside the Cube while keeping its inputs, outputs, and metadata.',
    ...
    blocked(operation) {
        const details = {
            ...
            'configure-interface': 'SugarCube interfaces are part of Cube authoring and are not configured through Subgraph widget tools.',
...
function requireMutableGraph(value) {
    const graph = value;
