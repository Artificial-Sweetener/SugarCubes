...
import { readIdentityPolicy, } from './HostSettingsContracts.js';
...
export class HostSettingsRepositoryController {
    ...
    async refresh({ checkForUpdates = true } = {}) {
        ...
        try {
            ...
            if (checkForUpdates && state.repos.length) {
                ...
                if (!checkResult.response.ok || checkData.error) {
                    ...
                    state.identityPolicy = readIdentityPolicy(checkData.identity_policy || state.identityPolicy);
                }
            }
        }
        catch (error) {
    ...
    async claimGithubOwner() {
        ...
        if (!normalizedOwner || normalizedOwner === currentOwner)
            return;
        if (normalizedOwner.toLowerCase() === 'artificial-sweetener' &&
        ...
        this.dependencies.pushToast('success', 'Authoring access updated', `Claimed owner is now ${normalizedOwner}.`);
    }
    /** Clear the persisted GitHub owner after confirmation. */
    async clearGithubOwner() {
        ...
        this.dependencies.pushToast('success', 'Authoring access updated', 'Tracked GitHub repos are read-only again.');
    }
    ...
    async addTrackedRepo() {
        ...
        if (!result.response.ok || data.error) {
            throw new Error(data.error?.message || result.response.statusText || 'Failed to add Cube Pack');
        }
        ...
        const detail = Number.isFinite(cubeCount)
            ...
            : '';
        this.dependencies.pushToast('success', 'Cube Pack added', `${owner}/${repo} is now tracked.${detail}`);
        await this.refresh();
        this.dependencies.invalidateDependentCatalogs();
    }
    /** Apply the available update for one tracked repository. */
    async syncTrackedRepo(repo) {
        await this.mutate(this.dependencies.cubeApi.updateCubePackNow(JSON.stringify({ owner: repo.owner, repo: repo.repo }), { headers: { 'Content-Type': 'application/json' } }), 'Failed to update Cube Pack');
        this.dependencies.pushToast('success', 'Cube Pack updated', `${repo.owner}/${repo.repo} is up to date.`);
        await this.refresh();
        this.dependencies.invalidateDependentCatalogs();
    }
    /** Apply every available tracked-repository update. */
    async syncAllTrackedRepos() {
        await this.mutate(this.dependencies.cubeApi.updateAllCubePacks(), 'Failed to update Cube Packs');
        this.dependencies.pushToast('success', 'Cube Packs updated', 'Available pack updates were applied.');
        await this.refresh();
        this.dependencies.invalidateDependentCatalogs();
    }
    /** Enable or disable one tracked repository. */
    async toggleTrackedRepo(repo) {
        await this.updateRepo(repo, { enabled: !repo.enabled });
        this.dependencies.pushToast('success', 'Cube Pack updated', `${repo.owner}/${repo.repo} ${repo.enabled ? 'disabled' : 'enabled'}.`);
        await this.refresh();
        this.dependencies.invalidateDependentCatalogs();
    }
    /** Change automatic update policy for one tracked repository. */
    async setAutoUpdate(repo, enabled) {
        await this.updateRepo(repo, { auto_update: Boolean(enabled) });
        this.dependencies.pushToast('success', 'Auto-update updated', `${repo.owner}/${repo.repo} auto-update ${enabled ? 'enabled' : 'disabled'}.`);
        await this.refresh({ checkForUpdates: false });
    }
    /** Remove one tracked repository after confirmation. */
    async removeTrackedRepo(repo) {
        const confirmed = await this.confirm({
        ...
        if (!confirmed)
            return;
        await this.mutate(this.dependencies.cubeApi.removeCubePack({ owner: repo.owner, repo: repo.repo }), 'Failed to remove Cube Pack');
        this.dependencies.pushToast('success', 'Cube Pack removed', `${repo.owner}/${repo.repo} removed from tracking.`);
        await this.refresh();
        this.dependencies.invalidateDependentCatalogs();
    }
    async updateIdentityPolicy(payload) {
        const result = await this.dependencies.cubeApi.updateIdentityPolicy(JSON.stringify(payload), {
            headers: { 'Content-Type': 'application/json' },
        });
        const data = result.data;
        if (!result.response.ok || data.error) {
            throw new Error(data.error?.message || result.response.statusText || 'Failed to update identity policy');
        }
        this.dependencies.state.identityPolicy = readIdentityPolicy(data);
        await this.refresh({ checkForUpdates: false });
    }
    async updateRepo(repo, updates) {
        await this.mutate(this.dependencies.cubeApi.updateCubePack(JSON.stringify({ owner: repo.owner, repo: repo.repo, ...updates }), { headers: { 'Content-Type': 'application/json' } }), 'Failed to update Cube Pack');
    }
    async mutate(resultPromise, fallback) {
    ...
    async confirm(options) {
        ...
        if (ui.dialogs?.confirm)
            return ui.dialogs.confirm(options);
        if (ui.confirmDialog?.open)
            return ui.confirmDialog.open(options);
