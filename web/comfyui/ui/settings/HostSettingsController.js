...
import { createRepoPanelState, } from './HostSettingsContracts.js';
...
export function createHostSettingsController(dependencies) {
    ...
    const repoPanelState = createRepoPanelState();
    ...
    function createClaimedGithubOwnerSettingRow() {
        ...
        const claimOwnerButton = createSettingsActionButton('Claim', () => {
            repositoryCommands.claimGithubOwner().catch((error) => {
        ...
        const clearClaimButton = createSettingsActionButton('Clear', () => {
            repositoryCommands.clearGithubOwner().catch((error) => {
                pushToastMessage('error', 'Settings update failed', readErrorMessage(error));
            });
        });
    ...
    function createTrackedPacksSettingRow() {
        ...
        refreshTrackedPacksSettingRow();
        if (!repoPanelState.loading) {
            repositoryCommands.refresh({ checkForUpdates: false }).catch((error) => {
    ...
    function createTrackedPackManagerDialogBody() {
        ...
        const addButton = createSettingsActionButton('Add Pack', () => {
            repositoryCommands.addTrackedRepo().catch((error) => {
        ...
        if (updateCount > 0) {
            const updateAllButton = createSettingsActionButton(updateCount > 1 ? `Update All (${updateCount})` : 'Update All', () => {
                repositoryCommands.syncAllTrackedRepos().catch((error) => {
                    const message = readErrorMessage(error);
        ...
        for (const repo of repoPanelState.repos) {
            ...
            autoUpdateToggle.addEventListener('change', () => {
                repositoryCommands.setAutoUpdate(repo, autoUpdateToggle.checked).catch((error) => {
                    autoUpdateToggle.checked = Boolean(repo.auto_update);
                    const message = readErrorMessage(error);
                    pushToastMessage('error', 'Pack update failed', message);
                });
            ...
            if (repo.enabled && repo.update_available) {
                const updateButton = createSettingsActionButton('Update', () => {
                    repositoryCommands.syncTrackedRepo(repo).catch((error) => {
                        const message = readErrorMessage(error);
                        pushToastMessage('error', 'Pack update failed', message);
                    });
                });
            ...
            if (!repo.default_base_repo) {
                const toggleButton = createSettingsActionButton(repo.enabled ? 'Disable' : 'Enable', () => {
                    repositoryCommands.toggleTrackedRepo(repo).catch((error) => {
                        const message = readErrorMessage(error);
                        pushToastMessage('error', 'Pack update failed', message);
                    });
                });
            ...
            if (!repo.default_base_repo) {
                const removeButton = createSettingsActionButton('Remove', () => {
                    repositoryCommands.removeTrackedRepo(repo).catch((error) => {
                        const message = readErrorMessage(error);
    ...
    return Object.freeze({
        ...
        refresh: (options) => repositoryCommands.refresh(options),
        refreshUi: refreshSugarCubesSettingsUi,
    });
}
