...
export function createHostSettingsController(dependencies) {
    ...
    let registeredSettingsManager = null;
    ...
    function readPackApiPayload(value) {
        return value;
    }
    function readTrackedRepos(value) {
    ...
    function summarizePackStatus(repo) {
        ...
        if ((typeof repo.last_check_status === 'string' && repo.last_check_status === 'error') ||
    ...
    function readIdentityPolicyFromPayload(payload) {
        ...
        const claimedOwner = typeof payload.claimed_github_owner === 'string' ? payload.claimed_github_owner.trim() : '';
        const allowSystemOwnerClaim = Boolean(payload.allow_system_owner_claim);
        const claimedOwnerSource = typeof payload.claimed_github_owner_source === 'string' && payload.claimed_github_owner_source
            ? payload.claimed_github_owner_source
            : 'default';
        const allowSystemOwnerClaimSource = typeof payload.allow_system_owner_claim_source === 'string' &&
    ...
    function refreshClaimedGithubOwnerSettingRow() {
        ...
        const ownerEnvManaged = isEnvManagedPolicySource(repoPanelState.identityPolicy.claimed_github_owner_source);
        ...
        appendSettingsSummaryLine(row.summary, claimedOwner
        ...
        if (!repoPanelState.identityPolicy.allow_system_owner_claim) {
            appendSettingsSummaryLine(row.summary, 'Artificial-Sweetener can only be claimed when SUGARCUBES_ALLOW_SYSTEM_OWNER_CLAIM is enabled in .env or the process environment.');
        }
        if (repoPanelState.identityPolicy.env_override_active) {
            appendSettingsSummaryLine(row.summary, 'Managed by environment configuration (.env or process environment).');
        }
        else {
            appendSettingsSummaryLine(row.summary, 'Changing this updates which tracked repos SugarCubes treats as writable on this install.');
        }
    }
    ...
    function createProximitySettingRow() {
        ...
        toggle.addEventListener('change', () => {
            ...
            pushToastMessage('success', 'Proximity links updated', toggle.checked
    ...
    function buildTrackedPackSummaryText() {
        ...
        const updateCount = repoPanelState.repos.filter((repo) => repo.enabled && repo.update_available).length;
        ...
        summaryParts.push(updateCount > 0
    ...
    function createTrackedPackManagerDialogBody() {
        ...
        const actions = documentRef.createElement('div');
        applySettingsActionGroupStyle(actions);
        const addButton = createSettingsActionButton('Add Pack', () => {
        ...
        const updateCount = repoPanelState.repos.filter((repo) => repo.enabled && repo.update_available).length;
        if (updateCount > 0) {
            const updateAllButton = createSettingsActionButton(updateCount > 1 ? `Update All (${updateCount})` : 'Update All', () => {
        ...
        for (const repo of repoPanelState.repos) {
            ...
            const detailText = (typeof repo.last_check_error === 'string' && repo.last_check_error) ||
            ...
            if (detailText) {
                ...
                Object.assign(detail.style, {
                    fontSize: '11px',
                    lineHeight: '1.45',
                    color: repo.last_check_error || repo.last_sync_error ? '#ffb3b3' : 'rgba(232, 232, 232, 0.72)',
                    whiteSpace: 'pre-wrap',
                });
    ...
    async function refreshTrackedRepoPanel({ checkForUpdates = true, } = {}) {
        ...
        try {
            ...
            if (checkForUpdates && repoPanelState.repos.length) {
                ...
                refreshSugarCubesSettingsUi();
                const checkResult = await cubeApi.checkAllCubePacks(JSON.stringify({ apply_auto_updates: true }), { headers: { 'Content-Type': 'application/json' } });
                const checkData = readPackApiPayload(checkResult.data);
                if (!checkResult.response.ok || checkData.error) {
                    const message = checkData.error?.message ||
                    ...
                    repoPanelState.repos = checkData.repos
                        ...
                        : repoPanelState.repos;
                    repoPanelState.identityPolicy = readIdentityPolicyFromPayload(checkData.identity_policy || repoPanelState.identityPolicy);
                }
            }
        }
        catch (error) {
    ...
    async function updateIdentityPolicy(payload) {
        const { response, data: rawData } = await cubeApi.updateIdentityPolicy(JSON.stringify(payload), {
            headers: { 'Content-Type': 'application/json' },
        });
        const data = readPackApiPayload(rawData);
        if (!response.ok || data.error) {
            throw new Error(data.error?.message || response.statusText || 'Failed to update identity policy');
        }
    ...
    async function handleClaimGithubOwner() {
        ...
        if (normalizeOwnerIdentity(normalizedOwner) === 'artificial-sweetener' &&
            !repoPanelState.identityPolicy.allow_system_owner_claim) {
            throw new Error('Artificial-Sweetener can only be claimed when SUGARCUBES_ALLOW_SYSTEM_OWNER_CLAIM is enabled in .env or the process environment.');
        }
        if (currentOwner) {
        ...
        pushToastMessage('success', 'Authoring access updated', `Claimed owner is now ${normalizedOwner}.`);
    }
    async function handleClearGithubOwnerClaim() {
        ...
        pushToastMessage('success', 'Authoring access updated', 'Tracked GitHub repos are read-only again.');
    }
    async function handleAddTrackedRepo() {
        ...
        const { response: preflightResponse, data: rawPreflightData } = await cubeApi.preflightCubePack(preflightBody, { headers: { 'Content-Type': 'application/json' } });
        const preflightData = readPackApiPayload(rawPreflightData);
        if (!preflightResponse.ok || preflightData.error) {
            throw new Error(preflightData.error?.message ||
                preflightResponse.statusText ||
                'Failed to verify Cube Pack');
        }
        const { response, data: rawData } = await cubeApi.addCubePack(JSON.stringify({
            owner,
            repo,
            enabled: true,
            auto_update: false,
        ...
        const cubeCountText = Number.isFinite(cubeCount)
            ...
            : '';
        pushToastMessage('success', 'Cube Pack added', `${owner}/${repo} is now tracked.${cubeCountText}`);
        await refreshTrackedRepoPanel();
        invalidateDependentCatalogs();
    }
    async function handleSyncTrackedRepo(repo) {
        const { response, data: rawData } = await cubeApi.updateCubePackNow(JSON.stringify({ owner: repo.owner, repo: repo.repo }), { headers: { 'Content-Type': 'application/json' } });
        const data = readPackApiPayload(rawData);
        if (!response.ok || data.error) {
    ...
    async function handleToggleTrackedRepo(repo) {
        const { response, data: rawData } = await cubeApi.updateCubePack(JSON.stringify({ owner: repo.owner, repo: repo.repo, enabled: !repo.enabled }), { headers: { 'Content-Type': 'application/json' } });
        const data = readPackApiPayload(rawData);
        if (!response.ok || data.error) {
            throw new Error(data.error?.message || response.statusText || 'Failed to update Cube Pack');
        }
        pushToastMessage('success', 'Cube Pack updated', `${repo.owner}/${repo.repo} ${repo.enabled ? 'disabled' : 'enabled'}.`);
        await refreshTrackedRepoPanel();
        invalidateDependentCatalogs();
    }
    async function handleSetPackAutoUpdate(repo, enabled) {
        const { response, data: rawData } = await cubeApi.updateCubePack(JSON.stringify({ owner: repo.owner, repo: repo.repo, auto_update: Boolean(enabled) }), { headers: { 'Content-Type': 'application/json' } });
        const data = readPackApiPayload(rawData);
        if (!response.ok || data.error) {
            throw new Error(data.error?.message || response.statusText || 'Failed to update Cube Pack');
        }
        pushToastMessage('success', 'Auto-update updated', `${repo.owner}/${repo.repo} auto-update ${enabled ? 'enabled' : 'disabled'}.`);
        await refreshTrackedRepoPanel({ checkForUpdates: false });
    }
    async function handleRemoveTrackedRepo(repo) {
        ...
        pushToastMessage('success', 'Cube Pack removed', `${repo.owner}/${repo.repo} removed from tracking.`);
        await refreshTrackedRepoPanel();
        invalidateDependentCatalogs();
    }
