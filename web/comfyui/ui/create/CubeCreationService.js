...
export class CubeCreationService {
  constructor({ adapter, api, toast, instanceManager, cubeBrowser, dialogs, saveReconciler } = {}) {
  ...
  collectCreateCandidateFromMarker(graph, markerNode, defaultAliasKey) {
    if (!CREATE_MARKER_TYPES.has(markerNode?.type)) {
    ...
    const enqueue = (nodeId) => {
      ...
      if (isCubeMarkerType(node)) {
        if (
          CREATE_MARKER_TYPES.has(node.type) &&
  ...
  startCreateCubePreview(candidate) {
    this.clearCreateCubePreview();
    if (![...(candidate?.nodes || []), ...(candidate?.markers || [])].some(Boolean)) return;
    ...
    if (canvas) {
      previewBackground = (...args) => {
        previousBackground?.apply(canvas, args);
        this.drawCreatePreviewGroup(args[0], canvas, candidate, bounds);
      };
  ...
  async saveCreatedCubeCandidate(candidate, values) {
    ...
    try {
      ...
      const workflowPayload = resolved?.workflow;
      if (
        !workflowPayload ||
      ...
      if (!response.ok || data?.error) {
        throw new SugarCubeExportError(
          data?.error?.message ?? response.statusText ?? 'Export failed',
          buildErrorDetail(data?.error),
          data?.error?.violations,
        );
      }
      const saved = Array.isArray(data?.saved) ? data.saved : [];
      const summary = saved.length
        ? saved.map(formatSaveSummaryEntry).join('\n')
