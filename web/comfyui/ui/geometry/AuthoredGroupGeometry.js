...
import { unionLayoutRects } from './AuthoredLayoutSolver.js';
...
export function hasAuthoredGroupGeometry(metadata) {
...
export function solveAuthoredGroupPresentationRect(baseline, solvedRects) {
    const authoredGroup = baseline.group;
    const sourceBounds = unionLayoutRects(Object.values(baseline.entries).map((entry) => authoredNodePresentationRect({
    ...
    if (!authoredGroup || !sourceBounds || !targetBounds)
        return null;
    const groupSource = {
...
export function toComfyGroupStorageRect(rect, renderer) {
    return {
        ...rect,
