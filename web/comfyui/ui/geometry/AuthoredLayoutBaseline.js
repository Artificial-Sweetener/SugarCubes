...
export function attachAuthoredLayoutBaselines(payload) {
    const layout = payload.layout;
    if (!layout)
        return payload;
    const entries = [...(payload.nodes ?? []), ...(payload.markers ?? [])];
    const entryIndex = indexEntries(entries);
    const groups = (layout.groups ?? []).map((group) => attachGroupBaseline(group, layout, entryIndex));
    return {
        ...payload,
        layout: {
            ...layout,
...
export function readPayloadAuthoredLayoutBaseline(payload) {
...
export function shiftAuthoredLayoutBaseline(metadata, shift) {
...
export function readAuthoredLayoutBaseline(value) {
...
export function translateAuthoredLayoutBaseline(baseline, shift) {
    return {
        ...baseline,
        origin: [baseline.origin[0] + shift[0], baseline.origin[1] + shift[1]],
    };
}
function attachGroupBaseline(group, layout, entryIndex) {
    ...
    for (const [, indexed] of selected) {
        ...
        if (parsed)
            entries[indexed.identity] = parsed;
    }
    if (!Object.keys(entries).length)
        return { ...group, sugarcubes: metadata };
    metadata[AUTHORED_LAYOUT_KEY] = {
...
function indexEntries(entries) {
    ...
    for (const entry of entries) {
        ...
        if (id != null && identity)
            indexed.set(String(id), { identity, entry });
        if (identity)
            indexed.set(identity, { identity, entry });
    }
    return indexed;
}
function readEntryIdentity(entry) {
    const value = entry.symbol ?? entry.alias;
    return typeof value === 'string' ? value.trim() : '';
}
function readMemberKeys(metadata) {
    const keys = new Set();
...
function readImportEntryRect(entry, origin) {
...
function readGroupRectVector(value) {
