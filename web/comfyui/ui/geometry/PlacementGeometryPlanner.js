...
import { attachAuthoredLayoutBaselines, AUTHORED_LAYOUT_KEY, readAuthoredLayoutBaseline, } from './AuthoredLayoutBaseline.js';
...
import { authoredNodePresentationRect, estimateCollapsedPresentationWidth, } from './ComfyNodeGeometry.js';
/** Return a renderer presentation clone suitable for both preview and insertion. */
export function planPlacementGeometry(payload, policy) {
    const withBaseline = attachAuthoredLayoutBaselines(clonePayload(payload));
    if (policy.renderer === 'litegraph')
        return withBaseline;
    ...
    const measurements = Object.entries(baseline.entries)
        ...
        const authoredPresentation = authoredNodePresentationRect({
            ...
            w: authored.w,
            h: authored.h,
        }, authored.collapsed, authored.title);
        return {
            identity,
            item,
    ...
    for (const item of solved)
        writeSolvedEntry(item.item, item.solved, baseline, policy);
...
function readPrimaryBaseline(groups) {
...
function forecastPresentationSize(authored, policy) {
...
function writeSolvedEntry(entry, solved, baseline, policy) {
    if (!entry.layout)
        return;
...
function writeSolvedGroup(group, baseline, solvedRects, policy) {
