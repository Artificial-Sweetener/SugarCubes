...
export function applyComfyNodeSize(node, authoredSize, policy, _logger = null) {
    const current = readVector2(node.size, 0, 0);
    const authored = readVector2(authoredSize, current[0], current[1]);
    const target = [Math.max(authored[0], policy.minimumExpandedNodeWidth), authored[1]];
    writeNodeSize(node, target);
    return target;
}
/** Estimate the renderer-neutral collapsed title rectangle from authored text. */
export function estimateCollapsedPresentationWidth(title, expandedWidth) {
    const measuredTitle = title.length * DEFAULT_TEXT_SIZE * 0.6 + DEFAULT_TITLE_HEIGHT * 2;
    return Math.min(expandedWidth, Math.max(DEFAULT_COLLAPSED_WIDTH, Number.isFinite(measuredTitle) ? measuredTitle : 0));
}
/** Convert a persisted node rect into its authored presentation rectangle. */
export function authoredNodePresentationRect(rect, collapsed, title = '') {
    if (!collapsed)
        return { ...rect };
    return {
        x: rect.x,
...
export function measureNodePresentationRect(node, context = { renderer: 'litegraph', document: null }) {
    const pos = readVector2(node.pos, Number.NaN, Number.NaN);
    const size = readVector2(node.size, Number.NaN, Number.NaN);
    if (![...pos, ...size].every(Number.isFinite))
        return null;
    const collapsed = isRecord(node.flags) && node.flags.collapsed === true;
    const renderedSize = context.renderer === 'vue' ? readMountedNodeSize(node, context.document) : null;
    if (!collapsed) {
        return {
            x: pos[0],
...
export function hasMountedNodePresentation(node, documentRef) {
...
export function writeNodePresentationPosition(node, rect, renderer = 'litegraph') {
    const collapsed = isRecord(node.flags) && node.flags.collapsed === true;
    const usesTitleOffset = renderer === 'vue' || collapsed;
    const target = [rect.x, usesTitleOffset ? rect.y + DEFAULT_TITLE_HEIGHT : rect.y];
...
function writeNodeSize(node, target) {
...
function readMountedNodeSize(node, documentRef) {
    if (!documentRef || node.id == null)
        return null;
    const identity = String(node.id);
    const elements = documentRef.querySelectorAll('[data-node-id]');
    for (const element of elements) {
        if (element.getAttribute('data-node-id') !== identity)
            continue;
