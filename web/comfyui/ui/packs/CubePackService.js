...
export class CubePackService {
  ...
  async loadCatalog() {
    ...
    if (!response?.ok || data?.error) {
      throw new Error(data?.error?.message || response?.statusText || 'Failed to load cube packs');
    }
    ...
    return { packs, identityPolicy: data?.identity_policy || {} };
  }
  ...
  async ensureClaimedOwner(identityPolicy = {}) {
    ...
    if (!response?.ok || data?.error) {
      throw new Error(data?.error?.message || response?.statusText || 'Failed to claim owner');
    }
