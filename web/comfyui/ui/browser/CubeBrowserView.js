...
export class CubeBrowserView {
  ...
  build() {
    ...
    const createActionButton = ({ className, title, outlineIcon, filledIcon, disabled = false }) =>
      $el(
        'button',
        {
          className,
    ...
    this.documentRef?.addEventListener?.('click', (event) => {
      if (versionControl && !versionControl.contains(event.target)) {
        this.commitVersionCombobox({ close: true });
      }
    });
    listBody.addEventListener('click', (event) => {
      const header = event.target.closest('[data-author]');
      if (header) {
        this.handlers.onToggleAuthorGroup?.(header.dataset.author || '');
        return;
      }
      const row = event.target.closest('[data-cube]');
      if (!row) {
        return;
      }
  ...
  getVisibleVersionEntries(options, inputValue) {
    ...
    return [entries[selectedIndex >= 0 ? selectedIndex : 0]].filter(Boolean);
  }
  ...
  moveVersionHighlight(direction) {
    ...
    this.versionComboboxOpen = true;
    this.versionHighlightedIndex = visibleEntries[nextVisibleIndex].index;
    this.renderVersionControl(this.state, this.getSelectedCubeFromState());
  }
  ...
  renderVersionControl(state, selected) {
    ...
    if (
      ...
      this.versionHighlightedIndex = visibleEntries[0].index;
    }
  ...
  renderCubeMetadataEditor(selected, state) {
    ...
    if (nameInput && inputs.derived_cube_id) {
      const updateDerivedIdentity = () => {
        ...
        inputs.derived_cube_id.value = next.value;
        inputs.derived_cube_id.title = next.error || 'Derived from Default Alias';
  ...
  resolveModelTokenBounds(value, selectionStart, selectionEnd) {
    ...
    const start = Number.isInteger(selectionStart) ? selectionStart : safeValue.length;
  ...
  getPreviewElements() {
    return {
      canvas: this.elements.previewCanvas,
      container: this.elements.previewContainer,
...
function deriveEditableCubeId(cubeId, defaultAlias, targetModel = '') {
  try {
    ...
    return {
      value: fallback,
      error: error?.message || 'Unable to derive cube id from Default Alias',
    };
  }
}
