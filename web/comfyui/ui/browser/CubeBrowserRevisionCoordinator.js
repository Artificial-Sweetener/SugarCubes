...
import { CURRENT_REVISION_REF, normalizeCubeVersion, normalizeRevisionRef, } from '../core/CubeDefinitionKey.js';
...
export class CubeBrowserRevisionCoordinator {
    ...
    async requestRevisions(cubeKey) {
        ...
        try {
            ...
            if (!response.ok || hasApiError(data)) {
                this.reportFailure(cubeId, readApiErrorMessage(data, response.statusText || 'Failed to load revisions'));
                return;
            }
            ...
            const activeRevision = versionOptions.some((entry) => entry?.revisionRef === store.state.selectedRevision)
                ? store.state.selectedRevision
                : CURRENT_REVISION_REF;
            store.setSelectedRevision(activeRevision);
            const activeOption = versionOptions.find((entry) => entry.revisionRef === activeRevision) ||
                versionOptions[0] ||
                null;
            if (activeOption) {
    ...
    selectVersion(version) {
        ...
        const option = this.options.store.state.versionOptions.find((entry) => entry.value === normalized);
        if (option) {
    ...
    commitVersionInput(version) {
        ...
        const fallback = options.find((entry) => entry.value === store.state.selectedVersion) ||
    ...
    findClosestVersionOption(typedValue, options = []) {
        const scored = options
            ...
            .sort((left, right) => left.score.category !== right.score.category
    ...
    getSelectedVersionOption() {
        const { store } = this.options;
        return (store.state.versionOptions.find((entry) => entry.revisionRef === store.state.selectedRevision) ||
            store.state.versionOptions.find((entry) => entry.value === store.state.selectedVersion) ||
            null);
    }
    reportFailure(cubeId, message) {
        const { store } = this.options;
    ...
    score(typedValue, option, index) {
        ...
        return {
            category: 3,
            distance: Math.abs(valueParts[0] - typedParts[0]) * 1000000 +
