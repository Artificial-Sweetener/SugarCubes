...
export class CubeBrowserViewBuilder {
    ...
    build() {
        ...
        const action = (className, title, outlineIcon, filledIcon) => $el('button', { className, type: 'button', title, disabled: true }, [
        ...
        const editButton = action('sugarcubes-browser__edit', 'Edit cube metadata', 'mdi-pencil-outline', 'mdi-pencil');
        const editSaveButton = action('sugarcubes-browser__edit-save sugarcubes-browser__action-hidden', 'Save changes', 'mdi-content-save-outline', 'mdi-content-save');
        const editCancelButton = action('sugarcubes-browser__edit-cancel sugarcubes-browser__action-hidden', 'Cancel edits', 'mdi-close-circle-outline', 'mdi-close-circle');
        const deleteButton = action('sugarcubes-browser__delete', 'Delete cube', 'mdi-trash-can-outline', 'mdi-trash-can');
        const promoteButton = action('sugarcubes-browser__promote', 'Move to cube pack', 'mdi-package-up', 'mdi-package-up');
        const placeButton = action('sugarcubes-browser__place', 'Place', 'mdi-arrow-right-bold-outline', 'mdi-arrow-right-bold');
        ...
        searchInput.addEventListener('input', (event) => {
            handlers().onSearchChange?.(event.target instanceof HTMLInputElement ? event.target.value : '');
        });
        listBody.addEventListener('click', (event) => {
            ...
            if (row)
                handlers().onSelect?.(row.dataset.cube || '', { focus: true });
        });
        listBody.addEventListener('dblclick', (event) => {
            const row = (event.target instanceof Element ? event.target : null)?.closest('[data-cube]');
            if (!row)
                return;
