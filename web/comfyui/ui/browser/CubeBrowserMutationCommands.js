...
export class CubeBrowserMutationCommands {
    ...
    async clearLineage(selected) {
        ...
        if (!selected.is_writable) {
            this.options.toast?.push('warn', 'Read-only cube', selected.write_block_reason || 'This cube is read-only.');
            return;
        }
        ...
        await this.withBusy(async () => {
            try {
                const { response, data } = await this.options.api.updateMetadata(JSON.stringify({ cube_id: selected.cube_id, metadata: { lineage: null } }), { headers: { 'Content-Type': 'application/json' } });
                if (!response.ok || hasApiError(data)) {
                    this.options.toast?.push('error', 'Lineage update failed', readApiErrorMessage(data, response.statusText || 'Update failed'));
                    return;
                }
    ...
    async requestDelete() {
        ...
        await this.withBusy(async () => {
            try {
                ...
                if (!response.ok || hasApiError(data)) {
                    this.options.toast?.push('error', 'Delete failed', readApiErrorMessage(data, response.statusText || 'Failed to delete cube'));
                    return;
                }
