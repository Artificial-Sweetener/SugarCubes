...
export class CubePreviewRenderer {
  constructor({ adapter, helpers = {}, placement = {} } = {}) {
  ...
  update({ payload, name, requestKey, loading, error } = {}) {
  ...
  handlePointerDown(event) {
    ...
    Promise.resolve(this.placement?.start?.(cubeId, { closeBrowser: false, defaultAlias })).then(
      () => {
  ...
  endPreviewDrag(event, placeOnRelease) {
    ...
    if (placeOnRelease && this.placement?.isPointerOverCanvas?.(event)) {
      ...
      Promise.resolve(this.placement?.commit?.()).finally(() => {
