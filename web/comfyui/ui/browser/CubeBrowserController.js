...
/**
 * Own the SugarCubes cube browser layer in `web/comfyui/ui/browser/CubeBrowserController.js`.
 */
...
function resolveBrowserApi(api) {
    return {
...
export class CubeBrowserController {
    ...
    bindHandlers() {
        this.view.setHandlers({
            ...
            onVersionClosest: (version) => this.findClosestVersionOption(version, this.store.state.versionOptions),
        });
    }
    ...
    open({ triggerButton } = {}) {
        ...
        void this.ensureInitialized().then(() => {
            ...
            const shouldRefresh = !this.store.state.cubes.length ||
    ...
    sortFavoritesFirst(list) {
        ...
        const compare = (left, right) => [
            ...
            return String(left?.[key] || fallback).localeCompare(String(right?.[key] || fallback), undefined, {
                sensitivity: 'base',
            });
        }, 0);
    ...
    ensureAuthorGroupDefaults(grouped) {
    ...
    async requestRevisions(cubeKey) {
        const key = typeof cubeKey === 'string' ? cubeKey.trim() : '';
        if (!key) {
            this.store.setRevisions([], null);
            this.store.setSelectedRevision(CURRENT_REVISION_REF);
    ...
    selectRevision(revisionRef) {
        ...
        const option = this.store.state.versionOptions.find((entry) => entry.revisionRef === normalized);
        if (option) {
    ...
    commitVersionInput(version) {
        ...
        const selectedFallback = options.find((entry) => entry.value === this.store.state.selectedVersion) ||
    ...
    findClosestVersionOption(typedValue, options = []) {
    ...
    scoreVersionOption(typedValue, option, index) {
        ...
        const distance = Math.abs(valueParts.major - typedParts.major) * 1000000 +
    ...
    render() {
        ...
        const displayName = typeof selectedEntry?.display_name === 'string' && selectedEntry.display_name.trim()
    ...
    toggleEdit() {
        ...
        if (!this.isWritableCube(selected)) {
            const reason = typeof selected?.write_block_reason === 'string' && selected.write_block_reason.trim()
    ...
    async saveEdit() {
        ...
        const newName = normalizeDefaultAliasTitle(String(inputs.name?.value || '')
            .split('/')
            .pop());
        const originalEditableName = typeof this.store.state.editDraft?.original_name === 'string' &&
        ...
        const selectedTargetModel = normalizeTargetModel(inputs.target_model?.value) ||
        ...
        const metadata = {
            ...
            supported_models: normalizeSupportedModels(this.getSupportedModelValues(inputs.supported_models), { targetModel: selectedTargetModel }),
        };
        ...
        try {
            ...
            if (targetCubeId && targetCubeId !== selected.cube_id) {
                ...
                const { response: renameResponse, data: renameData } = await this.api.rename(JSON.stringify({
                    cube_id: selected.cube_id || '',
                    default_alias: routeAlias,
                    target_cube_id: targetCubeId,
                }), { headers: { 'Content-Type': 'application/json' } });
                if (!renameResponse.ok || hasApiError(renameData)) {
                    const message = readApiErrorMessage(renameData, renameResponse.statusText || 'Rename failed');
    ...
    async clearLineage(selected) {
        ...
        if (!this.isWritableCube(selected)) {
            this.toast?.push('warn', 'Read-only cube', selected?.write_block_reason || 'This cube is read-only.');
            return;
        }
        const confirmed = await this.actions.openConfirmDialog?.({
            title: 'Clear lineage?',
        ...
        this.render();
        try {
            const { response, data } = await this.api.updateMetadata(JSON.stringify({
                cube_id: selected.cube_id,
    ...
    getSelectedVersionOption() {
        return (this.store.state.versionOptions.find((entry) => entry.revisionRef === this.store.state.selectedRevision) ||
            this.store.state.versionOptions.find((entry) => entry.value === this.store.state.selectedVersion) ||
            null);
    }
    moveSelection(delta) {
        ...
        const currentIndex = this.store.state.filtered.findIndex((cube) => this.getCubeKey(cube) === this.store.state.selected);
    ...
    async requestPreview(cubeKey) {
        ...
        const selected = this.getCubeBySelectionKey(key);
        const cubeId = selected?.cube_id || '';
        const revisionRef = typeof this.store.state.selectedRevision === 'string' && this.store.state.selectedRevision
            ? this.store.state.selectedRevision
            : 'WORKTREE';
    ...
    buildEditDraft(selected) {
        ...
        const targetModel = normalizeTargetModel(selected.target_model) ||
            this.deriveTargetModelFromCubeIdSafe(selected.cube_id);
    ...
    async loadSupportedModels() {
        try {
            ...
            const options = normalizeSupportedModels(text
                .split(/\r?\n/)
                .map((line) => line.trim())
                .filter(Boolean));
            if (!options.length) {
    ...
    async fetchSupportedModelList(fetchRef) {
 ...
 */
function deriveCubePackGroup(cube) {
 ...
 */
function deriveLegacyAuthorPackGroup(cube) {
