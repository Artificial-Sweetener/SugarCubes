...
export class CubeBrowserController {
  constructor({ adapter, api, events, storage, toast, scheduler } = {}) {
  ...
  async ensureInitialized() {
    ...
    this.loadSupportedModels().catch(() => {});
  }
  ...
  openForEdit({ cubeId, defaultAlias: _defaultAlias } = {}) {
    this.ensureInitialized().then(() => {
      if (!this.ensureView()) {
        return;
      }
  ...
  async refresh({ force = false } = {}) {
    await this.ensureInitialized();
    if (this.store.state.loading && this.fetchController) {
      return this.store.state.cubes;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const message = data?.error?.message || response.statusText || 'Failed to load cubes';
      ...
      const cubes = Array.isArray(data?.cubes)
        ? data.cubes.map((cube) => this.normalizeCatalogCube(cube))
        : [];
  ...
  selectCube(cubeKey, options = {}) {
    ...
    this.applyFilters();
    this.render();
    this.requestPreview(this.store.state.selected);
  ...
  async requestRevisions(cubeKey) {
    ...
    try {
      ...
      if (!response.ok || data?.error) {
        const message = data?.error?.message || response.statusText || 'Failed to load revisions';
  ...
  render() {
    ...
    this.requestPreview(this.getCubeKey(selectedEntry));
  }
  ...
  async saveEdit() {
    ...
    let version = inputs.version?.value?.trim() || '';
    ...
    try {
      ...
      if (targetCubeId && targetCubeId !== selected.cube_id) {
        ...
        if (!renameResponse.ok || renameData?.error) {
          const message =
            renameData?.error?.message || renameResponse.statusText || 'Rename failed';
      ...
      if (!response.ok || data?.error) {
        const message = data?.error?.message || response.statusText || 'Update failed';
  ...
  async requestDelete() {
    ...
    try {
      ...
      if (!response.ok || data?.error) {
        const message = data?.error?.message || response.statusText || 'Failed to delete cube';
  ...
  async requestPreview(cubeKey) {
    ...
    if (this.preview.requestKey === previewKey && this.preview.loading) {
      return;
    }
    if (this.preview.requestKey === previewKey && this.preview.payload && !this.preview.error) {
      this.preview.render();
      return;
    }
    if (!cubeId) {
    ...
    try {
      ...
      if (!response.ok || data?.error) {
        ...
        const message = errorPayload.message || response.statusText || 'Preview failed';
        this.preview.update({
          name: key,
          requestKey: previewKey,
          payload: null,
...
function deriveCubePackGroup(cube) {
  const source = cube?.source && typeof cube.source === 'object' ? cube.source : {};
