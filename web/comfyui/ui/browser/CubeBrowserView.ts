...
/**
 * Own the SugarCubes cube browser layer in `web/comfyui/ui/browser/CubeBrowserView.js`.
 */
...
function isLocalCubeEntry(cube: CubeLibraryEntry | null | undefined): boolean {
  const sourceType =
    typeof cube?.source?.type === 'string' ? cube.source.type.trim().toLowerCase() : '';
  const cubeId = typeof cube?.cube_id === 'string' ? cube.cube_id.trim().toLowerCase() : '';
  return sourceType === 'local' || cubeId.startsWith('local/');
}

function isPersonalCubeEntry(cube: CubeLibraryEntry | null | undefined): boolean {
  const cubeId = typeof cube?.cube_id === 'string' ? cube.cube_id.trim().toLowerCase() : '';
...
export class CubeBrowserView {
  ...
  editInputs: BrowserEditInputs | null;
  private state: CubeBrowserState | null;
  private editIdentityInvalid: boolean;
  private versionComboboxOpen: boolean;
  private versionHighlightedIndex: number;
  private versionInputDirty: boolean;
  private versionRenderSignature: string;
  ...
  build(): Partial<BrowserElements> {
    const createIcon = (...classNames: string[]): HTMLElement => {
  ...
  updateSearch(state: CubeBrowserState): void {
    const searchInput = this.elements.searchInput;
    if (!searchInput) {
      return;
    }
  ...
  updateEmptyState(state: CubeBrowserState): void {
  ...
  updateList(state: CubeBrowserState): void {
  ...
  updateSelectionStyles(state: CubeBrowserState): void {
    const listBody = this.elements.listBody;
    if (!listBody) {
      return;
    }
    const rows = listBody.querySelectorAll<HTMLElement>('[data-cube]');
    rows.forEach((row) => {
  ...
  updateDetails(state: CubeBrowserState): void {
  ...
  getSelectedCubeFromState(state: CubeBrowserState | null = this.state): CubeLibraryEntry | null {
    if (!state) {
      return null;
    }
  ...
  resetVersionComboboxState(): void {
    this.versionComboboxOpen = false;
    this.versionHighlightedIndex = -1;
    this.versionInputDirty = false;
  }
  ...
  handleVersionKeydown(event: KeyboardEvent): void {
    const options = Array.isArray(this.state?.versionOptions) ? this.state.versionOptions : [];
    if (!options.length) {
      return;
    }
  ...
  updateVersionHighlight(inputValue: string): void {
    const options = Array.isArray(this.state?.versionOptions) ? this.state.versionOptions : [];
    if (!options.length) {
      this.versionHighlightedIndex = -1;
      return;
    ...
    const closestIndex = closest
      ? options.findIndex(
          (option) => option.value === closest.value && option.revisionRef === closest.revisionRef,
        )
      : -1;
    if (closestIndex >= 0) {
    ...
    const selectedIndex = options.findIndex(
      (option) => option.value === this.state?.selectedVersion,
    );
    this.versionHighlightedIndex = selectedIndex >= 0 ? selectedIndex : 0;
  }

  getVisibleVersionEntries(
    ...
    const matchingEntries = entries.filter(({ option }) =>
      normalizeVersionQuery(option.value).toLowerCase().includes(query),
    );
  ...
  moveVersionHighlight(direction: number): void {
    const input = this.elements.versionInput;
    const options = Array.isArray(this.state?.versionOptions) ? this.state.versionOptions : [];
    ...
    const currentVisibleIndex = visibleEntries.findIndex(
      (entry) => entry.index === this.versionHighlightedIndex,
    );
  ...
  commitVersionCombobox({ close = false }: { close?: boolean } = {}): void {
    const input = this.elements.versionInput;
    const options = Array.isArray(this.state?.versionOptions) ? this.state.versionOptions : [];
    if (!input || input.disabled || !options.length) {
      if (close) {
    ...
    const committedValue =
      committed?.value || this.resolveSelectedVersionValue() || options[0]?.value || '';
  ...
  resolveSelectedVersionValue(): string {
    const options = Array.isArray(this.state?.versionOptions) ? this.state.versionOptions : [];
    return (
      options.find((option) => option.value === this.state?.selectedVersion)?.value ||
      options[0]?.value ||
      ''
    );
  }

  renderVersionControl(state: CubeBrowserState | null, selected: CubeLibraryEntry | null): void {
    ...
    if (!listbox.hidden && this.versionHighlightedIndex >= 0) {
      input.setAttribute(
        ...
        `${listbox.id}-option-${this.versionHighlightedIndex}`,
      );
    } else {
      input.removeAttribute('aria-activedescendant');
    }
  ...
  updateActionState(state: CubeBrowserState): void {
    const placeButton = this.elements.placeButton;
    const deleteButton = this.elements.deleteButton;
    const promoteButton = this.elements.promoteButton;
    const editButton = this.elements.editButton;
  ...
  isCubeDirty(cube: CubeLibraryEntry, state: CubeBrowserState): boolean {
    const cubeId = typeof cube?.cube_id === 'string' ? cube.cube_id.trim() : '';
    return Boolean(cubeId && state.dirtyCubeIds?.has(cubeId));
  }

  updateDetailIcon(selected: CubeLibraryEntry | null): void {
    const detailIcon = this.elements.detailIcon;
    if (!detailIcon || !this.documentRef) {
      return;
    }
    const icon = createCubeIconElement(
      this.documentRef,
      {
        icon: selected?.icon,
   ...
   */
  renderCubeMetadataReadOnly(selected: CubeLibraryEntry, _state: CubeBrowserState): void {
    const detailMeta = this.elements.detailMeta;
    const detailDescription = this.elements.detailDescription;
    const detailTitle = this.elements.detailTitle;
  ...
  renderCubeMetadataEditor(selected: CubeLibraryEntry, state: CubeBrowserState): void {
    ...
    const derivedIdentity = deriveEditableCubeId(
      selected.cube_id,
      draft.name || '',
      draft.target_model || '',
    );
    ...
    if (nameInput && derivedCubeIdInput) {
      const updateDerivedIdentity = () => {
        const next = deriveEditableCubeId(
          selected.cube_id,
          nameInput.value || '',
          inputs.target_model?.value || '',
        );
    ...
    const descriptionNodes: Node[] = [descriptionInput];
    ...
    if (selected.is_writable && selected.lineage && typeof selected.lineage === 'object') {
      const clearButton = $el(
        'button.sugarcubes-browser__lineage-clear',
        {
          type: 'button',
  ...
  renderTargetModelEditor(targetModel: unknown): TargetModelEditor {
    const input = $el('select', {
      className: 'sugarcubes-browser__target-model-select',
    });
    const normalizedTarget = normalizeTargetModel(targetModel);
    const options =
      normalizedTarget && !TARGET_MODEL_OPTIONS.includes(normalizedTarget)
...
function draftNameFromState(state: CubeBrowserState, selected: CubeLibraryEntry): string {
...
function deriveTargetModelFromCubeIdSafe(cubeId: unknown): string {
  try {
...
function normalizeVersionQuery(value: unknown): string {
  if (typeof value !== 'string') {
    return '';
  }
...
function getCubeSelectionKey(cube: CubeLibraryEntry | null | undefined): string {
