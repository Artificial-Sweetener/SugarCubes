...
export class ModelAutocompleteControl {
    ...
    #options;
    #positionSuggestions = () => {
        if (this.listbox.hidden || !this.input.isConnected || !this.listbox.isConnected)
            return;
        ...
        const left = Math.min(Math.max(viewportMargin, inputBounds.left), Math.max(viewportMargin, windowRef.innerWidth - viewportMargin - width));
        ...
        if (openAbove) {
            ...
            this.listbox.style.bottom = `${Math.max(viewportMargin, windowRef.innerHeight - inputBounds.top + popupGap)}px`;
            return;
        }
        this.listbox.style.top = `${Math.max(viewportMargin, Math.min(inputBounds.bottom + popupGap, windowRef.innerHeight - viewportMargin))}px`;
    ...
    constructor(options) {
        ...
        this.element.className = joinClassNames('sugarcubes-model-autocomplete', options.legacyClassNames?.container);
        this.input = this.#document.createElement('input');
        this.input.type = 'text';
        this.input.className = joinClassNames('sugarcubes-model-autocomplete__input', options.inputClassName, options.legacyClassNames?.input);
        ...
        this.listbox.className = joinClassNames('sugarcubes-model-autocomplete__suggestions', options.legacyClassNames?.listbox);
    ...
    #renderSuggestions(preserveHighlight = false) {
        const bounds = resolveModelTokenBounds(this.input.value, this.input.selectionStart, this.input.selectionEnd);
        const matches = buildModelSuggestions(bounds.token, this.#options, this.input.value);
        const previousValue = preserveHighlight && this.#highlightedIndex >= 0
        ...
        const children = matches.map((value, index) => this.#createSuggestion(value, index, index === this.#highlightedIndex));
    ...
    #createSuggestion(value, index, highlighted) {
        const button = this.#document.createElement('button');
        button.id = `${this.listbox.id}-option-${index}`;
        button.type = 'button';
        button.className = joinClassNames('sugarcubes-model-autocomplete__suggestion', this.#suggestionClassName);
    ...
    #commitSuggestion(value) {
        if (!value)
            return;
        const { tokenStart, tokenEnd } = resolveModelTokenBounds(this.input.value, this.input.selectionStart, this.input.selectionEnd);
...
function buildModelSuggestions(token, options, fullValue) {
    const normalizedToken = typeof token === 'string' ? token.trim().toLowerCase() : '';
    if (!normalizedToken)
        return [];
    const existingValues = new Set(parseModelInputValue(fullValue)
...
function parseModelInputValue(value) {
...
function resolveModelTokenBounds(value, selectionStart, selectionEnd) {
...
function ensureModelAutocompleteStyles(documentRef) {
