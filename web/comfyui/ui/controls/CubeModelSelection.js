...
export function targetModelSelectOptions(suggestions = []) {
    return [
...
export function isCustomTargetModel(value, suggestions = []) {
    if (!value)
        return false;
    const knownModels = new Set(uniqueModelNames([...TARGET_MODEL_OPTIONS, ...suggestions]).map((model) => model.toLocaleLowerCase()));
...
export function supportedModelSuggestions(suggestions, selected) {
