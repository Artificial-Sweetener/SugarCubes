...
import { findGroupForCubeId, findGroupForMarkerId, findGroupForNodeId, syncInstanceAlias, } from '../comfyui/ui/graph/InstanceAliasSync.js';
...
function findNode(graph, nodeId) {
    ...
    return graph._nodes?.find((node) => node.id === nodeId) || null;
}
function getFirstInputLink(node) {
    ...
    if (!slot)
        return null;
    const linkId = slot.link ?? (Array.isArray(slot.links) && slot.links.length ? slot.links[0] : null);
...
function ensureCreateCubeWidget(node) {
    ...
    const widget = node.addWidget('button', 'Create cube', null, () => startCreateCubeFromMarker(node), { serialize: false });
    if (!widget) {
        return null;
    }
...
function asMarkerNode(node) {
    if (typeof node.type !== 'string' || node.id == null)
        return null;
    return node;
}
...
function applyColors(node, type) {
    ...
    if (colors && colors.color && colors.bgcolor) {
        ...
        if (entry.defaultColor === undefined)
            delete node.color;
        else
            node.color = entry.defaultColor;
        if (entry.defaultBgColor === undefined)
            delete node.bgcolor;
        else
...
function touchWidget(node) {
    ...
    if (widget && !widget[WIDGET_FLAG]) {
        ...
        widget.callback = function (...args) {
            const previous = typeof widget.__sugarcubes_last_value === 'string'
            ...
            const next = typeof args[0] === 'string'
                ? args[0]
                : typeof widget.value === 'string'
                    ? widget.value
                    : '';
...
function cleanSlotList(list) {
    if (!Array.isArray(list))
        return [];
    return list.filter((value) => (typeof value === 'string' || typeof value === 'number') && Number(value) >= 0);
}
...
function wrapLifecycle(nodeType) {
    ...
    proto.onWidgetChanged = function (...args) {
    ...
    proto.getExtraMenuOptions = function (...args) {
        ...
        options.push({
            ...
            callback: async () => {
                ...
                const defaultAlias = currentDefaultAlias ||
...
app.registerExtension({
    ...
    beforeRegisterNodeDef(nodeType, nodeData) {
        if (typeof nodeData.name !== 'string' || !NODE_TYPES.has(nodeData.name))
            return;
        wrapLifecycle(nodeType);
    },
    nodeCreated(node) {
        const marker = asMarkerNode(node);
        if (!marker || !NODE_TYPES.has(marker.type))
            return;
